...
public final class LevenshteinDistance {
    ...
    public static int computeIterative(String first, String second) {
        ...
        // Distance from prefix of first to empty second (all deletions)
        for (int i = 0; i <= firstLength; i++) {
            dp[i][0] = i;
        }

        // Distance from empty first to prefix of second (all insertions)
        ...
        for (int i = 0; i < firstLength; i++) {
            ...
            for (int j = 0; j < secondLength; j++) {
                ...
                if (firstChar == secondChar) {
                    // No operation needed if characters are equal
                    dp[i + 1][j + 1] = dp[i][j];
                } else {
