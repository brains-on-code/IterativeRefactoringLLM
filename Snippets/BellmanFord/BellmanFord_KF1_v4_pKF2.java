...
class BellmanFord {
    ...
    public void runFromInput() {
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            Edge[] inputEdges = readEdgesFromInput(scanner, edgesCount);

            int[] distance = new int[vertices];
            int[] parent = new int[vertices];

            int sourceVertex = 0;
            initializeSingleSource(distance, parent, sourceVertex);

            relaxEdges(vertices, edgesCount, inputEdges, distance, parent);

            boolean hasNegativeCycle = detectNegativeCycle(edgesCount, inputEdges, distance);

            if (!hasNegativeCycle) {
                printDistancesFromSource(distance, sourceVertex);
                printAllPathsFromSource(parent, sourceVertex);
            }
        }
    }

    private Edge[] readEdgesFromInput(Scanner scanner, int edgesCount) {
    ...
    private void initializeSingleSource(int[] distance, int[] parent, int sourceVertex) {
    ...
    private boolean detectNegativeCycle(int edgesCount, Edge[] edges, int[] distance) {
        for (int j = 0; j < edgesCount; j++) {
            Edge edge = edges[j];
            if (distance[edge.source] == Integer.MAX_VALUE) {
                continue;
    ...
    private void printDistancesFromSource(int[] distance, int sourceVertex) {
        System.out.println("Vertex distances from source (" + sourceVertex + "):");
        for (int i = 0; i < distance.length; i++) {
            System.out.println(i + " " + distance[i]);
        }
    }

    private void printAllPathsFromSource(int[] parent, int sourceVertex) {
    ...
    public void shortestPath(int sourceVertex, int targetVertex, Edge[] graphEdges) {
        ...
        if (!hasNegativeCycle) {
            System.out.println(
                "Shortest distance from " + sourceVertex + " to " + targetVertex + " is: " + distance[targetVertex]
            );
