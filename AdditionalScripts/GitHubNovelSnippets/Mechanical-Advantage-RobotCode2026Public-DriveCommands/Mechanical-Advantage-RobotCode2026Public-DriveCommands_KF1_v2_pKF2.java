...
public class Class1 {
  ...
  /**
   * Processes joystick X/Y inputs into a translation vector:
   * - Applies deadband
   * - Computes magnitude and direction
   * - Squares magnitude for finer low-speed control
   */
  ...
  /** Processes a single joystick input (e.g. rotation) with deadband and signed squaring. */
   ...
   * <p>Translation:
   * - Squared magnitude
   * - Scaled by {@link DriveConstants#maxLinearSpeed}
   *
   * <p>Rotation:
   * - Squared
   * - Scaled by {@link DriveConstants#maxAngularSpeed}
   *
   ...
   * Returns true when:
   * - Robot is enabled
   * - Robot heading is within the configured angular tolerance of the desired launch drive angle
   */
  @AutoLogOutput
  ...
  /**
   * Field-relative drive command that:
   * - Aligns robot heading to the launch angle using a profiled PID controller
   * - Allows manual translation control via joystick
   *
  ...
  /**
   * Characterizes wheel radius by:
   * - Spinning the robot in place at a constant angular velocity
   * - Comparing gyro rotation to wheel encoder rotation
   *
   * <p>Intermediate values are logged to AdvantageScope; final results are printed to the console.
   */
