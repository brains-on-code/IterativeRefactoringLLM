...
public final class MatrixRank {
    ...
    public static int computeRank(double[][] matrix) {
        ...
        for (int columnIndex = 0; columnIndex < columnCount; ++columnIndex) {
            int pivotRowIndex = findPivotRow(workingMatrix, isRowUsedAsPivot, columnIndex);
            if (pivotRowIndex != rowCount) {
                ++rank;
                isRowUsedAsPivot[pivotRowIndex] = true;
                normalizePivotRow(workingMatrix, pivotRowIndex, columnIndex);
                eliminateRows(workingMatrix, pivotRowIndex, columnIndex);
            }
        }
        return rank;
    }
     ...
     *
     * @param matrix The input matrix
     * @param isRowUsedAsPivot An array indicating which rows have been used as pivot rows
     * @param columnIndex The column index
     * @return The pivot row index, or the number of rows if no suitable pivot row was found
     */
    private static int findPivotRow(double[][] matrix, boolean[] isRowUsedAsPivot, int columnIndex) {
        int rowCount = matrix.length;
        for (int rowIndex = 0; rowIndex < rowCount; ++rowIndex) {
    ...
    private static void normalizePivotRow(double[][] matrix, int pivotRowIndex, int columnIndex) {
        ...
        for (int nextColumnIndex = columnIndex + 1; nextColumnIndex < columnCount; ++nextColumnIndex) {
            matrix[pivotRowIndex][nextColumnIndex] /= pivotValue;
        }
    }
     ...
     * @param columnIndex The column index
     */
    private static void eliminateRows(double[][] matrix, int pivotRowIndex, int columnIndex) {
        int rowCount = matrix.length;
        int columnCount = matrix[0].length;
        for (int rowIndex = 0; rowIndex < rowCount; ++rowIndex) {
            ...
            double factor = matrix[rowIndex][columnIndex];
            for (int currentColumnIndex = columnIndex + 1; currentColumnIndex < columnCount; ++currentColumnIndex) {
                matrix[rowIndex][currentColumnIndex] -= matrix[pivotRowIndex][currentColumnIndex] * factor;
            }
        }
    }
}