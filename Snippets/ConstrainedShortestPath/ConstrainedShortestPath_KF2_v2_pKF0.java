...
public class ConstrainedShortestPath {
    ...
    public int solve(int start, int target) {
        ...
        int[][] minCostForResource = initializeDpTable(numNodes);

        minCostForResource[0][start] = 0;
        relaxEdges(numNodes, minCostForResource);
        int minCost = findMinCostToTarget(target, minCostForResource);

        return minCost == Integer.MAX_VALUE ? -1 : minCost;
    }

    private int[][] initializeDpTable(int numNodes) {
    ...
    private void relaxEdges(int numNodes, int[][] dp) {
        for (int usedResource = 0; usedResource <= maxResource; usedResource++) {
            for (int node = 0; node < numNodes; node++) {
                int currentCost = dp[usedResource][node];
                ...
                for (Graph.Edge edge : graph.getEdges(node)) {
                    ...
                    if (newCost < dp[newResource][nextNode]) {
                        dp[newResource][nextNode] = newCost;
                    }
                }
            }
        }
    ...
    private int findMinCostToTarget(int target, int[][] dp) {
