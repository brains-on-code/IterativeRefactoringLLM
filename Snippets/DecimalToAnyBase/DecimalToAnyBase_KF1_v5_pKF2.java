...
/**
 * Utility class for converting non-negative integers to their string
 * representation in an arbitrary base between 2 and 36.
 */
public final class IntegerBaseConverter {
     ...
     * @throws IllegalArgumentException if {@code digit} is outside the range 0–35
     */
    private static char toDigitChar(int digit) {
        if (digit < 0 || digit >= MAX_BASE) {
            throw new IllegalArgumentException("Digit must be between 0 and " + (MAX_BASE - 1));
        }
