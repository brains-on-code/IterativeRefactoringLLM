...
final class NearestRightKey {
    ...
    public static void main(String[] args) {
        BinarySearchTreeNode rootNode = buildRandomTree();
        Scanner scanner = new Scanner(System.in);

        System.out.print("Enter number: ");
        int targetValue = scanner.nextInt();

        int nearestRightKey = findNearestRightKey(rootNode, targetValue);
    ...
    public static BinarySearchTreeNode buildRandomTree() {
        ...
        BinarySearchTreeNode rootNode = new BinarySearchTreeNode(null, null, randomValue);

        for (int i = 0; i < 1000; i++) {
            randomValue = ThreadLocalRandom.current().nextInt(0, 101);
            rootNode = rootNode.insert(rootNode, randomValue);
        }
