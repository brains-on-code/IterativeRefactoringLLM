...
public final class CoinChange {
    ...
    public static int change(int[] coins, int amount) {
        ...
        for (int coin : coins) {
            updateCombinationsForCoin(combinations, coin, amount);
        }

        return combinations[amount];
    }

    private static void updateCombinationsForCoin(int[] combinations, int coin, int amount) {
    ...
    public static int minimumCoins(int[] coins, int amount) {
        ...
        int[] minCoins = initializeMinCoinsArray(amount);

        for (int currentAmount = 1; currentAmount <= amount; currentAmount++) {
            minCoins[currentAmount] = findMinimumCoinsForAmount(coins, minCoins, currentAmount);
        }

        return minCoins[amount];
    }

    private static int[] initializeMinCoinsArray(int amount) {
    ...
    private static int findMinimumCoinsForAmount(int[] coins, int[] minCoins, int currentAmount) {
