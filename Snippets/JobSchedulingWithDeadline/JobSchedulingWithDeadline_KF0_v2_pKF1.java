    ...
    public static int[] jobSequencingWithDeadlines(Job[] jobs) {
        ...
        for (Job job : jobs) {
            if (job.arrivalTime <= job.deadline) {
                int latestAvailableSlotIndex = Math.min(job.deadline - 1, maxDeadline - 1);
                int earliestAvailableSlotIndex = job.arrivalTime - 1;

                for (int slotIndex = latestAvailableSlotIndex; slotIndex >= earliestAvailableSlotIndex; slotIndex--) {
