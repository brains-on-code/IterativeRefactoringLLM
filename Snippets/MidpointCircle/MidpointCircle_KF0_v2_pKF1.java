...
public final class MidpointCircle {
    ...
    public static List<int[]> generateCirclePoints(int centerX, int centerY, int radius) {
        ...
        addSymmetricPoints(circlePoints, centerX, centerY, xOffset, yOffset);

        while (xOffset > yOffset) {
            yOffset++;

            if (decisionParameter <= 0) {
                decisionParameter = decisionParameter + 2 * yOffset + 1;
            } else {
                xOffset--;
                decisionParameter = decisionParameter + 2 * yOffset - 2 * xOffset + 1;
            }
