    ...
    public static int[] scheduleJobsWithDeadlines(Job[] jobs) {
        ...
        for (Job job : jobs) {
            if (job.startTime <= job.deadline) {
                int lastAvailableSlot = Math.min(job.deadline - 1, latestDeadline - 1);
                for (int timeSlotIndex = lastAvailableSlot; timeSlotIndex >= job.startTime - 1; timeSlotIndex--) {
                    if (scheduledJobIdsByTimeSlot[timeSlotIndex] == -1) {
                        scheduledJobIdsByTimeSlot[timeSlotIndex] = job.jobId;
