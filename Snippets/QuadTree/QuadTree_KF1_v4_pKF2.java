 ...
 *
 * <p>Each node covers a square {@link Boundary} and stores up to {@code capacity} points.
 * When the capacity is exceeded, the node is subdivided into four child quadtrees
 * (northwest, northeast, southwest, southeast), each responsible for one quadrant
...
public class QuadTree {
    ...
    /** Points stored directly in this node (not in children). */
     ...
     *
     * @param boundary spatial region covered by this node
     ...
     *
     * <p>If the point lies outside this node's boundary, it is not inserted.
     * If the node has not reached capacity, the point is stored here.
     * Otherwise, the node is subdivided (if not already), and the point is
     * delegated to one of the child nodes.
     *
     ...
     *
     * <p>The children each cover a square region with half the side length of
     * this node's boundary, centered appropriately around this node's center.
     */
     ...
     *
     * <p>The search is pruned using boundary intersection checks: if this node's
     * boundary does not intersect the query range, the subtree is skipped.
     *
