...
public final class UniquePaths {
    ...
    public static int uniquePaths(final int rows, final int cols) {
        ...
        final int minDimension = Math.min(rows, cols);
        final int maxDimension = Math.max(rows, cols);

        final int[] dp = new int[maxDimension];
        Arrays.fill(dp, 1);
    ...
    public static int uniquePaths2(final int rows, final int cols) {
        ...
        initializeBorders(dp, rows, cols);
        fillDynamicProgrammingTable(dp, rows, cols);

        return dp[rows - 1][cols - 1];
    }

    private static void initializeBorders(final int[][] dp, final int rows, final int cols) {
    ...
    private static void fillDynamicProgrammingTable(final int[][] dp, final int rows, final int cols) {
