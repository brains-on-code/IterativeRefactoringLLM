...
public class StronglyConnectedComponentsCounter {
        ...
        if (neighbors != null) {
            for (int neighbor : neighbors) {
                if (visitState[neighbor] == UNVISITED) {
                    fillOrder(graph, visitState, finishOrder, neighbor);
                }
            }
        }
    ...
    private void depthFirstSearch(
        ...
        if (neighbors != null) {
            for (int neighbor : neighbors) {
                if (visitState[neighbor] == UNVISITED) {
                    depthFirstSearch(graph, visitState, neighbor, component);
                }
            }
        }
    }
    ...
    public int countStronglyConnectedComponents(HashMap<Integer, List<Integer>> graph, int vertexCount) {
        ...
        for (int vertex = 0; vertex < vertexCount; vertex++) {
            if (visitState[vertex] == UNVISITED) {
                fillOrder(graph, visitState, finishOrder, vertex);
            }
        }
        ...
        while (!finishOrder.isEmpty()) {
            ...
            if (visitState[vertex] == UNVISITED) {
                ...
                depthFirstSearch(transposedGraph, visitState, vertex, component);
