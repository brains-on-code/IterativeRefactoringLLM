...
public final class JohnsonsAlgorithm {
    ...
    private static double[] bellmanFord(double[][] edges, int numVertices) {
        ...
        relaxEdges(numVertices, dist, allEdges);
        detectNegativeCycle(dist, allEdges);

        return Arrays.copyOf(dist, numVertices);
    }

    private static void relaxEdges(int numVertices, double[] dist, double[][] allEdges) {
    ...
    private static void detectNegativeCycle(double[] dist, double[][] allEdges) {
        for (double[] edge : allEdges) {
            int from = (int) edge[0];
            int to = (int) edge[1];
            double weight = edge[2];
    ...
    public static double[] dijkstra(double[][] reweightedGraph, int source, double[] potentials) {
        ...
        for (int i = 0; i < numVertices - 1; i++) {
            ...
            for (int v = 0; v < numVertices; v++) {
                ...
                if (isRelaxationCandidate(visited, dist, u, v, weight)) {
                    dist[v] = dist[u] + weight;
                }
            }
        }

        adjustDistancesWithPotentials(dist, source, potentials);

        return dist;
    }

    private static boolean isRelaxationCandidate(
            boolean[] visited, double[] dist, int u, int v, double weight) {
        return !visited[v]
                && weight != 0
    ...
    private static void adjustDistancesWithPotentials(
            double[] dist, int source, double[] potentials) {
        for (int vertex = 0; vertex < dist.length; vertex++) {
            if (dist[vertex] != INF) {
                dist[vertex] = dist[vertex] - potentials[source] + potentials[vertex];
            }
        }
    }
