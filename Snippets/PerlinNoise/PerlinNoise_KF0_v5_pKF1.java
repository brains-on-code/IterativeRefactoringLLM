...
public final class PerlinNoise {
    ...
    static float[][] generatePerlinNoiseLayer(
            float[][] baseNoise, int width, int height, int octaveIndex) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            int sampleXStart = (x / period) * period;
            int sampleXEnd = (sampleXStart + period) % width;
            float horizontalBlend = (x - sampleXStart) * frequency;

            for (int y = 0; y < height; y++) {
                // calculates the vertical sampling indices
                int sampleYStart = (y / period) * period;
                int sampleYEnd = (sampleYStart + period) % height;
                float verticalBlend = (y - sampleYStart) * frequency;

                // blend top corners
                float topInterpolation =
                        interpolate(
                                baseNoise[sampleXStart][sampleYStart],
                                baseNoise[sampleXEnd][sampleYStart],
                                horizontalBlend);

                // blend bottom corners
                float bottomInterpolation =
                        interpolate(
                                baseNoise[sampleXStart][sampleYEnd],
                                baseNoise[sampleXEnd][sampleYEnd],
                                horizontalBlend);
