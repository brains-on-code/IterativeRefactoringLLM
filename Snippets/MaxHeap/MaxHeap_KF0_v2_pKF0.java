...
public class MaxHeap implements Heap {
    ...
    public HeapElement getElement(int elementIndex) {
        ...
        return maxHeap.get(internalIndex);
    }
    ...
    private double getElementKey(int elementIndex) {
        int internalIndex = toInternalIndex(elementIndex);
        return maxHeap.get(internalIndex).getKey();
    }
    ...
    private void swap(int index1, int index2) {
        int i = toInternalIndex(index1);
        int j = toInternalIndex(index2);
        swapInternal(i, j);
    }
    ...
    public void deleteElement(int elementIndex) throws EmptyHeapException {
        ...
        HeapElement lastElement = maxHeap.get(maxHeap.size() - 1);
        maxHeap.set(internalIndex, lastElement);
        maxHeap.remove(maxHeap.size() - 1);

        if (internalIndex < maxHeap.size()) {
            int parent = parentIndex(internalIndex);
            if (internalIndex > 0 && maxHeap.get(internalIndex).getKey() > maxHeap.get(parent).getKey()) {
                toggleUp(internalIndex + 1);
            } else {
