...
public final class MatrixRank {
    ...
    public static int computeRank(double[][] matrix) {
        ...
        int rank = 0;
        boolean[] isPivotRowUsed = new boolean[numRows];
     ...
     * @param isPivotRowUsed flags indicating which rows are already used as pivot rows
     * @param col           the column index
     * @return the pivot row index, or the number of rows if no suitable pivot row was found
     */
    private static int findPivotRow(double[][] matrix, boolean[] isPivotRowUsed, int col) {
        int numRows = matrix.length;

        for (int row = 0; row < numRows; row++) {
            if (!isPivotRowUsed[row] && !isZero(matrix[row][col])) {
                return row;
            }
        }
