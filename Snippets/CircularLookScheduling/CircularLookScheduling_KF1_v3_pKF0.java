...
public class Class1 {
    ...
    public List<Integer> schedule(List<Integer> requests) {
        List<Integer> validHigherRequests = new ArrayList<>();
        List<Integer> validLowerRequests = new ArrayList<>();
        ...
        for (int request : requests) {
            ...
            if (request > currentPosition) {
                validHigherRequests.add(request);
            } else if (request < currentPosition) {
                validLowerRequests.add(request);
            }
        }

        Collections.sort(validHigherRequests);
        Collections.sort(validLowerRequests);

        List<Integer> scheduledOrder = scanDirectionUp
            ? scheduleAscending(validHigherRequests, validLowerRequests)
            : scheduleDescending(validHigherRequests, validLowerRequests);
    ...
    private List<Integer> scheduleAscending(List<Integer> higherRequests,
        ...
        List<Integer> scheduledOrder = new ArrayList<>(higherRequests.size() + lowerRequests.size());
