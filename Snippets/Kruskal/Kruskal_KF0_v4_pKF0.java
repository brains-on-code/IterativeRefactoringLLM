...
public class Kruskal {
    ...
    public HashSet<Edge>[] kruskal(HashSet<Edge>[] graph) {
        ...
        while (connectedNodeCount != nodeCount && !edgeQueue.isEmpty()) {
            ...
            // Ensure fromComponent is the larger component (union by size)
            ...
            mergeComponents(componentLeader, components, fromLeader, toLeader, fromComponent, toComponent);
    ...
    private void mergeComponents(
        ...
        int newLeader,
        int oldLeader,
        HashSet<Integer> newComponent,
        HashSet<Integer> oldComponent
    ) {
        newComponent.addAll(oldComponent);
        for (int node : oldComponent) {
            componentLeader[node] = newLeader;
        }
        components[oldLeader] = null;
    }
}