...
public final class PrimeChecker {
    ...
    public static void main(String[] args) {
        ...
        int fermatIterationCount = 20;
        if (isPrimeProbabilistic(candidateNumber, fermatIterationCount)) {
    ...
    public static boolean isPrimeDeterministic(int candidateNumber) {
        ...
        int upperDivisorLimit = (int) Math.sqrt(candidateNumber);
        for (int divisor = 3; divisor <= upperDivisorLimit; divisor += 2) {
    ...
    public static boolean isPrimeProbabilistic(int candidateNumber, int iterationCount) {
        int minimumBase = 2;
        int maximumBase = candidateNumber - 2;

        for (int iterationIndex = 0; iterationIndex < iterationCount; iterationIndex++) {
            long randomBase =
                    (long) Math.floor(Math.random() * (maximumBase - minimumBase + 1) + minimumBase);
    ...
    private static long modularExponentiation(long base, long exponent, long modulus) {
        long modularPowerResult = 1;
        for (long exponentIndex = 0; exponentIndex < exponent; exponentIndex++) {
            modularPowerResult *= base;
            modularPowerResult %= modulus;
        }
        return modularPowerResult % modulus;
    }
}