...
public final class MatrixRank {
     ...
     *
     * @param matrix The input matrix
     * @param pivotRowUsed An array indicating which rows have been used as pivot rows
     * @param columnIndex The column index
     * @return The pivot row index, or the number of rows if no suitable pivot row was found
     */
    private static int findPivotRow(double[][] matrix, boolean[] pivotRowUsed, int columnIndex) {
        int rowCount = matrix.length;
        for (int rowIndex = 0; rowIndex < rowCount; ++rowIndex) {
            if (!pivotRowUsed[rowIndex] && !isEffectivelyZero(matrix[rowIndex][columnIndex])) {
                return rowIndex;
            }
        }
        return rowCount;
