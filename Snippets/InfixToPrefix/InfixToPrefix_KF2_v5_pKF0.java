...
public final class InfixToPrefix {
    ...
    public static String infix2Prefix(String infixExpression) {
        ...
        for (char ch : reversedInfix.toCharArray()) {
            if (Character.isLetterOrDigit(ch)) {
                ...
                char matchingClosing = getMatchingClosingParenthesis(ch);
                popUntilMatchingParenthesis(operators, output, matchingClosing);
            } else {
    ...
    private static void popUntilMatchingParenthesis(
            Stack<Character> operators, StringBuilder output, char matchingClosing) {
        while (!operators.isEmpty() && operators.peek() != matchingClosing) {
            output.append(operators.pop());
        }
    ...
    private static char getMatchingClosingParenthesis(char opening) {
        switch (opening) {
            case '(':
                return ')';
            case '{':
            ...
            case '<':
                return '>';
            default:
                throw new IllegalArgumentException("Invalid opening parenthesis: " + opening);
        }
    }
}