...
public final class Class1 {
    ...
    public static int method1(int[][] matrix) {
        ...
        while (candidates.size() > 1) {
            ...
            if (knows(matrix, personA, personB)) {
                // personA knows personB → personA cannot be a celebrity
                candidates.push(personB);
            } else {
                // personA does not know personB → personB cannot be a celebrity
    ...
    private static boolean isCelebrity(int[][] matrix, int candidate) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            boolean candidateKnowsI = matrix[candidate][i] == 1;
            boolean iDoesNotKnowCandidate = matrix[i][candidate] == 0;

            if (candidateKnowsI || iDoesNotKnowCandidate) {
                return false;
            }
        }
        return true;
