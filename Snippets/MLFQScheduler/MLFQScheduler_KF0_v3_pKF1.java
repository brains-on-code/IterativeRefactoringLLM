...
public class MLFQScheduler {
    ...
    private int elapsedTime; // Current time in the system
    ...
    public void run() {
        while (!areAllQueuesEmpty()) {
            for (int queueLevelIndex = 0; queueLevelIndex < feedbackQueues.size(); queueLevelIndex++) {
                Queue<ScheduledProcess> currentQueue = feedbackQueues.get(queueLevelIndex);
                if (currentQueue.isEmpty()) {
                    continue;
                }

                ScheduledProcess processToRun = currentQueue.poll();
                int timeQuantumForLevel = timeQuantaByLevel[queueLevelIndex];
                ...
                elapsedTime += timeSlice; // Update the system's current time

                if (processToRun.isFinished()) {
                    System.out.println(
                        "Process " + processToRun.processId + " finished at time " + elapsedTime
                    );
                } else {
                    if (queueLevelIndex < feedbackQueues.size() - 1) {
                        processToRun.priority++; // Demote the process to the next lower priority queue
                        feedbackQueues.get(queueLevelIndex + 1).add(processToRun); // Add to the next queue level
                    } else {
                        currentQueue.add(processToRun); // Stay in the same queue if it's the last level
                    }
                }
            }
        }
