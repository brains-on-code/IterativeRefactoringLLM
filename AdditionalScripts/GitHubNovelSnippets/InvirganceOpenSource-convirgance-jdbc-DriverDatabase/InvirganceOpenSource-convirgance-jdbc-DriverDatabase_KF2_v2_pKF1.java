...
class DriverDatabase implements Iterable<JSONObject> {
    ...
    private URLClassLoader createClassLoader(JSONArray<String> mavenArtifactCoordinates) {
        ConfigurableMavenResolverSystem mavenResolver = Maven.configureResolver();
        URL[] artifactUrls = mavenResolver
                .withMavenCentralRepo(true)
                .resolve(mavenArtifactCoordinates)
                .withTransitivity()
                .as(URL.class);
    ...
    private Object instantiateClass(JSONArray<String> mavenArtifactCoordinates, String fullyQualifiedClassName) {
        URLClassLoader classLoader = createClassLoader(mavenArtifactCoordinates);
        Class<?> targetClass;
