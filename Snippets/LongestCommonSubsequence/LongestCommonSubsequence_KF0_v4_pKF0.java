...
final class LongestCommonSubsequence {
    ...
    public static String getLCS(String first, String second) {
        ...
        int[][] lcsLengths = buildLcsTable(first, second);
        return buildLcsString(first, second, lcsLengths);
    }

    /**
     * Builds the DP table containing LCS lengths for all prefixes of the input strings.
     *
     * @param first  The first string.
     * @param second The second string.
     * @return A 2D array where lcsLengths[i][j] is the length of the LCS of
     *         first[0..i-1] and second[0..j-1].
     */
    private static int[][] buildLcsTable(String first, String second) {
