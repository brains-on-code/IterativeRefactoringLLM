...
public final class Verhoeff {
    ...
    public static boolean verhoeffCheck(String digits) {
        ...
        int checksum = 0;
        for (int position = 0; position < digitArray.length; position++) {
            int reversedIndex = digitArray.length - position - 1;
            byte permutedDigit = PERMUTATION_TABLE[position % 8][digitArray[reversedIndex]];
