...
public final class Mandelbrot {
            ...
            for (int pixelY = 0; pixelY < imageHeight; pixelY++) {
                ...
                Color pixelColor =
                        useDistanceColorCoding
                                ? mapNormalizedIterationToColor(normalizedEscapeIteration)
     ...
     */
    private static Color mapNormalizedIterationToBlackAndWhite(double normalizedEscapeIteration) {
     ...
     * @return The color corresponding to the distance.
     */
    private static Color mapNormalizedIterationToColor(double normalizedEscapeIteration) {
        ...
        int maxChannelValue = (int) value;
        int minChannelValue = 0;
        int descendingChannelValue = (int) (value * (1 - fractionalSector * saturation));
        int ascendingChannelValue = (int) (value * (1 - (1 - fractionalSector) * saturation));

        switch (hueSectorIndex) {
            case 0:
                return new Color(maxChannelValue, ascendingChannelValue, minChannelValue);
            case 1:
                return new Color(descendingChannelValue, maxChannelValue, minChannelValue);
            case 2:
                return new Color(minChannelValue, maxChannelValue, ascendingChannelValue);
            case 3:
                return new Color(minChannelValue, descendingChannelValue, maxChannelValue);
            case 4:
                return new Color(ascendingChannelValue, minChannelValue, maxChannelValue);
            default:
                return new Color(maxChannelValue, minChannelValue, descendingChannelValue);
        }
    }
    ...
    private static double computeNormalizedEscapeIteration(
        ...
        double currentRealComponent = figureX;
        double currentImaginaryComponent = figureY;
        ...
        for (int iterationIndex = 0; iterationIndex < maxIterations; iterationIndex++) {
            ...
            double nextRealComponent =
                    currentRealComponent * currentRealComponent
                            - currentImaginaryComponent * currentImaginaryComponent
                            + figureX;
            currentImaginaryComponent =
                    2 * currentRealComponent * currentImaginaryComponent + figureY;
            currentRealComponent = nextRealComponent;

            if (currentRealComponent * currentRealComponent
                            + currentImaginaryComponent * currentImaginaryComponent
                    > 4) {
                break;
            }
