    ...
    public void run() {
        while (!areAllQueuesEmpty()) {
            for (int level = 0; level < priorityQueues.size(); level++) {
                Queue<ScheduledProcess> currentQueue = priorityQueues.get(level);
                if (currentQueue.isEmpty()) {
                    continue;
                }

                ScheduledProcess process = currentQueue.poll();
                int timeQuantum = timeQuantaPerLevel[level];
                ...
                if (process.isFinished()) {
                    ...
                    if (level < priorityQueues.size() - 1) {
                        process.priority++; // Demote the process to the next lower priority queue
                        priorityQueues.get(level + 1).add(process); // Add to the next queue level
                    } else {
                        currentQueue.add(process); // Stay in the same queue if it's the last level
                    }
                }
            }
        }
