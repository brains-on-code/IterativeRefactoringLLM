...
public final class NQueensSolver {
    ...
    private static boolean isSafePosition(int[] queenPositions, int currentColumn, int currentRow) {
        for (int row = 0; row < currentRow; row++) {
            ...
            if (areOnSameDiagonal(existingColumn, currentColumn, currentRow, row)) {
                return false;
            }
        }
        return true;
    ...
    /**
     * Checks if two queens are on the same diagonal.
     *
     * @param existingColumn column of the already placed queen
     * @param currentColumn column of the queen to place
     * @param currentRow row of the queen to place
     * @param existingRow row of the already placed queen
     * @return true if they are on the same diagonal, false otherwise
     */
    private static boolean areOnSameDiagonal(
        int existingColumn,
        int currentColumn,
        int currentRow,
        int existingRow
        ...
        int rowDifference = Math.abs(currentRow - existingRow);
