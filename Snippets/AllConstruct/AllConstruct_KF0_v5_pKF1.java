...
public final class AllConstruct {
    ...
    public static List<List<String>> allConstruct(String target, Iterable<String> wordBank) {
        ...
        for (int currentIndex = 0; currentIndex <= targetLength; currentIndex++) {
            List<List<String>> constructionsAtCurrentIndex = constructionsByIndex.get(currentIndex);
            if (constructionsAtCurrentIndex.isEmpty()) {
                continue;
            }

            for (String word : wordBank) {
                int wordLength = word.length();
                int nextIndex = currentIndex + wordLength;

                if (nextIndex <= targetLength && target.startsWith(word, currentIndex)) {
                    List<List<String>> extendedConstructions = new ArrayList<>();

                    for (List<String> construction : constructionsAtCurrentIndex) {
