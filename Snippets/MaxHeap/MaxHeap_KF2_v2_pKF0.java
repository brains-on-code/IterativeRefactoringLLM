...
public class MaxHeap implements Heap {
    ...
    private int parentIndex(int zeroBasedIndex) {
        return (zeroBasedIndex - 1) / 2;
    }

    private int leftChildIndex(int zeroBasedIndex) {
        return 2 * zeroBasedIndex + 1;
    }

    private int rightChildIndex(int zeroBasedIndex) {
        return 2 * zeroBasedIndex + 2;
    }

    private void heapifyDown(int zeroBasedIndex) {
        int largest = zeroBasedIndex;
        int left = leftChildIndex(zeroBasedIndex);
        int right = rightChildIndex(zeroBasedIndex);
        ...
        if (largest != zeroBasedIndex) {
            swapZeroBased(zeroBasedIndex, largest);
            heapifyDown(largest);
        }
    }
    ...
    private void toggleDown(int oneBasedIndex) {
        ...
        while (true) {
            ...
            if (hasRight && getElementKey(rightChild) > getElementKey(leftChild)) {
