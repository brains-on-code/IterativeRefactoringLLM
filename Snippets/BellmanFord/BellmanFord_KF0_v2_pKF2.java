...
class BellmanFord {
    ...
    /**
     * Represents a directed, weighted edge (source -> destination) with a given weight.
     */
    static class Edge {
        int source;
        int destination;
    ...
    /**
     * Recursively prints the path from the source to vertex {@code vertex}
     * using the parent array.
     *
     * @param parent parent[v] is the predecessor of vertex v in the shortest path tree
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            for (int i = 0; i < vertices - 1; i++) {
                for (int j = 0; j < edgesCount; j++) {
                    int source = inputEdges[j].source;
                    int destination = inputEdges[j].destination;
                    int weight = inputEdges[j].weight;
    ...
    public void show(int source, int target, Edge[] edgeArray) {
        ...
        for (int i = 0; i < vertices - 1; i++) {
            for (int j = 0; j < edgesCount; j++) {
                int u = edgeArray[j].source;
                int v = edgeArray[j].destination;
                int w = edgeArray[j].weight;

                if ((int) distances[u] != Integer.MAX_VALUE && distances[v] > distances[u] + w) {
