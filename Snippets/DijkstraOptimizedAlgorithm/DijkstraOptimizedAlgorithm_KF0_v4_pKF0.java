...
public class DijkstraOptimizedAlgorithm {
    ...
    public int[] run(int[][] graph, int source) {
        ...
        while (!priorityQueue.isEmpty()) {
            int currentVertex = pollMinDistanceVertex(priorityQueue);
    ...
    private int pollMinDistanceVertex(Set<Pair<Integer, Integer>> priorityQueue) {
    ...
    private void relaxNeighbors(
        ...
        for (int neighbor = 0; neighbor < vertexCount; neighbor++) {
            if (canRelaxEdge(graph, currentVertex, neighbor, distances, visited)) {
