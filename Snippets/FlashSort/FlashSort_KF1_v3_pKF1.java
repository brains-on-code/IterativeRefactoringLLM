...
public class BucketSort implements SortAlgorithm {
    ...
    private <T extends Comparable<? super T>> void bucketSort(T[] array) {
        ...
        countBucketSizes(array, bucketBoundaries, scalingFactor, minValue);
        accumulateBucketBoundaries(bucketBoundaries);
        redistributeElementsToBuckets(array, bucketBoundaries, scalingFactor, minValue);
        insertionSort(array);
    }
    ...
    private <T extends Comparable<? super T>> int findMaxValueIndex(final T[] array) {
        int maxValueIndex = 0;
        for (int index = 1; index < array.length; index++) {
            if (array[index].compareTo(array[maxValueIndex]) > 0) {
                maxValueIndex = index;
            }
        }
    ...
    private <T extends Comparable<? super T>> void redistributeElementsToBuckets(
        ...
        while (placedElementCount < arrayLength - 1) {
            ...
            while (currentIndex != bucketBoundaries[currentBucketIndex]) {
                ...
                T temp = array[bucketBoundaries[currentBucketIndex] - 1];
