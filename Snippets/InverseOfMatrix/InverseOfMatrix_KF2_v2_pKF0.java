...
public final class InverseOfMatrix {
    ...
    public static double[][] invert(double[][] matrix) {
        ...
        double[][] luDecomposed = copyMatrix(matrix);
        double[][] inverse = new double[size][size];
        double[][] identity = createIdentityMatrix(size);
        int[] pivotIndices = new int[size];

        performGaussianElimination(luDecomposed, pivotIndices);
        applyRowOperations(luDecomposed, identity, pivotIndices);
        backSubstitution(luDecomposed, identity, inverse, pivotIndices);
    ...
    private static void applyRowOperations(double[][] lu, double[][] identity, int[] pivotIndices) {
        int size = lu.length;
    ...
    private static void backSubstitution(
        ...
        for (int col = 0; col < size; col++) {
            ...
            inverse[size - 1][col] = identity[lastPivotRowIndex][col] / lu[lastPivotRowIndex][size - 1];
