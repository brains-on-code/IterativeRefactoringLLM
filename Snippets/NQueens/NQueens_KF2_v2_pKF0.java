...
public final class NQueens {
    ...
    private static List<String> buildBoardRepresentation(int boardSize, int[] queenPositionsByColumn) {
        ...
        for (int row = 0; row < boardSize; row++) {
            ...
            int queenColumn = findQueenColumnInRow(queenPositionsByColumn, row);
    ...
    private static int findQueenColumnInRow(int[] queenPositionsByColumn, int row) {
        for (int col = 0; col < queenPositionsByColumn.length; col++) {
            if (queenPositionsByColumn[col] == row) {
                return col;
            }
        }
        return -1;
    ...
    private static boolean isSafe(int[] queenPositionsByColumn, int row, int column) {
        for (int previousColumn = 0; previousColumn < column; previousColumn++) {
            ...
            boolean sameRow = rowDifference == 0;
