...
public class HttpServer {
  ...
  public void start() {
    ...
    configureShutdownHook(httpServer);

    try {
      httpServer.start();
      log.info("Jetty-based HTTP server started on {}", formatLocalhostUrl());
      httpServer.join();
    } catch (InterruptedException e) {
      Thread.currentThread().interrupt();
      log.error("HTTP server thread was interrupted", e);
    } catch (Exception e) {
      log.error("Error starting or running HTTP server on {}", formatLocalhostUrl(), e);
    }
  }
  ...
  private void configureShutdownHook(Server httpServer) {
    Runtime.getRuntime().addShutdownHook(new Thread(() -> stopServer(httpServer), "http-server-shutdown-hook"));
  }
  ...
  private String formatLocalhostUrl() {
    return LOCALHOST_URL_TEMPLATE.replace("{}", String.valueOf(port));
  }
}