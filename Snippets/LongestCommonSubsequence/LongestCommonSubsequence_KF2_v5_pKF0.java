...
final class LongestCommonSubsequence {
    ...
    public static String getLCS(String first, String second) {
        ...
        int[][] lcsLengths = buildLcsLengthTable(first, second);
        return reconstructLcs(first, second, lcsLengths);
    }

    private static int[][] buildLcsLengthTable(String first, String second) {
        ...
        for (int row = 1; row <= firstLength; row++) {
            ...
            for (int col = 1; col <= secondLength; col++) {
                ...
                if (firstChar == secondChar) {
                    ...
                    lcsLengths[row][col] = Math.max(
                        lcsLengths[row - 1][col],
                        lcsLengths[row][col - 1]
                    );
                }
            }
        }
    ...
    private static String reconstructLcs(String first, String second, int[][] lcsLengths) {
        ...
        while (row > 0 && col > 0) {
            ...
            boolean moveUp = lcsLengths[row - 1][col] >= lcsLengths[row][col - 1];
            if (moveUp) {
                row--;
            } else {
                col--;
