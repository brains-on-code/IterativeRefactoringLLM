...
public final class InfixToPrefix {
    ...
    public static String infix2Prefix(String infixExpression) {
        ...
        for (char ch : reversedInfix.toCharArray()) {
            ...
            if (isClosingParenthesis(ch)) {
                ...
                popUntilMatchingParenthesis(operators, output);
            } else {
    ...
    private static void popUntilMatchingParenthesis(Stack<Character> operators, StringBuilder output) {
        while (!operators.isEmpty() && !isClosingParenthesis(operators.peek())) {
            output.append(operators.pop());
        }
