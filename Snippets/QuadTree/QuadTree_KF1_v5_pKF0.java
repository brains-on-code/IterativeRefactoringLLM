...
public class QuadTree {
    ...
    private void subdivide() {
        final double newHalfSize = boundary.halfSize / 2.0;
    ...
    private QuadTree createChild(final double centerX, final double centerY, final double halfSize) {
        final Point childCenter = new Point(centerX, centerY);
        final BoundingBox childBoundary = new BoundingBox(childCenter, halfSize);
    ...
    public List<Point> query(final BoundingBox range) {
        ...
        if (range == null || !boundary.intersects(range)) {
            return foundPoints;
        }
