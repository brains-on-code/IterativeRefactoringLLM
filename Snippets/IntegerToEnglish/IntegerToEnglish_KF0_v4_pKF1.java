...
public final class IntegerToEnglish {
     ...
     *
     * @param numberUnderThousand the integer value (0-999) to convert
     * @return the English word representation of the input number
     */
    private static String convertNumberUnderThousandToWords(int numberUnderThousand) {
        int lastTwoDigits = numberUnderThousand % 100;
        StringBuilder wordsBuilder = new StringBuilder();

        if (lastTwoDigits <= 20) {
            wordsBuilder.append(BASE_NUMBERS_MAP.get(lastTwoDigits));
        } else if (BASE_NUMBERS_MAP.containsKey(lastTwoDigits)) {
            wordsBuilder.append(BASE_NUMBERS_MAP.get(lastTwoDigits));
        } else {
            int tensDigit = lastTwoDigits / 10;
            int onesDigit = lastTwoDigits % 10;

            String tensWord = BASE_NUMBERS_MAP.getOrDefault(tensDigit * 10, "");
            String onesWord = BASE_NUMBERS_MAP.getOrDefault(onesDigit, "");
        ...
        int hundredsDigit = numberUnderThousand / 100;
        if (hundredsDigit > 0) {
            if (wordsBuilder.length() > 0) {
                wordsBuilder.insert(0, " ");
            }
            wordsBuilder.insert(0, String.format("%s Hundred", BASE_NUMBERS_MAP.get(hundredsDigit)));
        }
    ...
    public static String integerToEnglishWords(int number) {
        ...
        StringBuilder englishWordsBuilder = new StringBuilder();
        ...
        while (number > 0) {
            ...
            if (currentThreeDigitGroup > 0) {
                String currentGroupWords = convertNumberUnderThousandToWords(currentThreeDigitGroup);
                if (!currentGroupWords.isEmpty()) {
                    if (thousandGroupIndex > 0) {
                        currentGroupWords += " " + THOUSAND_POWER_MAP.get(thousandGroupIndex);
                    }
                    if (englishWordsBuilder.length() > 0) {
                        englishWordsBuilder.insert(0, " ");
                    }
                    englishWordsBuilder.insert(0, currentGroupWords);
                }
            }
        ...
        return englishWordsBuilder.toString().trim();
    }
}