...
public class IntrospectiveSort implements SortAlgorithm {
    ...
    private static <T extends Comparable<T>> void heapify(
        ...
        int largestArrayIndex = low + largestIndexInHeap;
        int leftArrayIndex = low + leftChildIndex;
        int rightArrayIndex = low + rightChildIndex;
        ...
        if (largestIndexInHeap != index) {
            SortUtils.swap(array, low + index, largestArrayIndex);
            heapify(array, largestIndexInHeap, heapSize, low);
        }
    }
}