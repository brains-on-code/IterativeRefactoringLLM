...
class VectorClock {
    ...
    VectorClock(int processId, int processCount) {
        ...
        validateProcessId(processId, processCount);

        this.processId = processId;
        this.processCount = processCount;
        this.clock = initializeClock(processCount);
    }
     ...
     */
    public void tick() {
        clock.computeIfPresent(processId, (id, value) -> value + 1);
    }
    ...
    public int getTotalEvents() {
        return clock.values().stream().mapToInt(Integer::intValue).sum();
    }
    ...
    public void merge(VectorClock other) {
        validateOtherClock(other);
        for (int i = 0; i < processCount; i++) {
            clock.compute(i, (id, value) -> Math.max(value, other.clock.get(i)));
        }
    }
    ...
    private static void validateProcessCount(int processCount) {
        if (processCount <= 0) {
            throw new IllegalArgumentException("processCount must be positive");
        }
    }

    private static void validateProcessId(int processId, int processCount) {
        if (processId < 0 || processId >= processCount) {
            throw new IllegalArgumentException("processId must be in range [0, processCount)");
        }
    }

    private static Map<Integer, Integer> initializeClock(int processCount) {
        Map<Integer, Integer> initialClock = new HashMap<>(processCount);
        for (int i = 0; i < processCount; i++) {
            initialClock.put(i, 0);
        }
        return initialClock;
    }
