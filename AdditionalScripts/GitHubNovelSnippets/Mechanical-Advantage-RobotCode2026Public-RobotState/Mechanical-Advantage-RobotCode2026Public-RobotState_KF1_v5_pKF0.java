...
public class Class1 {
  ...
  public void addVisionMeasurement(VisionUpdate update) {
    ...
    Optional<Pose2d> odometrySample = poseBuffer.getSample(update.timestampSeconds());
    if (odometrySample.isEmpty()) {
      return;
    }

    Pose2d odometryAtVisionTime = odometrySample.get();
  ...
  private boolean isVisionTimestampInRange(double visionTimestampSeconds) {
    try {
      ...
      double oldestValidTimestamp = newestTimestamp - POSE_BUFFER_DURATION_SECONDS;
