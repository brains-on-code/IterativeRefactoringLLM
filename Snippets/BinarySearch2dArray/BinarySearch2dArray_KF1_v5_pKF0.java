...
public final class MatrixSearch {
    ...
    private static final int[] NOT_FOUND_COORDINATES = {NOT_FOUND, NOT_FOUND};
    ...
    static int[] search(int[][] matrix, int target) {
        ...
        int[] midColumnResult = searchMidColumn(matrix, target, midCol, startRow, endRow);
        if (midColumnResult != null) {
            return midColumnResult;
        }

        // After narrowing, startRow and endRow are adjacent candidate rows
        // Search in 4 possible quadrants

        // 1st quadrant: startRow, columns [0 .. midCol - 1]
        if (midCol > 0 && target <= matrix[startRow][midCol - 1]) {
        ...
        if (midCol + 1 < colCount
                && target >= matrix[startRow][midCol + 1]
                && target <= matrix[startRow][colCount - 1]) {
            return binarySearchRow(matrix, target, startRow, midCol + 1, colCount - 1);
        }

        // 3rd quadrant: endRow, columns [0 .. midCol - 1]
        if (midCol > 0 && target <= matrix[endRow][midCol - 1]) {
    ...
    private static boolean isMatrixEmpty(int[][] matrix) {
        return matrix == null || matrix.length == 0 || matrix[0].length == 0;
    }

    private static int[] notFound() {
        return NOT_FOUND_COORDINATES.clone();
    }

    /**
     * Narrows the search to two candidate rows using the middle column and
     * checks the middle column for the target.
     *
     * @return coordinates if found in the middle column, otherwise null
     */
    private static int[] searchMidColumn(int[][] matrix, int target, int midCol, int startRow, int endRow) {
