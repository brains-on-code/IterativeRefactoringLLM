...
class MergeSort implements SortAlgorithm {
    ...
    private <T extends Comparable<T>> void merge(T[] array, int leftIndex, int middleIndex, int rightIndex) {
        int leftSubarrayPointer = leftIndex;
        int rightSubarrayPointer = middleIndex + 1;
        ...
        for (int currentIndex = leftIndex; currentIndex <= rightIndex; currentIndex++) {
            if (rightSubarrayPointer > rightIndex) {
                array[currentIndex] = (T) auxiliaryArray[leftSubarrayPointer++];
            } else if (leftSubarrayPointer > middleIndex) {
                array[currentIndex] = (T) auxiliaryArray[rightSubarrayPointer++];
            } else if (less(auxiliaryArray[rightSubarrayPointer], auxiliaryArray[leftSubarrayPointer])) {
                array[currentIndex] = (T) auxiliaryArray[rightSubarrayPointer++];
            } else {
                array[currentIndex] = (T) auxiliaryArray[leftSubarrayPointer++];
            }
