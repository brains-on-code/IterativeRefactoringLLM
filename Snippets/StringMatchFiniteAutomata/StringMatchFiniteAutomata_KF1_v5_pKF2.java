...
public final class FiniteAutomatonMatcher {
    ...
    public static Set<Integer> findAllOccurrences(final String text, final String pattern) {
        ...
        final Set<Integer> matchPositions = new TreeSet<>();
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (automaton.getCurrentState() == patternLength) {
                matchPositions.add(i - patternLength + 1);
            }
        }
    ...
    private static int nextState(
        ...
        // If the next character continues the current match, advance one state.
         ...
         * the current matched sequence followed by the new character.
         */
        for (int next = currentState; next > 0; next--) {
            ...
            if (isPrefixMatching(pattern, currentState, next)) {
                return next;
            }
        }

        // No prefix matches; reset to the initial state.
        return 0;
    }

    /**
     * Checks whether pattern[0..next-2] matches the suffix of the current
     * matched sequence that would result in state {@code next}.
     *
     * @param pattern      the pattern
     * @param currentState the current state in the automaton
     * @param next         the candidate next state
     * @return {@code true} if the prefix condition holds, {@code false} otherwise
     */
    private static boolean isPrefixMatching(
        final String pattern,
        final int currentState,
        final int next
    ) {
