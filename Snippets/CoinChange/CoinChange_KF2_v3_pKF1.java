        ...
        for (int currentAmount = 1; currentAmount <= targetAmount; currentAmount++) {
            for (int denomination : denominations) {
                if (denomination <= currentAmount) {
                    ...
                    int coinsForRemainingAmount = minCoinsForAmount[remainingAmount];

                    if (coinsForRemainingAmount != Integer.MAX_VALUE
                            && coinsForRemainingAmount + 1 < minCoinsForAmount[currentAmount]) {
                        minCoinsForAmount[currentAmount] = coinsForRemainingAmount + 1;
                    }
                }
            }
        }
