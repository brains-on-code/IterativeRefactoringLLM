...
public class SimpsonIntegration {
    ...
    public static void main(String[] args) {
        ...
        double integralValue =
                simpsonIntegration.computeIntegralWithSimpson(subIntervalCount, stepSize, lowerBound);
        System.out.println("The integral is equal to: " + integralValue);
    }

    /*
     * @param subIntervalCount: Number of intervals (must be even number N=2*k)
     ...
     */
    public double computeIntegralWithSimpson(int subIntervalCount, double stepSize, double lowerBound) {
        ...
        for (int index = 0; index < functionValuesByIndex.size(); index++) {
            ...
            if (index == 0 || index == lastIndex) {
                weightedSum += functionValueAtIndex;
                System.out.println("Multiply f(x" + index + ") by 1");
            } else if (index % 2 != 0) {
                weightedSum += 4.0 * functionValueAtIndex;
                System.out.println("Multiply f(x" + index + ") by 4");
            } else {
                weightedSum += 2.0 * functionValueAtIndex;
