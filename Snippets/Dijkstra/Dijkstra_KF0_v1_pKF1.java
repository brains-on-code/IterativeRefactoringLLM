...
class Graph {
    ...
    Graph(Edge[] edges) {
        ...
        for (Edge edge : edges) {
            ...
            // verticesByName.get(edge.targetName).neighbours.put(verticesByName.get(edge.sourceName), edge.distance); // also do this for an
            // undirected graph
        }
    }
    ...
    private void dijkstra(final NavigableSet<Vertex> unvisitedVertices) {
        while (!unvisitedVertices.isEmpty()) {
            ...
            for (Map.Entry<Vertex, Integer> neighbourEntry : currentVertex.neighbours.entrySet()) {
                Vertex neighbourVertex = neighbourEntry.getKey(); // the neighbour in this iteration

                final int alternateDistance = currentVertex.distance + neighbourEntry.getValue();
                if (alternateDistance < neighbourVertex.distance) { // shorter path to neighbour found
                    unvisitedVertices.remove(neighbourVertex);
                    neighbourVertex.distance = alternateDistance;
                    neighbourVertex.previous = currentVertex;
                    unvisitedVertices.add(neighbourVertex);
                }
            }
        }
    }
