...
public final class Class1 {
    ...
    private static <T extends Comparable<T>> int selectIndex(List<T> elements, int index) {
        return selectIndexInRange(elements, 0, elements.size() - 1, index);
    }

    private static <T extends Comparable<T>> int selectIndexInRange(
            List<T> elements,
            int left,
            int right,
            int targetIndex
        ...
        while (true) {
            ...
            pivotIndex = partitionAroundPivot(elements, low, high, pivotIndex, targetIndex);
    ...
    private static <T extends Comparable<T>> int choosePivotIndex(
        ...
        for (int groupStart = left; groupStart <= right; groupStart += 5) {
            ...
            Collections.swap(elements, medianIndex, medianWriteIndex);
            medianWriteIndex++;
        }

        int mediansRight = medianWriteIndex - 1;
        int medianOfMediansIndex = (mediansRight - left) / 2 + left;

        return selectIndexInRange(elements, left, mediansRight, medianOfMediansIndex);
    }
