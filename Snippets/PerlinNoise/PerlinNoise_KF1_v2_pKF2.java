...
public final class FractalNoiseGenerator {
    ...
    static float[][] generateSmoothNoise(float[][] baseNoise, int width, int height, int octave) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            for (int y = 0; y < height; y++) {
                ...
                float top = lerp(baseNoise[sampleX0][sampleY0], baseNoise[sampleX1][sampleY0], horizontalBlend);
                float bottom = lerp(baseNoise[sampleX0][sampleY1], baseNoise[sampleX1][sampleY1], horizontalBlend);
    ...
    public static void main(String[] args) {
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            int octaveCount = scanner.nextInt();

            System.out.println("Persistence (float): ");
            float persistence = scanner.nextFloat();
