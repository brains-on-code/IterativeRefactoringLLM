...
public final class PalindromicPartitioning {
    ...
    public static int minimalPartitions(String text) {
         ...
         */
        int[] minimumCuts = new int[textLength];
        ...
        for (int endIndex = 0; endIndex < textLength; endIndex++) {
            if (isPalindrome[0][endIndex]) {
                minimumCuts[endIndex] = 0;
            } else {
                minimumCuts[endIndex] = Integer.MAX_VALUE;
                for (int partitionIndex = 0; partitionIndex < endIndex; partitionIndex++) {
                    if (isPalindrome[partitionIndex + 1][endIndex]
                        && 1 + minimumCuts[partitionIndex] < minimumCuts[endIndex]) {
                        minimumCuts[endIndex] = 1 + minimumCuts[partitionIndex];
                    }
                }
            }
        }
        ...
        return minimumCuts[textLength - 1];
    }
}