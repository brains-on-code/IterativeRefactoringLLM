...
public class PrintAMatrixInSpiralOrder {
    ...
    public List<Integer> print(int[][] matrix, int totalRows, int totalCols) {
        ...
        while (topRow <= bottomRow && leftCol <= rightCol) {
            addTopRow(matrix, topRow, leftCol, rightCol, spiralOrder);
            topRow++;

            addRightColumn(matrix, rightCol, topRow, bottomRow, spiralOrder);
            rightCol--;

            if (topRow <= bottomRow) {
                addBottomRow(matrix, bottomRow, leftCol, rightCol, spiralOrder);
                bottomRow--;
            }

            if (leftCol <= rightCol) {
                addLeftColumn(matrix, leftCol, topRow, bottomRow, spiralOrder);
                leftCol++;
            }
        }
