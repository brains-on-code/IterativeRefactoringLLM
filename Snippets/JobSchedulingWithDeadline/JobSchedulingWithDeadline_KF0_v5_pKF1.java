...
public final class JobSchedulingWithDeadline {
    ...
    public static int[] jobSequencingWithDeadlines(Job[] jobs) {
        ...
        int[] scheduledJobIdsByTimeSlot = new int[maxDeadline];
        ...
        for (Job job : jobs) {
            if (job.arrivalTime <= job.deadline) {
                int latestPossibleSlotIndex = Math.min(job.deadline - 1, maxDeadline - 1);
                int earliestPossibleSlotIndex = job.arrivalTime - 1;

                for (int slotIndex = latestPossibleSlotIndex; slotIndex >= earliestPossibleSlotIndex; slotIndex--) {
                    if (scheduledJobIdsByTimeSlot[slotIndex] == -1) {
                        scheduledJobIdsByTimeSlot[slotIndex] = job.id;
                        scheduledJobCount++;
