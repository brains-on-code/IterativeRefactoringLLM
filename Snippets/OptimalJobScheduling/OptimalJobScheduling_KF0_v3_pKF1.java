...
public class OptimalJobScheduling {
    ...
    private final int[][] executionTimePerProcessMachine;
    private final int[][] transferDelayBetweenMachines;
    private final int[][] cumulativeCostPerProcessMachine;
     ...
     * @param executionTimePerProcessMachine N*M matrix refers to the cost of running each process on each machine
    ...
    public OptimalJobScheduling(
        int processCount,
        int machineCount,
        int[][] executionTimePerProcessMachine,
        int[][] transferDelayBetweenMachines
    ) {
        this.processCount = processCount;
        this.machineCount = machineCount;
        this.executionTimePerProcessMachine = executionTimePerProcessMachine;
        this.transferDelayBetweenMachines = transferDelayBetweenMachines;
        this.cumulativeCostPerProcessMachine = new int[processCount][machineCount];
    }
    ...
    private void calculateAllCumulativeCosts() {
        for (int processIndex = 0; processIndex < processCount; processIndex++) {
            for (int machineIndex = 0; machineIndex < machineCount; machineIndex++) {
                cumulativeCostPerProcessMachine[processIndex][machineIndex] =
    ...
    private int calculateCumulativeCostForProcessOnMachine(int processIndex, int machineIndex) {
        if (processIndex == 0) {
            ...
            return executionTimePerProcessMachine[processIndex][machineIndex];
        }

        int[] cumulativeCostsForPreviousMachine = new int[machineCount];
        ...
        for (int previousMachineIndex = 0; previousMachineIndex < machineCount; previousMachineIndex++) {
            cumulativeCostsForPreviousMachine[previousMachineIndex] =
                cumulativeCostPerProcessMachine[processIndex - 1][previousMachineIndex]
                    + transferDelayBetweenMachines[previousMachineIndex][machineIndex]
                    + executionTimePerProcessMachine[processIndex][machineIndex];
        }

        return findMinimumCost(cumulativeCostsForPreviousMachine);
    }
    ...
    private void printAllCumulativeCosts() {
        for (int processIndex = 0; processIndex < processCount; processIndex++) {
            for (int machineIndex = 0; machineIndex < machineCount; machineIndex++) {
                System.out.print(cumulativeCostPerProcessMachine[processIndex][machineIndex]);
                System.out.print(" ");
            }
            System.out.println();
        }
    ...
    public int getCost(int processIndex, int machineIndex) {
        return cumulativeCostPerProcessMachine[processIndex][machineIndex];
    }
}