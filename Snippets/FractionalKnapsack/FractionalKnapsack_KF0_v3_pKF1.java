...
public final class FractionalKnapsack {
    ...
    public static int fractionalKnapsack(int[] weights, int[] values, int capacity) {
        ...
        for (int sortedIndex = itemIndexAndValueDensity.length - 1; sortedIndex >= 0; sortedIndex--) {
            int originalIndex = (int) itemIndexAndValueDensity[sortedIndex][0];
            double valuePerWeight = itemIndexAndValueDensity[sortedIndex][1];

            int itemWeight = weights[originalIndex];
            int itemValue = values[originalIndex];
