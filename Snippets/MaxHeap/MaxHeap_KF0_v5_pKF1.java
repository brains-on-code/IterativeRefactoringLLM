...
public class MaxHeap implements Heap {
    ...
    public MaxHeap(List<HeapElement> initialElements) {
        ...
        for (int arrayIndex = elements.size() / 2; arrayIndex >= 0; arrayIndex--) {
    ...
    private void heapifyDown(int heapIndex) {
        ...
        if (leftChildIndex < elements.size()
                && elements.get(leftChildIndex).getKey()
                    > elements.get(largestIndex).getKey()) {
            largestIndex = leftChildIndex;
        }

        if (rightChildIndex < elements.size()
                && elements.get(rightChildIndex).getKey()
                    > elements.get(largestIndex).getKey()) {
