...
public final class Class1 {
    ...
    public static int countWaysMemoized(int current, int target, int[] memo) {
        ...
        memo[current] = countWaysFromPosition(current, target, memo);
        return memo[current];
    }

    private static int countWaysFromPosition(int current, int target, int[] memo) {
        int ways = 0;
        for (int step = 1; step <= MAX_STEP; step++) {
    ...
    public static int countWaysBottomUp(int current, int target, int[] dp) {
        ...
        for (int position = target - 1; position >= 0; position--) {
            dp[position] = countWaysFromPosition(position, target, dp);
        }

        return dp[current];
    }

    private static int countWaysFromPosition(int position, int target, int[] dp) {
        int ways = 0;
