...
public class SkylineProblem {
    ...
    public List<Skyline> mergeSkyline(List<Skyline> leftSkyline, List<Skyline> rightSkyline) {
        List<Skyline> mergedSkyline = new ArrayList<>();
        ...
        while (leftIndex < leftSkyline.size() && rightIndex < rightSkyline.size()) {
            ...
            if (maxHeightAtX != currentMaxHeight) {
                mergedSkyline.add(new Skyline(xCoordinate, maxHeightAtX));
    ...
    private int appendRemainingPoints(
        List<Skyline> targetSkyline,
        List<Skyline> sourceSkyline,
        int startIndex,
        int currentMaxHeight
    ) {
        int lastHeight = currentMaxHeight;
        for (int i = startIndex; i < sourceSkyline.size(); i++) {
            Skyline point = sourceSkyline.get(i);
            if (point.height != lastHeight) {
                targetSkyline.add(point);
