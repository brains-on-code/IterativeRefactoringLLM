...
public class GrahamScan {
    ...
    public GrahamScan(Point[] points) {
        ...
        // Step 1: sort points to find pivot (points[0]) and then by polar angle around pivot
        Arrays.sort(points);
        Arrays.sort(points, 1, points.length, points[0].polarOrder());

        hull.push(points[0]);

        // Step 2: find first point that is not identical to the pivot
        ...
        // Step 3: find first point that is not collinear with pivot and firstDistinctIndex
        int firstNonCollinearIndex = findFirstNonCollinearIndex(points, firstDistinctIndex);

        // Push the last collinear point with the pivot (furthest in sorted order)
        hull.push(points[firstNonCollinearIndex - 1]);

        // Step 4: process remaining points to build the convex hull
