...
public class PoseEstimator {
  ...
  private static final Matrix<N3, N1> VISION_STANDARD_DEVIATIONS =
  ...
  private PoseEstimator() {
    for (int row = 0; row < 3; ++row) {
      visionProcessVariance.set(
          row, 0, Math.pow(VISION_STANDARD_DEVIATIONS.get(row, 0), 2));
    }
  ...
  public void addOdometryMeasurement(OdometryMeasurement odometryMeasurement) {
    ...
    odometryMeasurement.gyroRotation()
        .ifPresent(
            gyroRotation -> {
    ...
    poseBuffer.addSample(odometryMeasurement.timestamp(), estimatedPose);
  ...
  public void addVisionMeasurement(VisionMeasurement visionMeasurement) {
    ...
    var odometrySampleAtVisionTime = poseBuffer.getSample(visionMeasurement.timestamp());
    ...
    for (int row = 0; row < 3; ++row) {
      double stdDev = visionMeasurement.standardDeviations().get(row, 0);
    ...
    for (int index = 0; index < 3; ++index) {
      ...
      if (processVariance == 0.0) {
        kalmanGain.set(index, index, 0.0);
      } else {
        kalmanGain.set(
            index,
            index,
            processVariance
                / (processVariance
                    + Math.sqrt(processVariance * measurementVariance[index])));
      }
    }
