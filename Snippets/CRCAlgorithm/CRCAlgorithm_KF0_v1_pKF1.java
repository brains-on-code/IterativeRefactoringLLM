...
public class CRCAlgorithm {
    ...
    private int wrongMessageCount;
    private int wrongMessageCaughtCount;
    private int wrongMessageNotCaughtCount;
     ...
     *                            CRC algorithm
     * @param messageSize         The size of every transmitted message
     * @param bitErrorRate        The Bit Error Rate
     */
    public CRCAlgorithm(String generatorPolynomial, int messageSize, double bitErrorRate) {
        ...
        this.wrongMessageCaughtCount = 0;
        this.wrongMessageNotCaughtCount = 0;
        this.bitErrorRate = bitErrorRate;
    }

    /**
     * @return wrongMessageCount, the number of Wrong Messages
     */
    public int getWrongMessageCount() {
        return wrongMessageCount;
    }

    /**
     * @return wrongMessageCaughtCount, the number of wrong messages, which are caught
     * by the CRC algorithm
     */
    public int getWrongMessageCaughtCount() {
        return wrongMessageCaughtCount;
    }

    /**
     * @return wrongMessageNotCaughtCount, the number of wrong messages, which are not
     * caught by the CRC algorithm
     */
    public int getWrongMessageNotCaughtCount() {
        return wrongMessageNotCaughtCount;
    }
     ...
     * variable wrongMessageCaughtCount changes. If it does not, it is accepted, so one
     * of the variables correctMessageCount, wrongMessageNotCaughtCount, changes. If check ==
    ...
    public void divideMessageWithGeneratorPolynomial(boolean check) {
        ...
        if (!check) {
            ...
            if (remainderHasOne && messageChanged) {
                wrongMessageCaughtCount++;
            } else if (!remainderHasOne && messageChanged) {
                wrongMessageNotCaughtCount++;
