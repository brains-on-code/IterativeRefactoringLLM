...
public final class AmicableNumber {
     ...
     * less than {@code divisor}. When initially called with
     * {@code divisor == number}, this effectively sums all proper divisors.
     *
    ...
    private static void validateRange(int from, int to) {
        if (from <= 0 || to <= 0 || to < from) {
            throw new IllegalArgumentException("Invalid range: both bounds must be positive and 'from' must be ≤ 'to'.");
        }
    }
