...
public class HillCipher {
    ...
    private String transform(String message, int[][] matrix) {
        ...
        int[] inputVector = new int[blockSize];
        int[] outputVector = new int[blockSize];

        for (int index = 0; index < sanitizedMessage.length(); index += blockSize) {
    ...
    private void validateKeyMatrix(int[][] keyMatrix) {
        ...
        int determinantMod = Math.floorMod(determinant(keyMatrix, size), MODULUS);
        if (determinantMod == 0) {
