...
public class Drive extends FullSubsystem {
  ...
  public void periodic() {
    ...
    if (DriverStation.isDisabled()) {
      if (hasStartedCoast || lastMovementTimer.hasElapsed(coastWaitTimeSeconds.get())) {
        for (Module module : modules) {
          module.coast();
        }
  ...
  public void runVelocity(ChassisSpeeds chassisSpeeds) {
    ...
    SwerveModuleState[] desiredModuleStates =
        kinematics.toSwerveModuleStates(discreteChassisSpeeds);
    SwerveDriveKinematics.desaturateWheelSpeeds(
        desiredModuleStates, DriveConstants.maxLinearSpeed);

    Logger.recordOutput("SwerveStates/Setpoints", desiredModuleStates);
    Logger.recordOutput("SwerveChassisSpeeds/Setpoints", discreteChassisSpeeds);
  ...
  private SwerveModuleState[] getModuleStates() {
    ...
    for (int moduleIndex = 0; moduleIndex < modules.length; moduleIndex++) {
      measuredStates[moduleIndex] = modules[moduleIndex].getState();
    }
  ...
  private SwerveModulePosition[] getModulePositions() {
    SwerveModulePosition[] measuredPositions = new SwerveModulePosition[modules.length];
    for (int moduleIndex = 0; moduleIndex < modules.length; moduleIndex++) {
      measuredPositions[moduleIndex] = modules[moduleIndex].getPosition();
    }
