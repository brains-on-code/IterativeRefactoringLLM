...
public class WiggleSort implements SortAlgorithm {
    ...
    private <T extends Comparable<T>> void threeWayPartitionAroundMedian(T[] array, T median) {
        ...
        while (current <= right) {
            int virtualCurrentIndex = getVirtualIndex(current, length);
            int comparisonResult = array[virtualCurrentIndex].compareTo(median);

            if (comparisonResult > 0) {
                SortUtils.swap(array, virtualCurrentIndex, getVirtualIndex(left, length));
                left++;
                current++;
            } else if (comparisonResult < 0) {
                SortUtils.swap(array, virtualCurrentIndex, getVirtualIndex(right, length));
                right--;
            } else {
                current++;
            }
