...
public final class PerlinNoise {
    ...
    static float[][] generatePerlinNoise(int width, int height, int octaveCount, float persistence, long seed) {
        ...
        for (int xCoordinate = 0; xCoordinate < width; xCoordinate++) {
            for (int yCoordinate = 0; yCoordinate < height; yCoordinate++) {
                baseNoise[xCoordinate][yCoordinate] = randomGenerator.nextFloat();
            }
        }
            ...
            for (int xCoordinate = 0; xCoordinate < width; xCoordinate++) {
                for (int yCoordinate = 0; yCoordinate < height; yCoordinate++) {
                    perlinNoise[xCoordinate][yCoordinate] +=
                        octaveNoiseLayers[octaveIndex][xCoordinate][yCoordinate] * currentAmplitude;
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        octaveCount = inputScanner.nextInt();

        System.out.println("Persistence (float): ");
        persistence = inputScanner.nextFloat();

        System.out.println("Seed (long): ");
        seed = inputScanner.nextLong();

        System.out.println("Charset (String): ");
        charset = inputScanner.next();
