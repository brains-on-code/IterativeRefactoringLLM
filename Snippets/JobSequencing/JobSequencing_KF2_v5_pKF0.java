...
public final class JobSequencing {
    ...
    public static String findJobSequence(List<Job> jobs, int maxSlots) {
        if (isInvalidInput(jobs, maxSlots)) {
            return "Job Sequence: ";
        }
        ...
        scheduleJobs(jobs, maxSlots, occupiedSlots, jobIndexAtSlot);

        return buildJobSequenceString(jobs, occupiedSlots, jobIndexAtSlot);
    }

    private static boolean isInvalidInput(List<Job> jobs, int maxSlots) {
        return jobs == null || jobs.isEmpty() || maxSlots <= 0;
    }
