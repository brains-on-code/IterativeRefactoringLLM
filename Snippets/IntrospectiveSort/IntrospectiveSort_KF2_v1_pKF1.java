...
public class IntrospectiveSort implements SortAlgorithm {
    ...
    private static <T extends Comparable<T>> void introspectiveSort(
            T[] array, final int startIndex, int endIndex, final int remainingDepth) {

        int high = endIndex;
        int depth = remainingDepth;

        while (high - startIndex > INSERTION_SORT_THRESHOLD) {
            if (depth == 0) {
                heapSort(array, startIndex, high);
                return;
            }
            final int pivotIndex = partition(array, startIndex, high);
            introspectiveSort(array, pivotIndex + 1, high, depth - 1);
            high = pivotIndex - 1;
            depth--;
        }
        insertionSort(array, startIndex, high);
    }
    ...
    private static <T extends Comparable<T>> void heapSort(
        ...
        for (int currentEnd = endIndex; currentEnd > startIndex; currentEnd--) {
            SortUtils.swap(array, startIndex, currentEnd);
            heapify(array, 0, currentEnd - startIndex, startIndex);
        }
    }
