...
public class CRCAlgorithm {
    ...
    /**
     * @return corruptedMessageCount, the number of erroneous messages
     */
    ...
    /**
     * @return detectedCorruptedMessageCount, the number of erroneous messages, which are caught
    ...
    /**
     * @return undetectedCorruptedMessageCount, the number of erroneous messages, which are not
    ...
    /**
     * @return validMessageCount, the number of the correct messages
     */
     ...
     * variable detectedCorruptedMessageCount changes. If it does not, it is accepted, so one
     * of the variables validMessageCount, undetectedCorruptedMessageCount, changes. If check ==
     * false, the remainderBits is added at the end of the messageBits list.
     *
     * @param verify the variable used to determine, if the message is going to
     *               be checked from the receiver if true, it is checked otherwise, it is not
     */
    public void divideMessageWithGeneratorPolynomial(boolean verify) {
        ...
        if (!verify) {
            ...
            if (remainderHasOne && messageModified) {
                detectedCorruptedMessageCount++;
            } else if (!remainderHasOne && messageModified) {
     ...
     * changes, the boolean variable messageModified gets the value: true, or
     * false.
     */
