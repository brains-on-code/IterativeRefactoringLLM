...
public final class MergeSortNoExtraSpace {
    ...
    private static void merge(int[] array, int start, int middle, int end, int maxElement) {
        ...
        // Merge while both subarrays have remaining elements
        while (leftIndex <= middle && rightIndex <= end) {
            int leftValue = array[leftIndex] % maxElement;
            int rightValue = array[rightIndex] % maxElement;

            if (leftValue <= rightValue) {
                encodeNewValue(array, mergeIndex, leftValue, maxElement);
                leftIndex++;
            } else {
                encodeNewValue(array, mergeIndex, rightValue, maxElement);
                rightIndex++;
            }
            mergeIndex++;
        }
    ...
    /**
     * Encodes the new value into the array element at the given index.
     *
     * <p>The element already contains the original value; this method adds
     * the new value in the higher "digits" using maxElement as the base.</p>
     *
     * @param array      the array being modified
     * @param index      the index to encode into
     * @param newValue   the new value to encode
     * @param maxElement the encoding base (greater than any array element)
     */
    private static void encodeNewValue(int[] array, int index, int newValue, int maxElement) {
        array[index] += newValue * maxElement;
    }
}