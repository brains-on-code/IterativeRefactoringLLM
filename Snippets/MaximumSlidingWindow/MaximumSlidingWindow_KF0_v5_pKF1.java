...
public class MaximumSlidingWindow {
    ...
    public int[] maxSlidingWindow(int[] nums, int windowSize) {
        ...
        int[] windowMaxValues = new int[nums.length - windowSize + 1];
        Deque<Integer> maxCandidateIndices = new ArrayDeque<>();

        for (int currentIndex = 0; currentIndex < nums.length; currentIndex++) {

            int currentWindowStart = currentIndex - windowSize + 1;

            // Remove indices that are outside the current window
            if (!maxCandidateIndices.isEmpty() && maxCandidateIndices.peekFirst() < currentWindowStart) {
            ...
            while (!maxCandidateIndices.isEmpty()
                    && nums[maxCandidateIndices.peekLast()] < nums[currentIndex]) {
                maxCandidateIndices.pollLast();
            }

            // Add the current index to the deque
            maxCandidateIndices.offerLast(currentIndex);

            // Record the maximum for the current window
            if (currentWindowStart >= 0) {
                windowMaxValues[currentWindowStart] = nums[maxCandidateIndices.peekFirst()];
            }
        }
