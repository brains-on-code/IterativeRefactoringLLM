...
public final class InverseOfMatrix {
    ...
    private static void computeScalingFactors(double[][] matrix, double[] scalingFactors) {
        ...
        for (int row = 0; row < size; row++) {
            double maxAbsValueInRow = 0.0;

            for (int col = 0; col < size; col++) {
                double value = Math.abs(matrix[row][col]);
                if (value > maxAbsValueInRow) {
                    maxAbsValueInRow = value;
                }
            }
    ...
    private static int selectPivotRow(
        ...
        for (int row = col; row < size; row++) {
            int currentRowIndex = pivotIndex[row];
            double scaledValue = scalingFactors[currentRowIndex];

            if (scaledValue == 0.0) {
                continue;
            }

            double ratio = Math.abs(matrix[currentRowIndex][col]) / scaledValue;
            if (ratio > maxRatio) {
                maxRatio = ratio;
                pivotRow = row;
            }
    ...
    private static void eliminateBelowPivot(double[][] matrix, int[] pivotIndex, int pivotCol) {
        ...
        for (int row = pivotCol + 1; row < size; row++) {
            ...
            double pivotValue = matrix[pivotRowIndex][pivotCol];
            double factor = matrix[currentRowIndex][pivotCol] / pivotValue;
