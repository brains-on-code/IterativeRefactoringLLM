...
public final class KCenters {
    ...
    public static int findKCenters(int[][] distances, int k) {
        ...
        // Choose the first center (index 0) and initialize distances to it.
        isCenter[0] = true;
        for (int i = 1; i < n; i++) {
            minDistanceToCenter[i] = distances[0][i];
        }

        // Greedily choose the remaining k - 1 centers.
    ...
    /**
     * Returns the index of the non-center point that is farthest from its nearest center.
     */
    ...
    /**
     * Returns the maximum value in the array of minimum distances to centers.
     */
