...
public final class NonPreemptivePriorityScheduling {
    ...
    public static Process[] scheduleProcesses(Process[] processes) {
        ...
        Process[] sortedByArrival = Arrays.copyOf(processes, processes.length);
        Arrays.sort(sortedByArrival, (p1, p2) -> Integer.compare(p1.arrivalTime, p2.arrivalTime));

        Queue<Process> arrivalQueue = new LinkedList<>(Arrays.asList(sortedByArrival));
    ...
    public static double calculateAverageWaitingTime(Process[] processes, Process[] executionOrder) {
        if (isInvalidInput(processes, executionOrder)) {
            return 0.0;
        }
    ...
    private static boolean isInvalidInput(Process[] processes, Process[] executionOrder) {
