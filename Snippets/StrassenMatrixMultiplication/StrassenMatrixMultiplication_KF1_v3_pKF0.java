...
public class Class1 {
    ...
    public int[][] multiply(int[][] matrixA, int[][] matrixB) {
        ...
        split(matrixA, a12, 0, half);
        split(matrixA, a21, half, 0);
        split(matrixA, a22, half, half);

        split(matrixB, b11, 0, 0);
        split(matrixB, b12, 0, half);
        split(matrixB, b21, half, 0);
        split(matrixB, b22, half, half);
    ...
    public void split(int[][] source, int[][] target, int rowOffset, int colOffset) {
        ...
        for (int row = 0; row < size; row++) {
            int sourceRow = row + rowOffset;
            for (int col = 0; col < size; col++) {
                int sourceCol = col + colOffset;
                target[row][col] = source[sourceRow][sourceCol];
            }
        }
    }
    ...
    public void join(int[][] source, int[][] target, int rowOffset, int colOffset) {
        ...
        for (int row = 0; row < size; row++) {
            ...
            for (int col = 0; col < size; col++) {
                ...
                target[targetRow][targetCol] = source[row][col];
            }
        }
    }
}