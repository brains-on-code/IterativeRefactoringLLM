...
public final class FastInverseSqrtChecker {
    ...
    private static float fastInverseSqrt(float value) {
        ...
        return newtonRaphsonStep(approximation, halfValue);
    }
    ...
    private static double fastInverseSqrt(double value) {
        ...
        for (int i = 0; i < DOUBLE_REFINEMENT_ITERATIONS; i++) {
            approximation = newtonRaphsonStep(approximation, halfValue);
        }

        return approximation;
    }

    private static float newtonRaphsonStep(float approximation, float halfValue) {
        return approximation * (FLOAT_ONE_POINT_FIVE - halfValue * approximation * approximation);
    }

    private static double newtonRaphsonStep(double approximation, double halfValue) {
        return approximation * (DOUBLE_ONE_POINT_FIVE - halfValue * approximation * approximation);
    }
}