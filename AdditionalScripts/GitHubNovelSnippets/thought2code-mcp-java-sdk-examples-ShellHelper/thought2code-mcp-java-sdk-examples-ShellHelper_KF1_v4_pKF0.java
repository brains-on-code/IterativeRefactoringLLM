...
public final class Class1 {
  ...
  public static List<String> method1(String... command) {
    ...
    try (BufferedReader reader = createProcessOutputReader(process)) {
      ...
      handleInterruptedException(e, process, commandString);
      return Collections.emptyList();
    } catch (IOException e) {
      handleIOException(e, process, commandString);
      return Collections.emptyList();
    }
  }
  ...
  private static void handleInterruptedException(InterruptedException e, Process process, String commandString) {
    Thread.currentThread().interrupt();
    LOGGER.error("Interrupted while waiting for command: {}", commandString, e);
    process.destroyForcibly();
  }

  private static void handleIOException(IOException e, Process process, String commandString) {
