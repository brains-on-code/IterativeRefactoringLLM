...
public class ScanScheduling {
    ...
    public List<Integer> execute(List<Integer> requests) {
        ...
        // Split requests into those below and those at/above the current head position
        ...
        if (movingUp) {
            // Move towards higher tracks first, then go to the end and reverse
            serviceOrder.addAll(rightOfHead);
            serviceOrder.add(diskSize - 1); // last track before reversing
            Collections.reverse(leftOfHead);
            serviceOrder.addAll(leftOfHead);
        } else {
            // Move towards lower tracks first, then go to the start and reverse
            Collections.reverse(leftOfHead);
            serviceOrder.addAll(leftOfHead);
            serviceOrder.add(0); // first track before reversing
            serviceOrder.addAll(rightOfHead);
        }
