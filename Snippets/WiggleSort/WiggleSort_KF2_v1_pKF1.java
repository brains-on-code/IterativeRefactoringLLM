...
public class WiggleSort implements SortAlgorithm {
    ...
    private int remappedIndex(int index, int length) {
    ...
    private <T extends Comparable<T>> void threeWayPartitionAroundMedian(T[] array, T median) {
        ...
        while (current <= right) {
            ...
            int mappedLeft = remappedIndex(left, length);
            int mappedRight = remappedIndex(right, length);

            int comparison = array[mappedCurrent].compareTo(median);

            if (comparison > 0) {
                SortUtils.swap(array, mappedCurrent, mappedLeft);
                left++;
                current++;
            } else if (comparison < 0) {
                SortUtils.swap(array, mappedCurrent, mappedRight);
                right--;
            } else {
                current++;
            }
