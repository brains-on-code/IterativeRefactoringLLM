...
public class MonteCarloTreeSearch {
    ...
    public Node monteCarloTreeSearch(Node rootNode) {
        ...
        // Main MCTS loop: selection, expansion, simulation, backpropagation
    ...
    /**
     * Uses UCT (Upper Confidence bounds applied to Trees) to select a promising
     * node for further exploration.
     *
    ...
    public Node getPromisingNode(Node rootNode) {
        ...
        // Traverse down the tree while there are expanded children
     ...
     * result up the tree.
     *
     * Note: In a real game implementation, this method should:
    ...
    public void simulateRandomPlay(Node promisingNode) {
        ...
        // 1 in 6 chance that the player wins the simulated play.
        promisingNode.playerWon = (rand.nextInt(6) == 0);
        boolean isPlayerWinner = promisingNode.playerWon;

        // Backpropagation of the simulation result
        while (tempNode != null) {
            tempNode.visitCount++;

            // Reward nodes where the current player matches the winner
