 ...
 * for tracking counts in a distributed and concurrent environment.
 *
...
class GCounter {
    ...
    /**
     * Returns the global value of the counter, i.e., the sum of all replica components.
     *
     ...
     *
     * This holds iff for every replica id i:
     * this[i] <= other[i].
     *
    ...
    public boolean compare(GCounter other) {
        for (int id = 0; id < replicaCount; id++) {
            if (this.replicaCounts.get(id) > other.replicaCounts.get(id)) {
                return false;
            }
        }
        return true;
     ...
     *
     * After merge, for every replica id i:
     * this[i] = max(this[i], other[i]).
     *
    ...
    public void merge(GCounter other) {
        for (int id = 0; id < replicaCount; id++) {
            int mergedValue = Math.max(
                this.replicaCounts.get(id),
                other.replicaCounts.get(id)
            );
            this.replicaCounts.put(id, mergedValue);
        }
    }
}