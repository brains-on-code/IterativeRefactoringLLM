...
public final class DecimalToAnyBase {
    ...
    private static final char DIGIT_ZERO_CHAR = '0';
    private static final char LETTER_A_CHAR = 'A';
    ...
    public static String convertToAnyBase(int decimalNumber, int targetBase) {
        ...
        if (decimalNumber == 0) {
            return String.valueOf(DIGIT_ZERO_CHAR);
        }

        List<Character> digitsInTargetBase = new ArrayList<>();
        int remainingValue = decimalNumber;

        while (remainingValue > 0) {
            int remainder = remainingValue % targetBase;
            digitsInTargetBase.add(convertDigitToChar(remainder));
            remainingValue /= targetBase;
        }

        StringBuilder convertedNumberBuilder = new StringBuilder(digitsInTargetBase.size());
        for (int index = digitsInTargetBase.size() - 1; index >= 0; index--) {
            convertedNumberBuilder.append(digitsInTargetBase.get(index));
        }
    ...
    private static char convertDigitToChar(int digitValue) {
        if (digitValue >= 0 && digitValue <= 9) {
            return (char) (DIGIT_ZERO_CHAR + digitValue);
        } else {
            return (char) (LETTER_A_CHAR + digitValue - FIRST_LETTER_DIGIT_VALUE);
        }
    }
}