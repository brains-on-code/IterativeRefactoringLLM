...
public class Class1 {
  ...
  private Class1() {
    for (int i = 0; i < 3; ++i) {
      double stdDev = STATE_STD_DEVS.get(i, 0);
      stateCovariance.set(i, 0, stdDev * stdDev);
    }
    kinematics = new SwerveDriveKinematics(DriveConstants.moduleTranslations);
  }

  public void resetPose(Pose2d newPose) {
    Rotation2d currentRotation = estimatedPose.getRotation();
    Rotation2d newRotation = newPose.getRotation();
    gyroOffset = newRotation.minus(currentRotation.minus(gyroOffset));
  ...
  public void addVisionMeasurement(VisionUpdate update) {
    if (!isVisionTimestampInHistory(update.timestampSeconds())) {
      return;
    }

    Optional<Pose2d> historySample = poseHistory.getSample(update.timestampSeconds());
    ...
    double[] visionVariances = computeVisionVariances(update.visionStdDevs());
    Matrix<N3, N3> kalmanGain = computeKalmanGain(visionVariances);

    Transform2d visionError =
        new Transform2d(visionAtCurrentTime, update.visionPose().toPose2d());

    Matrix<N3, N1> correctionVector =
        kalmanGain.times(
            VecBuilder.fill(
                visionError.getX(),
                visionError.getY(),
  ...
  private boolean isVisionTimestampInHistory(double visionTimestampSeconds) {
    try {
      double oldestValidTimestamp =
          poseHistory.getInternalBuffer().lastKey() - HISTORY_DURATION_SECONDS;
  ...
  private double[] computeVisionVariances(Matrix<N3, N1> visionStdDevs) {
    double[] visionVariances = new double[3];
    for (int i = 0; i < 3; ++i) {
      double stdDev = visionStdDevs.get(i, 0);
      visionVariances[i] = stdDev * stdDev;
    }
    return visionVariances;
  }

  private Matrix<N3, N3> computeKalmanGain(double[] visionVariances) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      double measurementVariance = visionVariances[i];
      double denominator =
          stateVariance + Math.sqrt(stateVariance * measurementVariance);
      kalmanGain.set(i, i, stateVariance / denominator);
    }
    return kalmanGain;
  }
