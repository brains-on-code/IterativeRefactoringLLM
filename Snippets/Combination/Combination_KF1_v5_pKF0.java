...
public final class Class1 {
    ...
    public static <T> List<TreeSet<T>> method1(T[] elements, int combLength) {
        validateCombinationLength(combLength);

        if (isTrivialCase(elements, combLength)) {
    ...
    private static void validateCombinationLength(int combLength) {
        if (combLength < 0) {
            throw new IllegalArgumentException("The combination length cannot be negative.");
        }
    }

    private static <T> boolean isTrivialCase(T[] elements, int combLength) {
        return elements == null
                || elements.length == 0
                || combLength == 0
                || combLength > elements.length;
    }
