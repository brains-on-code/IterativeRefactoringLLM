...
public final class Mandelbrot {
    ...
    public static void main(String[] args) {
        ...
        assertPixelColorsForBlackAndWhiteImage(blackAndWhiteImage, width, height);

        BufferedImage coloredImage =
                getImage(width, height, centerX, centerY, figureWidth, maxStep, true);

        assertPixelColorsForColoredImage(coloredImage, width, height);

        writeImageToFile(coloredImage, "Mandelbrot.png");
    }

    private static void assertPixelColorsForBlackAndWhiteImage(
            BufferedImage image, int width, int height) {
        assert image.getRGB(0, 0) == COLOR_WHITE.getRGB();
        assert image.getRGB(width / 2, height / 2) == COLOR_BLACK.getRGB();
    }

    private static void assertPixelColorsForColoredImage(
            BufferedImage image, int width, int height) {
        assert image.getRGB(0, 0) == COLOR_RED.getRGB();
        assert image.getRGB(width / 2, height / 2) == COLOR_BLACK.getRGB();
    }
        ...
        for (int imageX = 0; imageX < imageWidth; imageX++) {
            for (int imageY = 0; imageY < imageHeight; imageY++) {
                ...
                Color color = useDistanceColorCoding
                        ...
                        : getBlackAndWhiteColorForDistance(distance);
    ...
    private static double toFigureX(
        ...
        double normalizedX = (double) imageX / imageWidth - 0.5;
        return figureCenterX + normalizedX * figureWidth;
    }

    private static double toFigureY(
            int imageY, int imageHeight, double figureCenterY, double figureHeight) {
        double normalizedY = (double) imageY / imageHeight - 0.5;
        return figureCenterY + normalizedY * figureHeight;
    }
     ...
     */
    private static Color getBlackAndWhiteColorForDistance(double distance) {
     ...
     * @return The color corresponding to the distance.
     */
    private static Color getColorForDistance(double distance) {
        if (distance >= 1) {
            return COLOR_BLACK;
        }
    ...
    private static boolean hasDiverged(double a, double b) {
        return a * a + b * b > 4;
    }
}