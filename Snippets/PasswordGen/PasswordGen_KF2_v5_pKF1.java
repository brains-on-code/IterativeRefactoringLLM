...
final class PasswordGenerator {
    ...
    public static String generatePassword(int minimumLength, int maximumLength) {
        ...
        int passwordLength = RANDOM_GENERATOR.nextInt(maximumLength - minimumLength + 1) + minimumLength;
        ...
        for (int characterIndex = 0; characterIndex < passwordLength; characterIndex++) {
            int randomCharacterIndex = RANDOM_GENERATOR.nextInt(ALL_ALLOWED_CHARACTERS.length());
            passwordBuilder.append(ALL_ALLOWED_CHARACTERS.charAt(randomCharacterIndex));
        }
