...
public final class NQueens {
     ...
     * @param columns     columns[c] = row index where the queen is placed in column c
    ...
    private static void solve(int boardSize, List<List<String>> solutions, int[] columns, int columnIndex) {
        if (columnIndex == boardSize) {
            solutions.add(buildBoard(columns, boardSize));
            return;
        }

        for (int rowIndex = 0; rowIndex < boardSize; rowIndex++) {
            columns[columnIndex] = rowIndex;
            if (canPlace(columns, rowIndex, columnIndex)) {
     ...
     */
    private static List<String> buildBoard(int[] columns, int boardSize) {
        List<String> board = new ArrayList<>();
        for (int col = 0; col < boardSize; col++) {
            StringBuilder row = new StringBuilder();
            for (int rowIndex = 0; rowIndex < boardSize; rowIndex++) {
                row.append(rowIndex == columns[col] ? 'Q' : '.');
            }
     ...
     */
    private static boolean canPlace(int[] columns, int rowIndex, int columnIndex) {
        for (int col = 0; col < columnIndex; col++) {
            int rowDiff = Math.abs(columns[col] - rowIndex);
