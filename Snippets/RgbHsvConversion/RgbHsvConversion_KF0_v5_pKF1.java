...
public final class RgbHsvConversion {
    ...
    public static int[] hsvToRgb(double hue, double saturation, double value) {
        ...
        return getRgbByHueSector(hueSector, chroma, matchComponent, secondaryComponent);
    }
    ...
    public static double[] rgbToHsv(int red, int green, int blue) {
        ...
        double redNormalized = (double) red / 255;
        double greenNormalized = (double) green / 255;
        double blueNormalized = (double) blue / 255;

        double value = Math.max(Math.max(redNormalized, greenNormalized), blueNormalized);
        double minComponent = Math.min(Math.min(redNormalized, greenNormalized), blueNormalized);
        ...
        if (chroma == 0) {
            hue = 0;
        } else if (value == redNormalized) {
            hue = 60 * (0 + (greenNormalized - blueNormalized) / chroma);
        } else if (value == greenNormalized) {
            hue = 60 * (2 + (blueNormalized - redNormalized) / chroma);
        } else {
            hue = 60 * (4 + (redNormalized - greenNormalized) / chroma);
        }
    ...
    private static void validateRgbComponent(int componentValue, String componentName) {
        if (componentValue < 0 || componentValue > 255) {
            throw new IllegalArgumentException(componentName + " should be between 0 and 255");
        }
    }
    ...
    private static int toRgbComponent(double normalizedComponentValue) {
        return (int) Math.round(255 * normalizedComponentValue);
    }
}