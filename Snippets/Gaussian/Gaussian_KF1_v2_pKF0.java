...
public final class GaussianElimination {
    ...
    public static ArrayList<Double> solve(int size, List<Double> coefficients) {
        ...
        double[][] eliminatedMatrix = forwardElimination(size, augmentedMatrix);
        return backSubstitution(size, workingMatrix, eliminatedMatrix);
    }
    ...
    public static ArrayList<Double> backSubstitution(int size, double[][] workingMatrix, double[][] matrix) {
        ...
        for (int row = 0; row < size; row++) {
            System.arraycopy(matrix[row], 0, workingMatrix[row], 0, size + 1);
        }
