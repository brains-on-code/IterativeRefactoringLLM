...
final class LongestCommonSubsequence {
    ...
    public static String computeLCS(String first, String second) {
        ...
        int[][] lcsLengths = buildLcsLengthTable(firstChars, secondChars);

        return reconstructLCS(firstChars, secondChars, lcsLengths);
    }

    private static int[][] buildLcsLengthTable(char[] firstChars, char[] secondChars) {
     ...
     *
     * @param firstChars  the first input string as a char array
     * @param secondChars the second input string as a char array
     * @param lcsLengths  the DP table containing LCS lengths
     * @return the reconstructed LCS
     */
    private static String reconstructLCS(char[] firstChars, char[] secondChars, int[][] lcsLengths) {
        StringBuilder lcsBuilder = new StringBuilder();
        int i = firstChars.length;
        int j = secondChars.length;

        while (i > 0 && j > 0) {
            char firstChar = firstChars[i - 1];
            char secondChar = secondChars[j - 1];
