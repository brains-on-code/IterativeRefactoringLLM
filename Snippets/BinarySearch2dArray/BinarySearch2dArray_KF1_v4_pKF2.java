...
/**
 * Utility class for searching in a 2D matrix where:
 * <ul>
 *     <li>Each row is sorted in non-decreasing order.</li>
 *     <li>Each column is sorted in non-decreasing order.</li>
 * </ul>
 *
 * The public API returns the position of the target as {@code [row, column]},
 * or {@code [-1, -1]} if the target is not found.
 */
public final class MatrixSearch {
     ...
     * @return an array of size 2: {@code [rowIndex, columnIndex]} if found, otherwise {@code [-1, -1]}
     */
    static int[] search(int[][] matrix, int target) {
        int rowCount = matrix.length;
        int colCount = matrix[0].length;

        // Single-row matrix: search that row directly.
        if (rowCount == 1) {
        ...
        // Use the middle column to narrow the search to two candidate rows.
        ...
        // Search in the four candidate subranges (quadrants) around the middle column.
