...
public final class MergeKSortedArrays {
    ...
    public static int[] mergeKArrays(int[][] arrays) {
        ...
        for (int sourceArrayIndex = 0; sourceArrayIndex < arrays.length; sourceArrayIndex++) {
            if (arrays[sourceArrayIndex].length > 0) {
                int firstElementValue = arrays[sourceArrayIndex][0];
                int firstElementIndex = 0;
                minHeap.offer(new int[] {firstElementValue, sourceArrayIndex, firstElementIndex});
                totalElementCount += arrays[sourceArrayIndex].length;
            }
        }
        ...
        while (!minHeap.isEmpty()) {
            ...
            int sourceElementIndex = heapEntry[2];

            mergedArray[mergedArrayWriteIndex++] = value;

            int nextSourceElementIndex = sourceElementIndex + 1;
            if (nextSourceElementIndex < arrays[sourceArrayIndex].length) {
                int nextValue = arrays[sourceArrayIndex][nextSourceElementIndex];
                minHeap.offer(new int[] {nextValue, sourceArrayIndex, nextSourceElementIndex});
            }
        }
