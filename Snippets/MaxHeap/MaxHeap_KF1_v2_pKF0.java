...
public class MaxHeap implements Heap {
    ...
    private void heapifyUp(int index) {
        while (index > 1 && getKey(parentIndex(index)) < getKey(index)) {
    ...
    private void heapifyDownFrom(int index) {
        while (true) {
            ...
            if (left <= size() && getKey(left) > getKey(largest)) {
                largest = left;
            }
            if (right <= size() && getKey(right) > getKey(largest)) {
                largest = right;
            }
