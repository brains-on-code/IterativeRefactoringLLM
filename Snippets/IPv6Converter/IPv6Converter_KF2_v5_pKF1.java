...
public final class IPv6Converter {
    ...
    public static String ipv4ToIpv6(String ipv4Address) throws UnknownHostException {
        ...
        for (int byteIndex = IPV6_MAPPED_IPV4_PREFIX_LENGTH; byteIndex < IPV6_BYTE_LENGTH; byteIndex++) {
    ...
    public static String ipv6ToIpv4(String ipv6Address) throws UnknownHostException {
        ...
        if (isIpv6MappedIpv4Address(ipv6AddressBytes)) {
            byte[] ipv4AddressBytes =
                Arrays.copyOfRange(ipv6AddressBytes, IPV6_MAPPED_IPV4_PREFIX_LENGTH, IPV6_BYTE_LENGTH);
    ...
    private static boolean isIpv6MappedIpv4Address(byte[] ipv6AddressBytes) {
        ...
        for (int byteIndex = 0; byteIndex < IPV6_MAPPED_IPV4_PREFIX_LENGTH - 2; byteIndex++) {
            if (ipv6AddressBytes[byteIndex] != 0) {
                return false;
            }
        }

        return ipv6AddressBytes[IPV6_MAPPED_FF_POSITION_1] == (byte) 0xff
            && ipv6AddressBytes[IPV6_MAPPED_FF_POSITION_2] == (byte) 0xff;
    }
}