...
public final class IPv4IPv6Mapper {
    ...
    private static final int IPV6_MAPPED_PREFIX_BYTE_LENGTH = 12;
    private static final int IPV6_MAPPED_FF_FIRST_INDEX = 10;
    private static final int IPV6_MAPPED_FF_SECOND_INDEX = 11;
    ...
    public static String toIPv6MappedIPv4(String ipv4Address) throws UnknownHostException {
        ...
        ipv6Bytes[IPV6_MAPPED_FF_FIRST_INDEX] = (byte) 0xff;
        ipv6Bytes[IPV6_MAPPED_FF_SECOND_INDEX] = (byte) 0xff;
        System.arraycopy(
            ipv4Bytes,
            0,
            ipv6Bytes,
            IPV6_MAPPED_PREFIX_BYTE_LENGTH,
            IPV4_BYTE_LENGTH
        );

        StringBuilder ipv6MappedAddressBuilder = new StringBuilder("::ffff:");
        for (int index = IPV6_MAPPED_PREFIX_BYTE_LENGTH; index < IPV6_BYTE_LENGTH; index++) {
            ipv6MappedAddressBuilder.append(ipv6Bytes[index] & 0xFF);
    ...
    public static String fromIPv6MappedIPv4(String ipv6MappedAddress) throws UnknownHostException {
        ...
        if (isIPv6MappedIPv4(addressBytes)) {
            byte[] ipv4Bytes =
                Arrays.copyOfRange(addressBytes, IPV6_MAPPED_PREFIX_BYTE_LENGTH, IPV6_BYTE_LENGTH);
    ...
    private static boolean isIPv6MappedIPv4(byte[] addressBytes) {
        ...
        for (int index = 0; index < IPV6_MAPPED_PREFIX_BYTE_LENGTH - 2; index++) {
            if (addressBytes[index] != 0) {
                return false;
            }
        }

        return addressBytes[IPV6_MAPPED_FF_FIRST_INDEX] == (byte) 0xff
            && addressBytes[IPV6_MAPPED_FF_SECOND_INDEX] == (byte) 0xff;
    }
}