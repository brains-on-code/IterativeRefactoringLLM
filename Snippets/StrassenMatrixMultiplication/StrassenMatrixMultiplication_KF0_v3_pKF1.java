...
public class StrassenMatrixMultiplication {
    ...
    public int[][] multiply(int[][] leftMatrix, int[][] rightMatrix) {
        ...
        if (matrixSize == 1) {
            ...
            int[][] p1 = multiply(addMatrices(leftTopLeft, leftBottomRight), addMatrices(rightTopLeft, rightBottomRight));
            int[][] p2 = multiply(addMatrices(leftBottomLeft, leftBottomRight), rightTopLeft);
            int[][] p3 = multiply(leftTopLeft, subtractMatrices(rightTopRight, rightBottomRight));
            int[][] p4 = multiply(leftBottomRight, subtractMatrices(rightBottomLeft, rightTopLeft));
            int[][] p5 = multiply(addMatrices(leftTopLeft, leftTopRight), rightBottomRight);
            int[][] p6 = multiply(subtractMatrices(leftBottomLeft, leftTopLeft), addMatrices(rightTopLeft, rightTopRight));
            int[][] p7 = multiply(subtractMatrices(leftTopRight, leftBottomRight), addMatrices(rightBottomLeft, rightBottomRight));

            // Result submatrices
            int[][] resultTopLeft = addMatrices(subtractMatrices(addMatrices(p1, p4), p5), p7);
            int[][] resultTopRight = addMatrices(p3, p5);
            int[][] resultBottomLeft = addMatrices(p2, p4);
            int[][] resultBottomRight = addMatrices(subtractMatrices(addMatrices(p1, p3), p2), p6);
    ...
    public void copySubmatrix(int[][] sourceMatrix, int[][] targetMatrix, int rowOffset, int columnOffset) {
        for (int targetRow = 0, sourceRow = rowOffset;
             targetRow < targetMatrix.length;
             targetRow++, sourceRow++) {
            for (int targetColumn = 0, sourceColumn = columnOffset;
                 targetColumn < targetMatrix.length;
    ...
    public void pasteSubmatrix(int[][] sourceMatrix, int[][] targetMatrix, int rowOffset, int columnOffset) {
        for (int sourceRow = 0, targetRow = rowOffset;
             sourceRow < sourceMatrix.length;
             sourceRow++, targetRow++) {
            for (int sourceColumn = 0, targetColumn = columnOffset;
                 sourceColumn < sourceMatrix.length;
