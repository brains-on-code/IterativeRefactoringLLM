...
public class Class1 {
    ...
    public void processSequence(boolean updateSequence) {
        ...
        List<Integer> workingSequence = (ArrayList<Integer>) ((ArrayList<Integer>) bitSequence).clone();
        ...
        List<Integer> result = (ArrayList<Integer>) ((ArrayList<Integer>) window).clone();

        if (!updateSequence) {
            bitSequence.addAll(result);
        } else {
            if (result.contains(1) && mutatedInCurrentRun) {
                truePositiveCount++;
            } else if (!result.contains(1) && mutatedInCurrentRun) {
                trueNegativeCount++;
    ...
    public void mutateSequence() {
        for (int bit : bitSequence) {
            ...
            if (value < mutationProbability) {
                ...
                int index = bitSequence.indexOf(bit);
                if (bit == 1) {
                    bitSequence.set(index, 0);
                } else {
                    bitSequence.set(index, 1);
                }
            }
        }
