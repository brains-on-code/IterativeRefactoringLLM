...
public final class KaprekarNumbers {
     ...
     */
    public static List<Long> findKaprekarNumbersInRange(long start, long end) {
        if (start > end) {
    ...
    public static boolean isKaprekar(long n) {
        ...
        int splitIndex = squareLength - numberLength;
        String leftPartStr = squareStr.substring(0, splitIndex);
        String rightPartStr = squareStr.substring(splitIndex);
    ...
    private static BigInteger extractPrefixBeforeFirstZero(String value) {
        ...
        if (zeroIndex > 0) {
            String prefix = value.substring(0, zeroIndex);
