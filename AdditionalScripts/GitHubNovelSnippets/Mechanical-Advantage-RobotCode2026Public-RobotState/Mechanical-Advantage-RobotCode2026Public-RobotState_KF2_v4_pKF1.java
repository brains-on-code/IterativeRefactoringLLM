...
public class RobotState {
  ...
  private RobotState() {
    for (int stateIndex = 0; stateIndex < 3; ++stateIndex) {
      double stdDev = ODOMETRY_STATE_STD_DEVS.get(stateIndex, 0);
      processNoiseVarianceDiagonal.set(stateIndex, 0, stdDev * stdDev);
    }
  ...
  public void addVisionObservation(VisionObservation visionObservation) {
    try {
      ...
      if (oldestAllowedTimestamp > visionObservation.timestamp()) {
        return;
      }
    ...
    var odometryPoseAtVisionTimestampOptional =
        poseBuffer.getSample(visionObservation.timestamp());
    if (odometryPoseAtVisionTimestampOptional.isEmpty()) {
      return;
    }

    Pose2d odometryPoseAtVisionTimestamp = odometryPoseAtVisionTimestampOptional.get();

    Transform2d odometryAtVisionToCurrentOdometryTransform =
        new Transform2d(odometryPoseAtVisionTimestamp, odometryPose);
    Transform2d currentOdometryToOdometryAtVisionTransform =
        new Transform2d(odometryPose, odometryPoseAtVisionTimestamp);

    Pose2d estimatedPoseAtVisionTimestamp =
        estimatedPose.plus(currentOdometryToOdometryAtVisionTransform);

    double[] visionMeasurementVariance = new double[3];
    for (int stateIndex = 0; stateIndex < 3; ++stateIndex) {
      double stdDev = visionObservation.standardDeviations().get(stateIndex, 0);
      visionMeasurementVariance[stateIndex] = stdDev * stdDev;
    }

    Matrix<N3, N3> kalmanGainMatrix = new Matrix<>(Nat.N3(), Nat.N3());
    for (int stateIndex = 0; stateIndex < 3; ++stateIndex) {
      double processVariance = processNoiseVarianceDiagonal.get(stateIndex, 0);
      if (processVariance == 0.0) {
        kalmanGainMatrix.set(stateIndex, stateIndex, 0.0);
      } else {
        double measurementVariance = visionMeasurementVariance[stateIndex];
        double gain =
            processVariance
                / (processVariance + Math.sqrt(processVariance * measurementVariance));
        kalmanGainMatrix.set(stateIndex, stateIndex, gain);
      }
    }

    Transform2d visionMeasurementTransform =
        new Transform2d(
            estimatedPoseAtVisionTimestamp, visionObservation.visionPose().toPose2d());
    ...
    estimatedPose =
        estimatedPoseAtVisionTimestamp
            .plus(scaledCorrectionTransform)
            .plus(odometryAtVisionToCurrentOdometryTransform);
  }
