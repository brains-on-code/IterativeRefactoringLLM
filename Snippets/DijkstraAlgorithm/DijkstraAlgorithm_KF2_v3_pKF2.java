...
public class DijkstraAlgorithm {
    ...
    private boolean canRelaxEdge(
        ...
        boolean neighborAlreadyProcessed = processed[neighbor];
        boolean noEdgeBetweenVertices = graph[currentVertex][neighbor] == 0;
        boolean currentDistanceUnknown = distances[currentVertex] == Integer.MAX_VALUE;

        return !neighborAlreadyProcessed && !noEdgeBetweenVertices && !currentDistanceUnknown;
    }
    ...
    private int getMinDistanceUnprocessedVertex(int[] distances, boolean[] processed) {
        ...
        for (int vertex = 0; vertex < vertexCount; vertex++) {
            ...
            boolean isCloserOrEqual = distances[vertex] <= minDistance;

            if (isUnprocessed && isCloserOrEqual) {
