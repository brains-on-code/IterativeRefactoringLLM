...
public final class Class1 {
    ...
    private static List<Integer> buildSequence(int start) {
        ...
        while (current != 1) {
            current = calculateNextValue(current);
            sequence.add(current);
        }
    ...
    private static int calculateNextValue(int current) {
        double sqrt = Math.sqrt(current);
        double nextValue = isEven(current) ? sqrt : Math.pow(sqrt, 3);
        return (int) Math.floor(nextValue);
    }
