...
public final class JohnsonsAlgorithm {
        ...
        for (int sourceVertex = 0; sourceVertex < vertexCount; sourceVertex++) {
            for (int targetVertex = 0; targetVertex < vertexCount; targetVertex++) {
                ...
                if (sourceVertex != targetVertex && !Double.isInfinite(weight)) {
                    edges.add(new double[] {sourceVertex, targetVertex, weight});
                }
            }
        }
    ...
    private static double[] bellmanFord(double[][] edgeList, int vertexCount) {
        ...
        for (int iteration = 0; iteration < vertexCount; iteration++) {
            for (double[] edge : extendedEdgeList) {
                ...
                if (distancesFromSuperSource[sourceVertex] != INFINITY
                    && distancesFromSuperSource[sourceVertex] + weight < distancesFromSuperSource[targetVertex]) {
        ...
        for (double[] edge : extendedEdgeList) {
            ...
            if (distancesFromSuperSource[sourceVertex] + weight < distancesFromSuperSource[targetVertex]) {
            ...
            for (int targetVertex = 0; targetVertex < vertexCount; targetVertex++) {
                ...
                if (originalWeight != 0) {
                    reweightedAdjacencyMatrix[sourceVertex][targetVertex] =
                        originalWeight + vertexPotentials[sourceVertex] - vertexPotentials[targetVertex];
                }
            }
        }
    ...
    public static double[] dijkstra(
        ...
        for (int iteration = 0; iteration < vertexCount - 1; iteration++) {
            int nearestUnvisitedVertex = findMinDistanceVertex(shortestDistances, visited);
            visited[nearestUnvisitedVertex] = true;

            for (int neighborVertex = 0; neighborVertex < vertexCount; neighborVertex++) {
                double edgeWeight = reweightedAdjacencyMatrix[nearestUnvisitedVertex][neighborVertex];

                if (!visited[neighborVertex]
                    && edgeWeight != 0
                    && shortestDistances[nearestUnvisitedVertex] != INFINITY
                    && shortestDistances[nearestUnvisitedVertex] + edgeWeight < shortestDistances[neighborVertex]) {
                    shortestDistances[neighborVertex] =
                        shortestDistances[nearestUnvisitedVertex] + edgeWeight;
                }
            }
        }

        for (int vertex = 0; vertex < vertexCount; vertex++) {
            if (shortestDistances[vertex] != INFINITY) {
                shortestDistances[vertex] =
                    shortestDistances[vertex] - vertexPotentials[sourceVertex] + vertexPotentials[vertex];
            }
        }
    ...
    public static int findMinDistanceVertex(double[] distances, boolean[] visited) {
        ...
        for (int vertex = 0; vertex < distances.length; vertex++) {
            if (!visited[vertex] && distances[vertex] <= minimumDistance) {
                ...
                vertexWithMinimumDistance = vertex;
            }
        }
