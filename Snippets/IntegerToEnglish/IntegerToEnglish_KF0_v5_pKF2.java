...
public final class IntegerToEnglish {
    ...
    public static String integerToEnglishWords(int number) {
        ...
        while (number > 0) {
            ...
            if (groupValue > 0) {
                String groupWords = convertThreeDigitGroupToWords(groupValue);
                if (!groupWords.isEmpty()) {
                    prependGroupWithScale(groupWords, thousandGroupIndex, result);
                }
            }
     ...
     * @return the English word representation of the input number
     */
    private static String convertThreeDigitGroupToWords(int number) {
        StringBuilder result = new StringBuilder();
    ...
    /**
     * Prepends the group words and corresponding scale word (if any) to the result.
     *
     * @param groupWords         words for the current 3-digit group
     * @param thousandGroupIndex index of the 3-digit group (0 = units, 1 = thousands, etc.)
     * @param result             the StringBuilder accumulating the final phrase
     */
    private static void prependGroupWithScale(
        String groupWords,
        int thousandGroupIndex,
