...
public final class ArrayCombination {
    ...
    public static List<List<Integer>> combination(int n, int k) {
        if (n < 0 || k < 0 || k > n) {
            throw new IllegalArgumentException(
                "Invalid input: n must be non-negative, k must be non-negative, and k must not exceed n."
            );
        }
    ...
    /**
     * Recursively builds combinations by trying all valid next integers.
     *
     * @param combinations collected complete combinations
     * @param current      combination under construction
