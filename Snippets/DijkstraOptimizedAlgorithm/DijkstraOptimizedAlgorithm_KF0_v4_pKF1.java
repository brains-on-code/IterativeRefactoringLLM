...
public class DijkstraOptimizedAlgorithm {
    ...
    public int[] run(int[][] adjacencyMatrix, int sourceVertex) {
        ...
        Set<Pair<Integer, Integer>> distanceToVertexSet = new TreeSet<>();

        Arrays.fill(shortestDistances, Integer.MAX_VALUE);
        Arrays.fill(visitedVertices, false);
        shortestDistances[sourceVertex] = 0;
        distanceToVertexSet.add(Pair.of(0, sourceVertex));

        while (!distanceToVertexSet.isEmpty()) {
            Pair<Integer, Integer> distanceAndVertex = distanceToVertexSet.iterator().next();
            distanceToVertexSet.remove(distanceAndVertex);
            ...
            for (int neighborVertex = 0; neighborVertex < vertexCount; neighborVertex++) {
                ...
                boolean neighborNotVisited = !visitedVertices[neighborVertex];
                ...
                if (neighborNotVisited && edgeExists && currentDistanceIsFinite) {
                    ...
                    if (newDistance < shortestDistances[neighborVertex]) {
                        distanceToVertexSet.remove(Pair.of(shortestDistances[neighborVertex], neighborVertex));
                        shortestDistances[neighborVertex] = newDistance;
                        distanceToVertexSet.add(Pair.of(shortestDistances[neighborVertex], neighborVertex));
                    }
                }
            }
        }
