...
public final class NumberToWords {
    ...
    public static String convert(BigDecimal number) {
        ...
        BigDecimal[] parts = absNumber.divideAndRemainder(BigDecimal.ONE);
    ...
    private static String extractFractionalPart(BigDecimal fractional) {
        ...
        String plain = fractional.toPlainString();
        int decimalIndex = plain.indexOf('.');

        if (decimalIndex < 0 || decimalIndex + 1 >= plain.length()) {
            return "";
        }

        return plain.substring(decimalIndex + 1);
    }
    ...
    private static String convertWholeNumberToWords(BigDecimal number) {
        ...
        while (number.compareTo(BigDecimal.ZERO) > 0) {
            ...
            if (chunk > 0) {
                ...
                StringBuilder chunkBuilder = new StringBuilder(chunkWords);

                if (powerIndex > 0 && powerIndex < POWERS.length) {
                    chunkBuilder.append(" ").append(POWERS[powerIndex]);
                }
