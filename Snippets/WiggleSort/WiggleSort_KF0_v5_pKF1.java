...
public class WiggleSort implements SortAlgorithm {
    ...
    private <T extends Comparable<T>> void threeWayPartitionAroundMedian(T[] array, T median) {
        ...
        while (scanIndex <= rightIndex) {
            int virtualScanIndex = toVirtualIndex(scanIndex, length);
            int comparisonResult = array[virtualScanIndex].compareTo(median);

            if (comparisonResult > 0) {
                SortUtils.swap(array, virtualScanIndex, toVirtualIndex(leftIndex, length));
                leftIndex++;
                scanIndex++;
            } else if (comparisonResult < 0) {
                SortUtils.swap(array, virtualScanIndex, toVirtualIndex(rightIndex, length));
                rightIndex--;
            } else {
                scanIndex++;
            }
