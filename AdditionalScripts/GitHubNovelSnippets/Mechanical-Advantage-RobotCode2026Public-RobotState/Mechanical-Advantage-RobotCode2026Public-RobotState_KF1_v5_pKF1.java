...
public class PoseEstimator {
  private static final double POSE_HISTORY_DURATION_SECONDS = 2.0;
  private static final Matrix<N3, N1> VISION_MEASUREMENT_STD_DEVS =
  ...
  private final TimeInterpolatableBuffer<Pose2d> poseHistory =
      TimeInterpolatableBuffer.createBuffer(POSE_HISTORY_DURATION_SECONDS);
  private final Matrix<N3, N1> visionProcessVariances = new Matrix<>(Nat.N3(), Nat.N1());
  ...
  private PoseEstimator() {
    for (int row = 0; row < 3; ++row) {
      double stdDev = VISION_MEASUREMENT_STD_DEVS.get(row, 0);
      visionProcessVariances.set(row, 0, Math.pow(stdDev, 2));
    }
  ...
  public void addOdometryMeasurement(OdometryMeasurement odometryMeasurement) {
    ...
    poseHistory.addSample(odometryMeasurement.timestamp(), estimatedPose);
  ...
  public void addVisionMeasurement(VisionMeasurement visionMeasurement) {
    ...
    try {
      double oldestValidTimestamp =
          poseHistory.getInternalBuffer().lastKey() - POSE_HISTORY_DURATION_SECONDS;
    ...
    var odometrySampleAtVisionTime = poseHistory.getSample(visionTimestamp);
