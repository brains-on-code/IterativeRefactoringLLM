...
public final class InsertDeleteInArray {
    ...
    private static int[] insertElement(int[] original, int insertIndex, int elementToInsert) {
        validateIndexForInsert(insertIndex, original.length);
    ...
    private static int[] deleteElement(int[] original, int deleteIndex) {
        validateIndexForDelete(deleteIndex, original.length);
        ...
        System.arraycopy(
            ...
            result,
            deleteIndex,
            original.length - deleteIndex - 1
        );

        return result;
    }

    private static void validateIndexForInsert(int index, int length) {
        if (index < 0 || index > length) {
            throw new IndexOutOfBoundsException("Insert index out of bounds: " + index);
        }
    }

    private static void validateIndexForDelete(int index, int length) {
        if (index < 0 || index >= length) {
