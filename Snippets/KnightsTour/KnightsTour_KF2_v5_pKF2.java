...
public final class KnightsTour {
     ...
     * The playable area is (BOARD_SIZE - 4) x (BOARD_SIZE - 4).
     */
    ...
    /**
     * Initialize the board with a 2-cell-wide invalid border.
    ...
    /**
     * Attempt to complete a knight's tour using backtracking and Warnsdorff's heuristic.
     *
    ...
    static boolean solve(int row, int col, int move) {
        ...
        // Warnsdorff's heuristic: visit squares with the fewest onward moves first.
    ...
    /**
     * Return all valid, unvisited knight moves from (row, col),
    ...
    /**
     * Count how many valid, unvisited knight moves exist from (row, col).
     *
     * @param row current row
     * @param col current column
    ...
    /**
     * Detect "orphan" squares: unvisited squares that will have no onward moves later,
    ...
    /**
     * Check if (row, col) lies within the board bounds.
     *
     * @param row row index
