...
class PNCounter {
    ...
    public boolean isLessThanOrEqualTo(PNCounter otherCounter) {
        ...
        for (int nodeId = 0; nodeId < totalNodeCount; nodeId++) {
            boolean thisHasGreaterPositive = this.positiveCountsByNodeId.get(nodeId) > otherCounter.positiveCountsByNodeId.get(nodeId);
            boolean thisHasGreaterNegative = this.negativeCountsByNodeId.get(nodeId) > otherCounter.negativeCountsByNodeId.get(nodeId);
    ...
    public void merge(PNCounter otherCounter) {
        ...
        for (int nodeId = 0; nodeId < totalNodeCount; nodeId++) {
            this.positiveCountsByNodeId.put(
                nodeId,
                Math.max(this.positiveCountsByNodeId.get(nodeId), otherCounter.positiveCountsByNodeId.get(nodeId))
            );
            this.negativeCountsByNodeId.put(
                nodeId,
                Math.max(this.negativeCountsByNodeId.get(nodeId), otherCounter.negativeCountsByNodeId.get(nodeId))
            );
        }
    }
}