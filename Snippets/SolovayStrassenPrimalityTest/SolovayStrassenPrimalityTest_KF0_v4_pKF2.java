...
final class SolovayStrassenPrimalityTest {
    ...
    public int calculateJacobi(long a, long n) {
        ...
        while (a != 0) {
            // Factor out powers of 2 from a and adjust the sign using (2/n) rules
            while ((a & 1) == 0) {
                a >>= 1;
                long nMod8 = n & 7;
    ...
    public boolean solovayStrassen(long n, int iterations) {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            boolean isJacobiZero = jacobi == 0;
            boolean isModDifferentFromJacobi = mod != jacobi;
            if (isJacobiZero || isModDifferentFromJacobi) {
                return false;
            }
        }
