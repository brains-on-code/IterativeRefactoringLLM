...
class JdbcDriverRegistry implements Iterable<JSONObject> {
    ...
    private void configureSlf4jLogging(PrintStream originalErr) {
        ...
        System.setErr(new PrintStream(originalErr) {
            ...
            public void println(String x) {
                if (x != null && x.startsWith(SLF4J_PREFIX) && suppressedSlf4jMessages < MAX_SUPPRESSED_SLF4J_MESSAGES) {
    ...
    public JSONObject findByName(String name) {
        ...
        for (JSONObject driverConfig : this) {
            String configName = driverConfig.getString(CONFIG_KEY_NAME);
    ...
    public Driver createDriver(JSONObject driverConfig) {
        ...
        JSONArray<String> artifacts = driverConfig.getJSONArray(CONFIG_KEY_ARTIFACT);
        String driverClassName = driverConfig.getString(CONFIG_KEY_DRIVER);

        return (Driver) instantiateFromArtifacts(artifacts, driverClassName);
    }
    ...
    public DataSource createDataSource(JSONObject driverConfig) {
        ...
        JSONArray<String> artifacts = driverConfig.getJSONArray(CONFIG_KEY_ARTIFACT);
        String dataSourceClassName = driverConfig.getString(CONFIG_KEY_DATASOURCE);

        return (DataSource) instantiateFromArtifacts(artifacts, dataSourceClassName);
    }
