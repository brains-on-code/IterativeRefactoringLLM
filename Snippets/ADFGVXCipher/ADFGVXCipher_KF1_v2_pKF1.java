...
public class AdfgvxCipher {
    ...
    static {
        for (int rowIndex = 0; rowIndex < SUBSTITUTION_SQUARE.length; rowIndex++) {
            for (int columnIndex = 0; columnIndex < SUBSTITUTION_SQUARE[rowIndex].length; columnIndex++) {
                String coordinate = "" + COORDINATE_LABELS[rowIndex] + COORDINATE_LABELS[columnIndex];
                char value = SUBSTITUTION_SQUARE[rowIndex][columnIndex];
    ...
    public String encrypt(String plaintext, String key) {
        ...
        for (char character : normalizedPlaintext.toCharArray()) {
            fractionatedPlaintext.append(CHAR_TO_COORDINATE.get(character));
        }
    ...
    private String encryptColumnarTransposition(String text, String key) {
        ...
        char[][] transpositionGrid = new char[rowCount][key.length()];
        ...
        for (int index = 0; index < text.length(); index++) {
            transpositionGrid[index / key.length()][index % key.length()] = text.charAt(index);
        }
