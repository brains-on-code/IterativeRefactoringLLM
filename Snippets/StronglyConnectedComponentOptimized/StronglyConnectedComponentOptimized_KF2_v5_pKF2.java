...
public class StronglyConnectedComponentOptimized {
        ...
        if (neighbors != null) {
            for (int neighbor : neighbors) {
                if (visited[neighbor] == UNVISITED) {
                    fillFinishOrder(graph, visited, finishOrder, neighbor);
                }
            }
        }
    ...
    private void collectComponent(
        ...
        if (neighbors != null) {
            for (int neighbor : neighbors) {
                if (visited[neighbor] == UNVISITED) {
                    collectComponent(reversedGraph, visited, neighbor, component);
                }
            }
        }
    }
    ...
    public int getOutput(HashMap<Integer, List<Integer>> graph, int n) {
        ...
        for (int node = 0; node < n; node++) {
            if (visited[node] == UNVISITED) {
                fillFinishOrder(graph, visited, finishOrder, node);
            }
        }

        HashMap<Integer, List<Integer>> reversedGraph = buildReversedGraph(graph, n);
        ...
        while (!finishOrder.isEmpty()) {
            ...
            if (visited[node] == UNVISITED) {
                ...
                collectComponent(reversedGraph, visited, node, component);
