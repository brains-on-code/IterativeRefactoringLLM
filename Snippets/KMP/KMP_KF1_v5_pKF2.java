...
public final class KmpSearch {
    ...
    public static void search(final String text, final String pattern) {
        ...
        final int[] lps = computeLongestPrefixSuffixArray(pattern);

        int patternIndex = 0;

        for (int textIndex = 0; textIndex < textLength; textIndex++) {
            // Move patternIndex back using LPS until characters match or patternIndex becomes 0
            while (patternIndex > 0 && text.charAt(textIndex) != pattern.charAt(patternIndex)) {
                patternIndex = lps[patternIndex - 1];
            }

            // If characters match, advance in the pattern
    ...
    private static int[] computeLongestPrefixSuffixArray(final String pattern) {
        ...
        for (int i = 1; i < patternLength; i++) {
            // Reduce currentPrefixLength using LPS until characters match or it becomes 0
            while (currentPrefixLength > 0 && pattern.charAt(currentPrefixLength) != pattern.charAt(i)) {
                currentPrefixLength = lps[currentPrefixLength - 1];
            }

            // If characters match, extend the current prefix length
            if (pattern.charAt(currentPrefixLength) == pattern.charAt(i)) {
                currentPrefixLength++;
            }

            lps[i] = currentPrefixLength;
        }
