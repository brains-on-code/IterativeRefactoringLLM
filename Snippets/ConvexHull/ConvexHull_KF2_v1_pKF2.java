...
public final class ConvexHull {
    ...
    /**
     * Returns true if point k lies on the left side of the directed line from i to j,
     * or if k is collinear and lies between i and j (inclusive).
     */
    private static boolean isPointOnLeftOrBetween(Point i, Point j, Point k) {
        int orientation = Point.orientation(i, j, k);

        if (orientation > 0) {
            return true; // k is to the left of segment ij
        } else if (orientation < 0) {
            return false; // k is to the right of segment ij
        } else {
            // Collinear: check if k lies between i and j
    ...
    /**
     * Recursively builds the convex hull between points left and right
     * from the given candidate set, adding hull points to hullPoints.
     */
