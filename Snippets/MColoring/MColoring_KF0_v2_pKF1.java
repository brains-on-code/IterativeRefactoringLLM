...
class GraphVertex {
    ...
    Set<Integer> adjacentVertexIndices = new HashSet<>(); // Adjacent vertex indices
}
...
public final class MColoring {
     ...
     *
     * @param vertices        List of vertices representing the graph (1-based indexing).
     ...
     */
    static boolean isColoringPossible(ArrayList<GraphVertex> vertices, int vertexCount, int maxAllowedColors) {

        // visited[i] == 1 means vertex i has been processed.
        ArrayList<Integer> visited = new ArrayList<>();
        for (int i = 0; i <= vertexCount; i++) {
            visited.add(0); // Initialize all vertices as unvisited (0)
        }
        ...
        for (int startVertexIndex = 1; startVertexIndex <= vertexCount; startVertexIndex++) {
            if (visited.get(startVertexIndex) > 0) {
                continue; // Skip vertices that are already visited
            }

            visited.set(startVertexIndex, 1);
            ...
            while (!bfsQueue.isEmpty()) {
                ...
                GraphVertex currentVertex = vertices.get(currentVertexIndex);

                // Check all adjacent vertices of the current vertex
                for (int adjacentVertexIndex : currentVertex.adjacentVertexIndices) {
                    GraphVertex adjacentVertex = vertices.get(adjacentVertexIndex);
                    ...
                    maxUsedColor = Math.max(maxUsedColor, Math.max(currentVertex.color, adjacentVertex.color));
                    ...
                    if (visited.get(adjacentVertexIndex) == 0) {
                        visited.set(adjacentVertexIndex, 1);
