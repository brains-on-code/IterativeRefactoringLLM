...
public final class IntervalUtils {
    ...
    public static boolean hasOverlappingIntervals(int[][] intervals) {
        ...
        int[] activeIntervalDeltas = new int[maxEndTime + 2];

        for (int[] interval : intervals) {
            int startTime = interval[0];
            int endTime = interval[1];

            activeIntervalDeltas[startTime] += 1;
            activeIntervalDeltas[endTime + 1] -= 1;
        }

        int activeIntervalCount = 0;
        int maxActiveIntervalCount = 0;
        for (int delta : activeIntervalDeltas) {
            activeIntervalCount += delta;
            maxActiveIntervalCount = Math.max(maxActiveIntervalCount, activeIntervalCount);
        }
