...
public final class NQueens {
     ...
     * @param rows        rows[c] = row index where the queen is placed in column c
     * @param columnIndex current column to place a queen in
     */
    private static void solve(int boardSize, List<List<String>> solutions, int[] rows, int columnIndex) {
        if (columnIndex == boardSize) {
            solutions.add(buildBoard(rows, boardSize));
            return;
        }

        for (int rowIndex = 0; rowIndex < boardSize; rowIndex++) {
            rows[columnIndex] = rowIndex;
            if (canPlace(rows, rowIndex, columnIndex)) {
                solve(boardSize, solutions, rows, columnIndex + 1);
            }
        }
    }

    /**
     * Builds a board representation from the rows array.
     *
     * @param rows      rows[c] = row index where the queen is placed in column c
     * @param boardSize size of the board
     * @return list of strings representing the board
     */
    private static List<String> buildBoard(int[] rows, int boardSize) {
        List<String> board = new ArrayList<>();
        for (int col = 0; col < boardSize; col++) {
            StringBuilder row = new StringBuilder();
            for (int rowIndex = 0; rowIndex < boardSize; rowIndex++) {
                row.append(rowIndex == rows[col] ? 'Q' : '.');
            }
     ...
     */
    private static boolean canPlace(int[] rows, int rowIndex, int columnIndex) {
        for (int col = 0; col < columnIndex; col++) {
            int rowDiff = Math.abs(rows[col] - rowIndex);
