...
public final class NQueensSolver {
    ...
    private static void solveNQueens(
        ...
        if (currentRow == boardSize) {
            ...
            for (int row = 0; row < boardSize; row++) {
                ...
                for (int column = 0; column < boardSize; column++) {
                    rowBuilder.append(column == queenColumnForRow[row] ? "Q" : ".");
                }
        ...
        for (int column = 0; column < boardSize; column++) {
            queenColumnForRow[currentRow] = column;
            if (isSafePlacement(queenColumnForRow, column, currentRow)) {
                solveNQueens(boardSize, solutions, queenColumnForRow, currentRow + 1);
            }
        }
    }

    private static boolean isSafePlacement(int[] queenColumnForRow, int candidateColumn, int candidateRow) {
        for (int previousRow = 0; previousRow < candidateRow; previousRow++) {
            int columnDifference = Math.abs(queenColumnForRow[previousRow] - candidateColumn);
