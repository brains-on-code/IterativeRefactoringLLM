...
public final class Class1 {
    ...
    /**
     * Computes the median of two sorted integer arrays using a binary search
    ...
    public static double method1(int[] nums1, int[] nums2) {
        ...
        while (left <= right) {
            ...
            boolean correctPartition =
                maxLeft1 <= minRight2 && maxLeft2 <= minRight1;

            if (correctPartition) {
                boolean totalLengthIsOdd = ((len1 + len2) & 1) == 1;
                if (totalLengthIsOdd) {
            ...
            boolean moveLeft = maxLeft1 > minRight2;
            if (moveLeft) {
                right = partition1 - 1;
            } else {
                left = partition1 + 1;
