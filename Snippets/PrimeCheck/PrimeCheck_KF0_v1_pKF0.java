...
public final class PrimeCheck {
    ...
    public static void main(String[] args) {
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            printDeterministicCheckResult(n);
            printFermatCheckResult(n, DEFAULT_FERMAT_ITERATIONS);
        }
    }

    private static void printDeterministicCheckResult(int n) {
        if (isPrime(n)) {
    ...
    private static void printFermatCheckResult(int n, int iterations) {
        if (isProbablyPrimeFermat(n, iterations)) {
    ...
    private static long getRandomLongInRange(int minInclusive, int maxInclusive) {
        return minInclusive + (long) RANDOM.nextInt(maxInclusive - minInclusive + 1);
    }
