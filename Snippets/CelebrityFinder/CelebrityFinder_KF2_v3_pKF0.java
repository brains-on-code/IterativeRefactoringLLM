...
public final class CelebrityFinder {
    ...
    private static int findPotentialCelebrity(int[][] party, Stack<Integer> candidates) {
        while (candidates.size() > 1) {
            ...
            if (knows(party, firstPerson, secondPerson)) {
                // If firstPerson knows secondPerson, firstPerson cannot be a celebrity
                candidates.push(secondPerson);
            } else {
                // If firstPerson does not know secondPerson, secondPerson cannot be a celebrity
    ...
    private static boolean isCelebrity(int[][] party, int candidate) {
        ...
        for (int person = 0; person < n; person++) {
            ...
            if (knows(party, candidate, person) || !knows(party, person, candidate)) {
                return false;
            }
        }
