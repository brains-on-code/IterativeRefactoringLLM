...
/**
 * Example usage of Dijkstra's shortest path algorithm on a small graph.
 */
public final class Class1 {

    private Class1() {}

    /** Sample graph edges: (source, target, weight). */
...
class Class2 {
    ...
    public static class Vertex implements Comparable<Vertex> {
        ...
        /** Recursively prints the path from the start vertex to this vertex. */
    ...
    Class2(Edge[] edges) {
        ...
        for (Edge edge : edges) {
            ...
            if (!vertices.containsKey(edge.target)) {
                vertices.put(edge.target, new Vertex(edge.target));
            }
        }

        // Add neighbours and weights.
        for (Edge edge : edges) {
            vertices.get(edge.source).neighbours.put(vertices.get(edge.target), edge.weight);
        }
    }
    ...
    /** Internal Dijkstra loop using a priority queue. */
    ...
    /** Prints the shortest paths from the start vertex to all vertices. */
