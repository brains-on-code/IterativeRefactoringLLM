...
public class FlashSort implements SortAlgorithm {
    ...
    private <T extends Comparable<? super T>> int getClassIndex(
        final T value,
        final double normalizationConstant,
        final T min
    ) {
        return (int) (normalizationConstant * value.compareTo(min));
    }
    ...
    private <T extends Comparable<? super T>> void classify(
        ...
        for (T value : array) {
            int classIndex = getClassIndex(value, normalizationConstant, min);
    ...
    private <T extends Comparable<? super T>> void permute(
        ...
        final int length = array.length;
        final int classCount = classCounts.length;
        ...
        while (movedElements < length - 1) {
            while (currentIndex > classCounts[currentClass] - 1) {
                currentIndex++;
                currentClass = getClassIndex(array[currentIndex], normalizationConstant, min);
            }

            T flash = array[currentIndex];

            while (currentIndex != classCounts[currentClass]) {
                currentClass = getClassIndex(flash, normalizationConstant, min);
