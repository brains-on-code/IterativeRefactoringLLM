...
public final class AssignmentUsingBitmask {
    ...
    public AssignmentUsingBitmask(List<List<Integer>> taskPerformed, int totalTasks) {
        ...
        this.dp = createDpTable();
        this.peoplePerTaskId = createPeoplePerTaskMapping(taskPerformed);
    }

    private int[][] createDpTable() {
    ...
    private List<List<Integer>> createPeoplePerTaskMapping(List<List<Integer>> taskPerformed) {
    ...
    private int countWays(int mask, int taskId) {
        ...
        for (int personId : peoplePerTaskId.get(taskId)) {
            if ((mask & (1 << personId)) == 0) {
                int updatedMask = mask | (1 << personId);
