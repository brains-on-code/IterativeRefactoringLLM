...
public class SkylineAlgorithm {
    ...
    public ArrayList<Point> computeSkyline(ArrayList<Point> points) {
        ...
        ArrayList<Point> leftPartitionPoints = new ArrayList<>();
        ArrayList<Point> rightPartitionPoints = new ArrayList<>();
        int middleIndex = numberOfPoints / 2;

        for (int index = 0; index < numberOfPoints; index++) {
            if (index < middleIndex) {
                leftPartitionPoints.add(points.get(index));
            } else {
                rightPartitionPoints.add(points.get(index));
            }
        }

        ArrayList<Point> leftSkyline = computeSkyline(leftPartitionPoints);
        ArrayList<Point> rightSkyline = computeSkyline(rightPartitionPoints);
    ...
    public ArrayList<Point> mergeSkylines(ArrayList<Point> leftSkyline, ArrayList<Point> rightSkyline) {
        ...
        int minimumYValue = leftSkyline.get(0).getY();
        for (int index = 1; index < leftSkyline.size(); index++) {
            int currentYValue = leftSkyline.get(index).getY();
            if (minimumYValue > currentYValue) {
                minimumYValue = currentYValue;
                if (minimumYValue == 1) {
                    break;
                }
            }
        ...
        for (int index = 0; index < rightSkyline.size(); index++) {
            if (rightSkyline.get(index).getY() >= minimumYValue) {
    ...
    public static class Point {
         ...
         * Dimension points.
         *
         * @param xCoordinate the point's x-value.
         * @param yCoordinate the point's y-value.
         */
        ...
        /**
         * @return xCoordinate, the x-value
         */
        public int getX() {
            return xCoordinate;
        }
        ...
        /**
         * @return yCoordinate, the y-value
         */
        public int getY() {
            return yCoordinate;
        }
