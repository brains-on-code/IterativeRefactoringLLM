...
public final class Class1 {
    ...
    public static int[] method1(int[] arr, int length) {
        ...
        computePreviousSmallerIndices(arr, length, indicesStack, previousSmallerIndex);
        indicesStack.clear();
        computeNextSmallerIndices(arr, length, indicesStack, nextSmallerIndex);

        int[] result = new int[length + 1];
        Arrays.fill(result, 0);

        fillResultWithWindowMinimums(arr, length, previousSmallerIndex, nextSmallerIndex, result);
        propagateMaximumsToSmallerWindows(length, result);

        return result;
    }

    private static void computePreviousSmallerIndices(
            int[] arr, int length, Stack<Integer> stack, int[] previousSmallerIndex) {
    ...
    private static void computeNextSmallerIndices(
            int[] arr, int length, Stack<Integer> stack, int[] nextSmallerIndex) {

        for (int i = length - 1; i >= 0; i--) {
            while (!stack.isEmpty() && arr[stack.peek()] >= arr[i]) {
                stack.pop();
            }
            nextSmallerIndex[i] = stack.isEmpty() ? length : stack.peek();
            stack.push(i);
        }
    }
    ...
    private static void propagateMaximumsToSmallerWindows(int length, int[] result) {
