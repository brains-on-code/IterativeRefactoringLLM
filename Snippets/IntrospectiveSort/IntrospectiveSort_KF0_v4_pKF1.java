package com.thealgorithms.sorts;

/**
 * Introspective Sort Algorithm Implementation
 *
 * @see <a href="https://en.wikipedia.org/wiki/Introsort">IntroSort Algorithm</a>
 */
public class IntrospectiveSort implements SortAlgorithm {

    private static final int INSERTION_SORT_THRESHOLD = 16;

    /**
     * Sorts the given array using Introspective Sort, which combines quicksort, heapsort, and insertion sort.
     *
     * @param array The array to be sorted
     * @param <T>   The type of elements in the array, which must be comparable
     * @return The sorted array
     */
    @Override
    public <T extends Comparable<T>> T[] sort(T[] array) {
        if (array == null || array.length <= 1) {
            return array;
        }
        final int maxRecursionDepth = 2 * (int) (Math.log(array.length) / Math.log(2));
        introspectiveSort(array, 0, array.length - 1, maxRecursionDepth);
        return array;
    }

    /**
     * Performs introspective sort on the specified subarray.
     *
     * @param array            The array to be sorted
     * @param leftIndex        The starting index of the subarray
     * @param rightIndex       The ending index of the subarray
     * @param remainingDepth   The remaining recursion depth before switching to heapsort
     * @param <T>              The type of elements in the array, which must be comparable
     */
    private static <T extends Comparable<T>> void introspectiveSort(
            T[] array, final int leftIndex, int rightIndex, final int remainingDepth) {

        int currentRightIndex = rightIndex;
        int currentDepth = remainingDepth;

        while (currentRightIndex - leftIndex > INSERTION_SORT_THRESHOLD) {
            if (currentDepth == 0) {
                heapSort(array, leftIndex, currentRightIndex);
                return;
            }
            final int pivotIndex = partition(array, leftIndex, currentRightIndex);
            introspectiveSort(array, pivotIndex + 1, currentRightIndex, currentDepth - 1);
            currentRightIndex = pivotIndex - 1;
            currentDepth--;
        }
        insertionSort(array, leftIndex, currentRightIndex);
    }

    /**
     * Partitions the array around a pivot.
     *
     * @param array      The array to be partitioned
     * @param leftIndex  The starting index of the subarray
     * @param rightIndex The ending index of the subarray
     * @param <T>        The type of elements in the array, which must be comparable
     * @return The index of the pivot
     */
    private static <T extends Comparable<T>> int partition(T[] array, final int leftIndex, final int rightIndex) {
        final int pivotIndex = leftIndex + (int) (Math.random() * (rightIndex - leftIndex + 1));
        SortUtils.swap(array, pivotIndex, rightIndex);
        final T pivotValue = array[rightIndex];

        int lastSmallerElementIndex = leftIndex - 1;
        for (int currentIndex = leftIndex; currentIndex < rightIndex; currentIndex++) {
            if (array[currentIndex].compareTo(pivotValue) <= 0) {
                lastSmallerElementIndex++;
                SortUtils.swap(array, lastSmallerElementIndex, currentIndex);
            }
        }
        SortUtils.swap(array, lastSmallerElementIndex + 1, rightIndex);
        return lastSmallerElementIndex + 1;
    }

    /**
     * Sorts a subarray using insertion sort.
     *
     * @param array      The array to be sorted
     * @param leftIndex  The starting index of the subarray
     * @param rightIndex The ending index of the subarray
     * @param <T>        The type of elements in the array, which must be comparable
     */
    private static <T extends Comparable<T>> void insertionSort(T[] array, final int leftIndex, final int rightIndex) {
        for (int currentIndex = leftIndex + 1; currentIndex <= rightIndex; currentIndex++) {
            final T currentValue = array[currentIndex];
            int previousIndex = currentIndex - 1;

            while (previousIndex >= leftIndex && array[previousIndex].compareTo(currentValue) > 0) {
                array[previousIndex + 1] = array[previousIndex];
                previousIndex--;
            }
            array[previousIndex + 1] = currentValue;
        }
    }

    /**
     * Sorts a subarray using heapsort.
     *
     * @param array      The array to be sorted
     * @param leftIndex  The starting index of the subarray
     * @param rightIndex The ending index of the subarray
     * @param <T>        The type of elements in the array, which must be comparable
     */
    private static <T extends Comparable<T>> void heapSort(T[] array, final int leftIndex, final int rightIndex) {
        final int heapSize = rightIndex - leftIndex + 1;

        for (int heapIndex = (heapSize / 2) - 1; heapIndex >= 0; heapIndex--) {
            heapify(array, heapIndex, heapSize, leftIndex);
        }

        for (int currentRightIndex = rightIndex; currentRightIndex > leftIndex; currentRightIndex--) {
            SortUtils.swap(array, leftIndex, currentRightIndex);
            heapify(array, 0, currentRightIndex - leftIndex, leftIndex);
        }
    }

    /**
     * Maintains the heap property for a subarray.
     *
     * @param array        The array to be heapified
     * @param rootIndex    The index of the root element in the heap (relative to the heap)
     * @param heapSize     The size of the heap
     * @param offsetIndex  The starting index of the subarray in the original array
     * @param <T>          The type of elements in the array, which must be comparable
     */
    private static <T extends Comparable<T>> void heapify(
            T[] array, final int rootIndex, final int heapSize, final int offsetIndex) {

        final int leftChildIndex = 2 * rootIndex + 1;
        final int rightChildIndex = 2 * rootIndex + 2;
        int largestIndex = rootIndex;

        if (leftChildIndex < heapSize
                && array[offsetIndex + leftChildIndex].compareTo(array[offsetIndex + largestIndex]) > 0) {
            largestIndex = leftChildIndex;
        }
        if (rightChildIndex < heapSize
                && array[offsetIndex + rightChildIndex].compareTo(array[offsetIndex + largestIndex]) > 0) {
            largestIndex = rightChildIndex;
        }
        if (largestIndex != rootIndex) {
            SortUtils.swap(array, offsetIndex + rootIndex, offsetIndex + largestIndex);
            heapify(array, largestIndex, heapSize, offsetIndex);
        }
    }
}