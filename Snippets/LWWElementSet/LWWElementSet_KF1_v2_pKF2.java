...
/**
 * Represents a single operation (add or remove) on an element in the OR-Set.
 */
class Operation {
 ...
 *
 * <p>Tracks the latest add and remove operation per element and supports
 * validation, subset checks, and merging between replicas.</p>
 */
class BiasedORSet {
    ...
    /**
     * Merges another BiasedORSet into this one.
     *
    ...
    public void merge(BiasedORSet other) {
        for (Operation incoming : other.adds.values()) {
            Operation current = adds.get(incoming.element);
            if (current == null || dominates(current, incoming)) {
                adds.put(incoming.element, incoming);
            }
        }

        for (Operation incoming : other.removals.values()) {
            Operation current = removals.get(incoming.element);
            if (current == null || dominates(current, incoming)) {
                removals.put(incoming.element, incoming);
            }
        }
    }
    ...
    public boolean dominates(Operation current, Operation candidate) {
        ...
        if (comparison == 0) {
            ...
            // For ADDS bias, existing add wins; for REMOVALS bias, candidate wins.
