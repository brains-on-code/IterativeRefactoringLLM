...
public class BaconianCipher {
    ...
    static {
        ...
        // Map I and J to the same code; decode that code as I
     ...
     * 2. Convert remaining characters to uppercase.
     * 3. Replace each character with its 5-letter Baconian code.
     *
     ...
     * - Each 5-character group must be a valid Baconian code.
     *
     * @param encoded the Baconian-encoded string (only 'A' and 'B')
     * @return the decoded plaintext
     * @throws IllegalArgumentException if the length is invalid or a code group is not recognized
     */
    public String decode(String encoded) {
        if (encoded.length() % GROUP_SIZE != 0) {
            throw new IllegalArgumentException("Encoded string length must be a multiple of " + GROUP_SIZE + ".");
        }

        StringBuilder decoded = new StringBuilder();

        for (int i = 0; i < encoded.length(); i += GROUP_SIZE) {
            String group = encoded.substring(i, i + GROUP_SIZE);
