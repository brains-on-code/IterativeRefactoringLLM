...
public final class ChineseRemainderTheorem {
    ...
    public static int solveCRT(List<Integer> remainders, List<Integer> moduli) {
        int totalModulusProduct = 1;
        int combinedSolution = 0;

        for (int modulus : moduli) {
            totalModulusProduct *= modulus;
        }
        ...
        for (int index = 0; index < moduli.size(); index++) {
            ...
            int partialModulusProduct = totalModulusProduct / modulus;
            int inverse = modularInverse(partialModulusProduct, modulus);

            combinedSolution += remainder * partialModulusProduct * inverse;
        }

        combinedSolution %= totalModulusProduct;
        if (combinedSolution < 0) {
            combinedSolution += totalModulusProduct;
        }
