...
public final class NonPreemptivePriorityScheduling {
    ...
    public static Process[] scheduleProcesses(Process[] processes) {
        ...
        Queue<Process> pendingArrivalQueue = new LinkedList<>();
        int currentTime = 0;
        int executionOrderIndex = 0;
        Process[] executionOrder = new Process[processes.length];

        Collections.addAll(pendingArrivalQueue, processes);

        while (!pendingArrivalQueue.isEmpty() || !readyQueue.isEmpty()) {
            while (!pendingArrivalQueue.isEmpty() && pendingArrivalQueue.peek().arrivalTime <= currentTime) {
                readyQueue.add(pendingArrivalQueue.poll());
            }

            if (!readyQueue.isEmpty()) {
                Process currentProcess = readyQueue.poll();
                currentProcess.startTime = currentTime;
                executionOrder[executionOrderIndex++] = currentProcess;
                currentTime += currentProcess.burstTime;
            } else {
                currentTime = pendingArrivalQueue.peek().arrivalTime;
            }
        }
