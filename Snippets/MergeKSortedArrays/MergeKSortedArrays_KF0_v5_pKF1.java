...
public final class MergeKSortedArrays {
    ...
    public static int[] mergeKArrays(int[][] arrays) {
        PriorityQueue<int[]> minHeap =
            new PriorityQueue<>(Comparator.comparingInt(heapNode -> heapNode[0]));

        int totalElementCount = 0;
        for (int arrayIndex = 0; arrayIndex < arrays.length; arrayIndex++) {
            int[] currentArray = arrays[arrayIndex];
            if (currentArray.length > 0) {
                int firstElementValue = currentArray[0];
                int firstElementIndex = 0;
                minHeap.offer(new int[] {firstElementValue, arrayIndex, firstElementIndex});
        ...
        while (!minHeap.isEmpty()) {
            int[] heapNode = minHeap.poll();
            int value = heapNode[0];
            int sourceArrayIndex = heapNode[1];
            int sourceElementIndex = heapNode[2];

            mergedArray[mergedArrayIndex++] = value;

            int nextElementIndex = sourceElementIndex + 1;
            int[] sourceArray = arrays[sourceArrayIndex];
            if (nextElementIndex < sourceArray.length) {
                int nextValue = sourceArray[nextElementIndex];
                minHeap.offer(new int[] {nextValue, sourceArrayIndex, nextElementIndex});
            }
        }
