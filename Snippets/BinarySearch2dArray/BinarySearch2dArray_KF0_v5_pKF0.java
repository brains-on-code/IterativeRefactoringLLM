...
public final class BinarySearch2dArray {
    ...
    static int[] binarySearch(int[][] arr, int target) {
        ...
        int[] narrowedRows = narrowToTwoRows(arr, target, midCol, startRow, endRow);
        startRow = narrowedRows[0];
        endRow = narrowedRows[1];
    ...
    /**
     * Narrows the search space to at most two rows using binary search on rows.
     *
     * @param arr      The 2D array.
     * @param target   The value to search for.
     * @param midCol   The middle column index.
     * @param startRow The starting row index.
     * @param endRow   The ending row index.
     * @return An array containing the updated startRow and endRow.
     */
    private static int[] narrowToTwoRows(int[][] arr, int target, int midCol, int startRow, int endRow) {
        while (startRow < endRow - 1) {
            int midRow = startRow + (endRow - startRow) / 2;
            int midValue = arr[midRow][midCol];

            if (midValue == target) {
                return new int[] {midRow, midRow};
            }
