    ...
    public static int[] mergeKArrays(int[][] arrays) {
        ...
        for (int arrayIndex = 0; arrayIndex < arrays.length; arrayIndex++) {
            if (arrays[arrayIndex].length > 0) {
                int firstElementValue = arrays[arrayIndex][0];
                int elementIndex = 0;
                minHeap.offer(new int[] {firstElementValue, arrayIndex, elementIndex});
        ...
        while (!minHeap.isEmpty()) {
            ...
            int arrayIndex = heapNode[1];
            int elementIndex = heapNode[2];
