...
class PNCounter {
    ...
    public void increment() {
        increments.compute(nodeId, (id, value) -> value == null ? 1 : value + 1);
    }

    /** Decrements the counter for this node. */
    public void decrement() {
        decrements.compute(nodeId, (id, value) -> value == null ? 1 : value + 1);
    }
    ...
    public void merge(PNCounter other) {
        ...
        for (int i = 0; i < nodeCount; i++) {
            increments.compute(i, (id, value) -> Math.max(
                value == null ? 0 : value,
                other.increments.getOrDefault(id, 0)
            ));
            decrements.compute(i, (id, value) -> Math.max(
                value == null ? 0 : value,
                other.decrements.getOrDefault(id, 0)
            ));
        }
    }
