...
public class Class1 {
    ...
    private void calculateTurnaroundTimes() {
        ...
        initializeRemainingBurstTimes(remainingBurstTimes);
        ...
        while (completedProcesses < processCount) {
            ...
            currentTime = adjustStartTimeIfFirstExecution(currentTime, currentIndex, currentProcess, remainingBurstTimes);

            if (remainingBurstTimes[currentIndex] > timeQuantum) {
                remainingBurstTimes[currentIndex] -= timeQuantum;
                currentTime += timeQuantum;
            } else {
                currentTime = completeProcess(currentTime, currentIndex, currentProcess, remainingBurstTimes);
                completedProcesses++;
            }

            enqueueArrivedProcesses(processCount, currentTime, isInQueue, remainingBurstTimes, readyQueue);
            ...
            if (readyQueue.isEmpty()) {
                enqueueNextAvailableProcess(processCount, isInQueue, remainingBurstTimes, readyQueue);
            }
        }
    }

    private void initializeRemainingBurstTimes(final int[] remainingBurstTimes) {
