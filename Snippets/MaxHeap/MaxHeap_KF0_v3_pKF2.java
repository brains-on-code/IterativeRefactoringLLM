...
public class MaxHeap implements Heap {
    ...
    /** Internal storage for heap elements (0-based index, public API uses 1-based indices). */
    ...
    /** Builds a valid max-heap from the current internal list (bottom-up). */
    private void buildHeap() {
        for (int i = maxHeap.size() / 2; i >= 0; i--) {
            heapifyDown(i + 1); // heapifyDown expects a 1-based index
        }
    }

    /**
     * Restores the heap property for the subtree rooted at the given index by moving the element
    ...
    private void heapifyDown(int elementIndex) {
        ...
        if (largest != elementIndex - 1) {
            swapZeroBased(elementIndex - 1, largest);
    ...
    private void swap(int index1, int index2) {
        swapZeroBased(index1 - 1, index2 - 1);
    }

    /** Swaps two elements in the heap using 0-based indices. */
    private void swapZeroBased(int index1, int index2) {
        HeapElement temporaryElement = maxHeap.get(index1);
        maxHeap.set(index1, maxHeap.get(index2));
        maxHeap.set(index2, temporaryElement);
    }
