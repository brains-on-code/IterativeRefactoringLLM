...
public final class QuickSelect {
    ...
    public static <T extends Comparable<T>> T select(List<T> list, int n) {
        ...
        return list.get(selectIndex(list, n));
    }
    ...
    private static <T extends Comparable<T>> int partition(
        ...
        int lessThanPivotEnd = partitionLessThanPivot(list, left, right, pivotValue);
        int equalToPivotEnd = partitionEqualToPivot(list, lessThanPivotEnd, right, pivotValue);

        Collections.swap(list, right, equalToPivotEnd);

        if (n < lessThanPivotEnd) {
            return lessThanPivotEnd;
        }
        return Math.min(n, equalToPivotEnd);
    }
    ...
    private static <T extends Comparable<T>> int choosePivotIndex(List<T> list, int left, int right) {
        ...
        for (; groupStart <= right; groupStart += GROUP_SIZE) {
            ...
            int medianOfGroup = partition5(list, groupStart, subRight);
            int targetIndex = left + (groupStart - left) / GROUP_SIZE;
            Collections.swap(list, medianOfGroup, targetIndex);
        }
