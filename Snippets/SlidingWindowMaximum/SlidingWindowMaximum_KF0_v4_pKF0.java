...
public final class SlidingWindowMaximum {
    ...
    public static int[] maxSlidingWindow(int[] nums, int k) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            evictOutOfWindowIndices(windowIndices, windowStart);
            evictSmallerValuesFromBack(nums, windowIndices, i);
    ...
    private static void evictOutOfWindowIndices(Deque<Integer> windowIndices, int windowStart) {
        while (!windowIndices.isEmpty() && windowIndices.peekFirst() < windowStart) {
            windowIndices.pollFirst();
        }
    }

    private static void evictSmallerValuesFromBack(int[] nums, Deque<Integer> windowIndices, int currentIndex) {
