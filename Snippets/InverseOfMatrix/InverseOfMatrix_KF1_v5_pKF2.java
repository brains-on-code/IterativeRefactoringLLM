...
public final class Class1 {
    ...
    public static double[][] method1(double[][] matrix) {
        ...
        for (int col = 0; col < n; ++col) {
            ...
            int lastPivotRow = pivotIndices[n - 1];
            inverse[n - 1][col] = identity[lastPivotRow][col] / matrix[lastPivotRow][n - 1];

            // Backward substitution for remaining rows.
            for (int row = n - 2; row >= 0; --row) {
                int pivotRow = pivotIndices[row];
    ...
    private static void method2(double[][] matrix, int[] pivotIndices) {
        ...
        for (int col = 0; col < n - 1; ++col) {
            ...
            for (int row = col + 1; row < n; ++row) {
                int currentRowIndex = pivotIndices[row];
                int pivotIndex = pivotIndices[col];

                double multiplier = matrix[currentRowIndex][col] / matrix[pivotIndex][col];

                // Store multiplier in L (below diagonal).
                matrix[currentRowIndex][col] = multiplier;

                // Update remaining entries in the current row.
                for (int k = col + 1; k < n; ++k) {
                    matrix[currentRowIndex][k] -= multiplier * matrix[pivotIndex][k];
                }
            }
        }
    }
