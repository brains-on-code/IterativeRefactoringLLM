...
public class GeneticPatternMatcher {
     ...
     */
    public GeneticPatternMatcher(String targetPatternString, int chromosomeLength, double mutationRate) {
        this.mutationOccurredInRun = false;
        this.chromosome = new ArrayList<>();
        this.chromosomeLength = chromosomeLength;
        this.targetPattern = new ArrayList<>();
        for (int index = 0; index < targetPatternString.length(); index++) {
            targetPattern.add(Character.getNumericValue(targetPatternString.charAt(index)));
        }
    ...
    public void processChromosome(boolean evaluateOnly) {
        ...
        if (!evaluateOnly) {
            for (int index = 0; index < targetPattern.size() - 1; index++) {
                workingChromosome.add(0);
            }
        }

        while (!workingChromosome.isEmpty()) {
            while (slidingWindow.size() < targetPattern.size() && !workingChromosome.isEmpty()) {
                slidingWindow.add(workingChromosome.get(0));
                workingChromosome.remove(0);
            }
            if (slidingWindow.size() == targetPattern.size()) {
                for (int index = 0; index < targetPattern.size(); index++) {
                    if (slidingWindow.get(index).equals(targetPattern.get(index))) {
                        slidingWindow.set(index, 0);
                    } else {
                        slidingWindow.set(index, 1);
                    }
                }
                for (int index = 0; index < slidingWindow.size() && slidingWindow.get(index) != 1; index++) {
                    slidingWindow.remove(0);
                }
            }
        }

        List<Integer> resultPattern = new ArrayList<>(slidingWindow);

        if (!evaluateOnly) {
            chromosome.addAll(resultPattern);
        } else {
            if (resultPattern.contains(1) && mutationOccurredInRun) {
                successfulMutationRunCount++;
            } else if (!resultPattern.contains(1) && mutationOccurredInRun) {
