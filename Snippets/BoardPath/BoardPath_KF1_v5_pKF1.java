...
public final class DiceThrow {
    ...
    public static int countWaysRecursive(int currentPosition, int targetPosition) {
        ...
        int totalWays = 0;
        for (int rollValue = 1; rollValue <= DIE_SIDES; rollValue++) {
            totalWays += countWaysRecursive(currentPosition + rollValue, targetPosition);
        }
        return totalWays;
    }
     ...
     * @param currentPosition the starting position
     * @param targetPosition  the destination position
     * @param waysFromPosition memoization array where waysFromPosition[i]
     *                         stores the number of ways to reach targetPosition
     *                         from position i
     * @return number of ways to reach targetPosition from currentPosition
     */
    public static int countWaysMemoized(int currentPosition, int targetPosition, int[] waysFromPosition) {
        if (currentPosition == targetPosition) {
            return 1;
        }
        if (currentPosition > targetPosition) {
        ...
        if (waysFromPosition[currentPosition] != 0) {
            return waysFromPosition[currentPosition];
        }

        int totalWays = 0;
        for (int rollValue = 1; rollValue <= DIE_SIDES; rollValue++) {
            totalWays += countWaysMemoized(currentPosition + rollValue, targetPosition, waysFromPosition);
        }
        waysFromPosition[currentPosition] = totalWays;
        return totalWays;
    }
    ...
    public static int countWaysBottomUp(int startPosition, int targetPosition, int[] waysFromPosition) {
        ...
        for (int position = targetPosition - 1; position >= 0; position--) {
            int totalWays = 0;
            for (int rollValue = 1; rollValue <= DIE_SIDES && position + rollValue <= targetPosition; rollValue++) {
                totalWays += waysFromPosition[position + rollValue];
            }
