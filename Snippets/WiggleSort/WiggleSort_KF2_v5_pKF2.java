...
public class WiggleSort implements SortAlgorithm {
    ...
    private <T extends Comparable<T>> T[] wiggleSort(T[] array) {
        ...
        int halfRoundedUp = (int) ceil(n / 2.0);

        if (isOddLength && medianCount == halfRoundedUp) {
        ...
        if (medianCount > halfRoundedUp) {
            throw new IllegalArgumentException(
                "No value may appear more than ceil(n/2) times in a valid wiggle sort."
            );
        }
