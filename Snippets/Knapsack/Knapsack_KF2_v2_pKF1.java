...
public final class Knapsack {
    ...
    public static int knapSack(final int capacity, final int[] weights, final int[] values) throws IllegalArgumentException {
        ...
        for (int itemIndex = 0; itemIndex < values.length; itemIndex++) {
            ...
            for (int currentCapacity = capacity; currentCapacity > 0; currentCapacity--) {
                if (itemWeight <= currentCapacity) {
                    maxValueAtCapacity[currentCapacity] =
                        Math.max(
                            maxValueAtCapacity[currentCapacity],
                            maxValueAtCapacity[currentCapacity - itemWeight] + itemValue
                        );
                }
            }
        }
