...
public final class MyAtoi {
    ...
    private static boolean willOverflow(int current, int digit, boolean isNegative) {
        ...
        if (!isNegative && (current > threshold || (current == threshold && digit > remainder))) {
            return true;
        }

        if (isNegative) {
            int negativeDigit = -digit;
            if (current < threshold || (current == threshold && negativeDigit < remainder)) {
                return true;
            }
        }
