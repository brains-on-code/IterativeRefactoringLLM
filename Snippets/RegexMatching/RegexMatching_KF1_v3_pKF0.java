...
public final class WildcardMatcher {
    ...
    public static boolean matchesRecursive(String text, String pattern) {
        ...
        if (patternChar == MULTI_WILDCARD) {
            boolean matchZeroChars = matchesRecursive(text, remainingPattern);
            boolean matchOneOrMoreChars = matchesRecursive(remainingText, pattern);
    ...
    static boolean matchesRecursive(String text, String pattern, int textIndex, int patternIndex) {
        ...
        if (patternChar == MULTI_WILDCARD) {
            boolean matchZeroChars = matchesRecursive(text, pattern, textIndex, patternIndex + 1);
            boolean matchOneOrMoreChars = matchesRecursive(text, pattern, textIndex + 1, patternIndex);
            return matchZeroChars || matchOneOrMoreChars;
        }
    ...
    public static boolean matchesRecursiveMemo(
        ...
        if (patternChar == textChar || patternChar == SINGLE_WILDCARD) {
            ...
            boolean matchZeroChars = matchesRecursiveMemo(text, pattern, textIndex, patternIndex + 1, memo);
            boolean matchOneOrMoreChars = matchesRecursiveMemo(text, pattern, textIndex + 1, patternIndex, memo);
