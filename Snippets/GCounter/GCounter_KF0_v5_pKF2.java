...
class GCounter {
    ...
    /** Increments this replica's component of the counter. */
    public void increment() {
        replicaCounts.computeIfPresent(replicaId, (id, count) -> count + 1);
    }
     ...
     */
    public int value() {
        return replicaCounts.values().stream().mapToInt(Integer::intValue).sum();
    }
    ...
    public boolean compare(GCounter other) {
        for (int id = 0; id < replicaCount; id++) {
            int thisCount = replicaCounts.getOrDefault(id, 0);
            int otherCount = other.replicaCounts.getOrDefault(id, 0);
    ...
    public void merge(GCounter other) {
        for (int id = 0; id < replicaCount; id++) {
            int thisCount = replicaCounts.getOrDefault(id, 0);
            int otherCount = other.replicaCounts.getOrDefault(id, 0);
            replicaCounts.put(id, Math.max(thisCount, otherCount));
        }
    }
}