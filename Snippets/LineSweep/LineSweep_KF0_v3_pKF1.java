...
public final class LineSweep {
    ...
    public static boolean hasOverlap(int[][] ranges) {
        ...
        for (int[] range : ranges) {
            ...
            sweepLineDeltas[start] += 1;
            sweepLineDeltas[end + 1] -= 1;
        }

        int currentActiveRangeCount = 0;
        int maxActiveRangeCount = 0;

        for (int delta : sweepLineDeltas) {
            currentActiveRangeCount += delta;
            maxActiveRangeCount = Math.max(maxActiveRangeCount, currentActiveRangeCount);
        }

        return maxActiveRangeCount > 1;
    }
}