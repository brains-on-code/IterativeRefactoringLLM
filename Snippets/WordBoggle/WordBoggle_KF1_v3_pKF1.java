...
public final class WordSearch {
    ...
    private static void depthFirstSearch(
        ...
        if (!trieNode.children.containsKey(currentCharacter)) {
            return;
        }

        visitedCells[row][column] = true;
        trieNode = trieNode.children.get(currentCharacter);

        if (trieNode.children.containsKey(Trie.END_SYMBOL)) {
            foundWords.add(trieNode.word);
        }

        List<int[]> neighborPositions = getNeighborPositions(row, column, board);
        for (int[] neighborPosition : neighborPositions) {
            depthFirstSearch(neighborPosition[0], neighborPosition[1], board, trieNode, visitedCells, foundWords);
        }
    ...
    private static List<int[]> getNeighborPositions(int row, int column, char[][] board) {
        ...
        if (row > 0 && column > 0) {
            neighborPositions.add(new int[] {row - 1, column - 1});
        }
        ...
        if (column > 0) {
            neighborPositions.add(new int[] {row, column - 1});
        }
