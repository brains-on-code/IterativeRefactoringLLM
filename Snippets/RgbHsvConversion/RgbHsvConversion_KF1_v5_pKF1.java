...
public final class HsvRgbConverter {
    ...
    public static int[] hsvToRgb(double hue, double saturation, double value) {
        ...
        double hueSectorIndex = hue / 60;
        double secondaryComponent = chroma * (1 - Math.abs(hueSectorIndex % 2 - 1));
        double matchComponent = value - chroma;

        return composeRgbFromChroma(hueSectorIndex, chroma, matchComponent, secondaryComponent);
    }
    ...
    private static boolean hsvAlmostEquals(double[] actualHsv, double[] expectedHsv) {
        double actualHue = actualHsv[0];
        double actualSaturation = actualHsv[1];
        double actualValue = actualHsv[2];

        double expectedHue = expectedHsv[0];
        double expectedSaturation = expectedHsv[1];
        double expectedValue = expectedHsv[2];

        boolean isHueClose = Math.abs(actualHue - expectedHue) < 0.2;
        boolean isSaturationClose = Math.abs(actualSaturation - expectedSaturation) < 0.002;
        boolean isValueClose = Math.abs(actualValue - expectedValue) < 0.002;

        return isHueClose && isSaturationClose && isValueClose;
    }

    private static int[] composeRgbFromChroma(
            double hueSectorIndex, double chroma, double matchComponent, double secondaryComponent) {

        int red;
        int green;
        int blue;

        if (hueSectorIndex >= 0 && hueSectorIndex <= 1) {
    ...
    private static double normalizeRgbComponent(int rgbComponent) {
        return (double) rgbComponent / 255;
    }
