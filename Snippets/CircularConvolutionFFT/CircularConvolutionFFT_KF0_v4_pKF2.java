...
public final class CircularConvolutionFFT {
     ...
     * If {@code signal.size() >= newSize}, this method does nothing.
     *
     ...
     * Algorithm:
     * <ol>
     *   <li>Zero-pad both signals to length {@code max(a.size(), b.size())}.</li>
     *   <li>Compute the FFT of both padded signals.</li>
     *   <li>Multiply the FFT results element-wise.</li>
     *   <li>Apply the inverse FFT to the product.</li>
     * </ol>
     *
     * Bluestein's FFT algorithm is used so the transform length matches
     * the (padded) signal length.
     *
