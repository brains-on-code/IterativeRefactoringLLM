...
public class ADFGVXCipher {
    ...
    static {
        for (int rowIndex = 0; rowIndex < POLYBIUS_SQUARE.length; rowIndex++) {
            for (int columnIndex = 0; columnIndex < POLYBIUS_SQUARE[rowIndex].length; columnIndex++) {
                String coordinate = "" + POLYBIUS_LABELS[rowIndex] + POLYBIUS_LABELS[columnIndex];
                char mappedCharacter = POLYBIUS_SQUARE[rowIndex][columnIndex];

                COORDINATE_TO_CHARACTER.put(coordinate, mappedCharacter);
                CHARACTER_TO_COORDINATE.put(mappedCharacter, coordinate);
            }
        }
    }
    ...
    public String encrypt(String plaintext, String key) {
        ...
        for (char currentChar : sanitizedPlaintext.toCharArray()) {
            fractionatedTextBuilder.append(CHARACTER_TO_COORDINATE.get(currentChar));
        }

        String fractionatedText = fractionatedTextBuilder.toString();
        return performColumnarTransposition(fractionatedText, key);
    }
    ...
    public String decrypt(String ciphertext, String key) {
        ...
        for (int index = 0; index < fractionatedText.length(); index += 2) {
            String coordinate = fractionatedText.substring(index, index + 2);
            plaintextBuilder.append(COORDINATE_TO_CHARACTER.get(coordinate));
        }
