...
public class WiggleSort implements SortAlgorithm {
    ...
    private <T extends Comparable<T>> void triColorSort(T[] array, T median) {
        ...
        while (current <= right) {
            int mappedCurrentIndex = mapIndex(current, length);
            int comparison = array[mappedCurrentIndex].compareTo(median);

            if (comparison > 0) {
                SortUtils.swap(array, mappedCurrentIndex, mapIndex(left, length));
                left++;
                current++;
            } else if (comparison < 0) {
                SortUtils.swap(array, mappedCurrentIndex, mapIndex(right, length));
                right--;
            } else {
                current++;
            }
    ...
    private <T extends Comparable<T>> T[] wiggleSort(T[] array) {
        ...
        validateWiggleSortable(array, median, length, medianCount);
    ...
    private <T extends Comparable<T>> void validateWiggleSortable(
        ...
        if (isOdd(length) && medianCount == ceil(halfLength)) {
        ...
        if (medianCount > ceil(halfLength)) {
            throw new IllegalArgumentException(
