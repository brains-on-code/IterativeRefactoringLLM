...
public class IntrospectiveSort implements SortAlgorithm {
    ...
    private static <T extends Comparable<T>> int partition(
        ...
        for (int current = low; current < high; current++) {
            if (array[current].compareTo(pivot) <= 0) {
                boundary++;
                SortUtils.swap(array, boundary, current);
            }
        }
    ...
    private static <T extends Comparable<T>> void heapify(
        ...
        int largestIndexInHeap = index;

        int largestArrayIndex = offset + largestIndexInHeap;
        int leftArrayIndex = offset + leftChildIndex;
        int rightArrayIndex = offset + rightChildIndex;

        if (leftChildIndex < heapSize
            && array[leftArrayIndex].compareTo(array[largestArrayIndex]) > 0) {
            largestIndexInHeap = leftChildIndex;
            largestArrayIndex = leftArrayIndex;
        }

        if (rightChildIndex < heapSize
            && array[rightArrayIndex].compareTo(array[largestArrayIndex]) > 0) {
            largestIndexInHeap = rightChildIndex;
            largestArrayIndex = rightArrayIndex;
        }

        if (largestIndexInHeap != index) {
            SortUtils.swap(array, offset + index, largestArrayIndex);
            heapify(array, largestIndexInHeap, heapSize, offset);
        }
    }
}