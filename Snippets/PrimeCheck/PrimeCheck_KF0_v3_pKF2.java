...
public final class PrimeCheck {
    ...
    public static void main(String[] args) {
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            printDeterministicResult(n);
            printProbabilisticResult(n, DEFAULT_FERMAT_ITERATIONS);
        }
    }

    private static void printDeterministicResult(int n) {
        if (isPrime(n)) {
    ...
    private static void printProbabilisticResult(int n, int iterations) {
        if (fermatPrimeChecking(n, iterations)) {
    ...
    /**
     * Deterministically checks if a number is prime using trial division.
     *
    ...
    /**
     * Probabilistically checks if a number is prime using Fermat's little theorem.
     *
     * @param n          the number to test
     * @param iterations the number of random bases to try; higher values reduce
