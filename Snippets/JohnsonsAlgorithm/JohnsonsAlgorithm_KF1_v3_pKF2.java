        ...
        for (int source = 0; source < vertexCount; source++) {
            for (int target = 0; target < vertexCount; target++) {
                if (adjacencyMatrix[source][target] != 0) {
                    reweighted[source][target] =
                            adjacencyMatrix[source][target] + potentials[source] - potentials[target];
                } else {
                    reweighted[source][target] = adjacencyMatrix[source][target];
                }
            }
        }
    ...
    public static double[] method5(double[][] reweightedGraph, int source, double[] potentials) {
        ...
        for (int i = 0; i < vertexCount - 1; i++) {
            ...
            for (int neighbor = 0; neighbor < vertexCount; neighbor++) {
                if (!visited[neighbor]
                        && reweightedGraph[currentVertex][neighbor] != 0
                        && distance[currentVertex] != INFINITY
                        && distance[currentVertex] + reweightedGraph[currentVertex][neighbor]
                                < distance[neighbor]) {
                    distance[neighbor] =
                            distance[currentVertex] + reweightedGraph[currentVertex][neighbor];
                }
