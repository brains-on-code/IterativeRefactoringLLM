...
public final class QuickSelect {
    ...
    private static <T extends Comparable<T>> int partitionAroundPivot(
        ...
        int smallerElementsEnd = leftIndex;

        for (int currentIndex = leftIndex; currentIndex < rightIndex; currentIndex++) {
            if (elements.get(currentIndex).compareTo(pivotValue) < 0) {
                Collections.swap(elements, smallerElementsEnd, currentIndex);
                smallerElementsEnd++;
            }
        }

        int equalElementsEnd = smallerElementsEnd;

        for (int currentIndex = smallerElementsEnd; currentIndex < rightIndex; currentIndex++) {
            if (elements.get(currentIndex).compareTo(pivotValue) == 0) {
                Collections.swap(elements, equalElementsEnd, currentIndex);
                equalElementsEnd++;
            }
        }

        Collections.swap(elements, rightIndex, equalElementsEnd);

        return (targetIndex < smallerElementsEnd)
                ? smallerElementsEnd
                : Math.min(targetIndex, equalElementsEnd);
    }
    ...
    private static <T extends Comparable<T>> int choosePivotIndex(List<T> elements, int leftIndex, int rightIndex) {
        ...
        for (int groupStart = leftIndex; groupStart < rightIndex; groupStart += 5) {
            int groupEnd = Math.min(groupStart + 4, rightIndex);
            int medianIndex = medianOfFive(elements, groupStart, groupEnd);
            int medianTargetIndex = leftIndex + (groupStart - leftIndex) / 5;
