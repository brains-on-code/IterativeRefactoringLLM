...
public final class KnightsTour {
    ...
    /** Board dimension including 2-cell-wide invalid border. */
    ...
    /** Board representation; -1 = invalid border, 0 = unvisited, >0 = move index. */
    static int[][] board;
     ...
     * Inner (BOARD_SIZE - 4) x (BOARD_SIZE - 4) region is valid.
     */
    ...
    static boolean solve(int row, int col, int move) {
        ...
        for (int[] neighbor : neighbors) {
            ...
            if (!orphanDetected(move, nextRow, nextCol) && solve(nextRow, nextCol, move + 1)) {
                return true;
            }
    ...
    static boolean orphanDetected(int move, int row, int col) {
        ...
        List<int[]> neighbors = getNeighbors(row, col);
        for (int[] neighbor : neighbors) {
            if (countUnvisitedNeighbors(neighbor[0], neighbor[1]) == 0) {
                return true;
            }
        }
