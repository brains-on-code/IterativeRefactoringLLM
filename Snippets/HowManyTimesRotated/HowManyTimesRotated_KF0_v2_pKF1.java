...
final class HowManyTimesRotated {
    ...
    public static int countRotations(int[] values) {
        ...
        while (left <= right) {
            ...
            if (values[pivotIndex] < values[pivotIndex - 1] && values[pivotIndex] < values[pivotIndex + 1]) {
                break;
            } else if (values[pivotIndex] > values[pivotIndex - 1] && values[pivotIndex] < values[pivotIndex + 1]) {
                right = pivotIndex + 1;
            } else if (values[pivotIndex] > values[pivotIndex - 1] && values[pivotIndex] > values[pivotIndex + 1]) {
                left = pivotIndex - 1;
            }
