...
public final class BinaryToHexadecimal {
    ...
    public static String binToHex(int binaryInput) {
        ...
        int remainingBinaryValue = binaryInput;

        while (remainingBinaryValue != 0) {
            int nibbleDecimalValue = 0;
            for (int bitPosition = 0; bitPosition < BITS_PER_HEX_DIGIT; bitPosition++) {
                int currentBinaryDigit = remainingBinaryValue % DECIMAL_BASE;
                if (currentBinaryDigit > 1) {
                    throw new IllegalArgumentException("Incorrect binary digit: " + currentBinaryDigit);
                }
                remainingBinaryValue /= DECIMAL_BASE;
                nibbleDecimalValue += (int) (currentBinaryDigit * Math.pow(BINARY_BASE, bitPosition));
            }
            hexStringBuilder.insert(0, decimalToHexDigitMap.get(nibbleDecimalValue));
        }

        return !hexStringBuilder.isEmpty() ? hexStringBuilder.toString() : "0";
    }
