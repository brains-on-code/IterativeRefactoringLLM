...
public final class SelectionUtils {
    ...
    private static <T extends Comparable<T>> int quickSelectWithMedianOfMedians(List<T> elements, int index) {
        return quickSelectWithMedianOfMedians(elements, 0, elements.size() - 1, index);
    }
    ...
    private static <T extends Comparable<T>> int choosePivotIndexMedianOfMedians(
        ...
        if (right - left < 5) {
            return medianIndexOfRange(elements, left, right);
        }
        ...
        for (int i = left; i < right; i += 5) {
            ...
            int medianIndex = medianIndexOfRange(elements, i, groupRight);
        ...
        return quickSelectWithMedianOfMedians(elements, left, mediansRight, medianOfMediansIndex);
    }

    private static <T extends Comparable<T>> int medianIndexOfRange(List<T> elements, int left, int right) {
