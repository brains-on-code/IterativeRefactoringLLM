...
public final class KochSnowflake {
    ...
    private static final double KOCH_ANGLE_DEGREES = 60.0;
    ...
    public static void main(String[] args) {
        testIterationOnUnitSegment();
        testKochSnowflakeImage();
    }

    private static void testIterationOnUnitSegment() {
        ArrayList<Vector2> initial = new ArrayList<>();
        initial.add(new Vector2(0, 0));
        initial.add(new Vector2(1, 0));

        ArrayList<Vector2> result = iterate(initial, 1);
    ...
    private static void testKochSnowflakeImage() {
        int imageWidth = 600;
    ...
    private static ArrayList<Vector2> iterationStep(List<Vector2> vectors) {
        ...
        for (int i = 0; i < vectors.size() - 1; i++) {
            ...
            Vector2 segmentThird = end.subtract(start).multiply(ONE_THIRD);
            Vector2 oneThird = start.add(segmentThird);
            Vector2 twoThirds = start.add(segmentThird.multiply(2.0));
            Vector2 peak = oneThird.add(segmentThird.rotate(KOCH_ANGLE_DEGREES));
    ...
    /**
     * Simple immutable 2D vector with basic operations.
     */
