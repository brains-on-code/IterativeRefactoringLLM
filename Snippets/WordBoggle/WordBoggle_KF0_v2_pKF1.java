...
public final class WordBoggle {
    ...
    public static List<String> boggleBoard(char[][] board, String[] words) {
        ...
        for (int rowIndex = 0; rowIndex < board.length; rowIndex++) {
            for (int colIndex = 0; colIndex < board[rowIndex].length; colIndex++) {
                exploreBoard(rowIndex, colIndex, board, trie.root, visitedCells, foundWords);
            }
        }
    ...
    public static void exploreBoard(
        ...
        char currentCharacter = board[rowIndex][colIndex];
        if (!currentTrieNode.children.containsKey(currentCharacter)) {
            return;
        }

        visitedCells[rowIndex][colIndex] = true;
        TrieNode nextTrieNode = currentTrieNode.children.get(currentCharacter);
        ...
        for (int[] neighborCoordinate : neighborCoordinates) {
            ...
            exploreBoard(neighborRowIndex, neighborColIndex, board, nextTrieNode, visitedCells, foundWords);
        }
...
class Trie {
    ...
    public void add(String wordToAdd) {
        ...
        for (int charIndex = 0; charIndex < wordToAdd.length(); charIndex++) {
            char currentCharacter = wordToAdd.charAt(charIndex);
            if (!currentTrieNode.children.containsKey(currentCharacter)) {
                TrieNode newTrieNode = new TrieNode();
                currentTrieNode.children.put(currentCharacter, newTrieNode);
            }
            currentTrieNode = currentTrieNode.children.get(currentCharacter);
        }
        currentTrieNode.children.put(this.endSymbol, null);
        currentTrieNode.word = wordToAdd;
    }
}