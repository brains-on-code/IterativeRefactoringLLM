...
class GCounter {
    ...
    GCounter(int replicaId, int replicaCount) {
        ...
        initializeReplicaCounters();
    }

    /** Initializes all replica counters to zero. */
    private void initializeReplicaCounters() {
    ...
    public boolean compare(GCounter other) {
        for (int id = 0; id < replicaCount; id++) {
            if (isReplicaCounterGreaterThan(other, id)) {
                return false;
            }
        }
        return true;
    ...
    public void merge(GCounter other) {
        for (int id = 0; id < replicaCount; id++) {
            replicaCounters.put(id, mergedReplicaValue(other, id));
        }
    }

    /**
     * Returns {@code true} if this counter's value for the given replica ID
     * is greater than the corresponding value in {@code other}.
     */
    private boolean isReplicaCounterGreaterThan(GCounter other, int id) {
        return replicaCounters.get(id) > other.replicaCounters.get(id);
    }

    /**
     * Returns the merged value for a given replica ID between this and another counter.
     * The merged value is the maximum of the two per-replica values.
     */
    private int mergedReplicaValue(GCounter other, int id) {
        return Math.max(replicaCounters.get(id), other.replicaCounters.get(id));
    }
}