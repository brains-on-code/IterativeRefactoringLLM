...
/**
 * Knight's Tour on a 12x12 board with an 8x8 inner playable area.
 *
 * The outer 2-cell-wide border is marked as invalid (-1). The algorithm
 * searches for a tour within the inner cells using Warnsdorff-like
...
public final class Class1 {
    ...
    private static int targetMoveCount;
    ...
    private static boolean isBorderCell(int row, int col) {
        return row < 2 || row > BOARD_SIZE - 3 || col < 2 || col > BOARD_SIZE - 3;
    }
    ...
    static boolean solveKnightTour(int row, int col, int move) {
        ...
        nextMoves.sort(Comparator.comparingInt(Move::onwardMoves));

        for (Move candidate : nextMoves) {
            int nextRow = candidate.row();
            int nextCol = candidate.col();
     ...
     * @param row current row
     * @param col current column
     * @return list of moves with onward move counts
     */
    static List<Move> getNextMoves(int row, int col) {
        ...
        for (int[] offset : KNIGHT_MOVES) {
            ...
            if (isValidUnvisitedCell(nextRow, nextCol)) {
                int onwardMoves = countOnwardMoves(nextRow, nextCol);
                moves.add(new Move(nextRow, nextCol, onwardMoves));
            }
        }
    ...
    private static boolean isValidUnvisitedCell(int row, int col) {
    ...
    static boolean createsDeadEnd(int move, int row, int col) {
        ...
        List<Move> nextMoves = getNextMoves(row, col);
        for (Move candidate : nextMoves) {
            if (countOnwardMoves(candidate.row(), candidate.col()) == 0) {
                return true;
            }
        }
    ...
    private record Move(int row, int col, int onwardMoves) {}
}