...
public final class CombinationGenerator {
    ...
    public static <T> List<TreeSet<T>> generateCombinations(T[] inputArray, int combinationLength) {
        ...
        buildCombinations(sortedInput, combinationLength, 0, new TreeSet<>(), combinations);
        return combinations;
    }
    ...
    private static <T> void buildCombinations(
        ...
        for (int index = startIndex; index < sortedInput.length; index++) {
            T element = sortedInput[index];
            currentCombination.add(element);
            buildCombinations(sortedInput, combinationLength, index + 1, currentCombination, allCombinations);
            currentCombination.remove(element);
        }
    }
}