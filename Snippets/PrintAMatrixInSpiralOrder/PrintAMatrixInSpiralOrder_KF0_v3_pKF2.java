...
public class PrintAMatrixInSpiralOrder {
    ...
    public List<Integer> print(int[][] matrix, int rows, int cols) {
        ...
        while (topRow <= bottomRow && leftCol <= rightCol) {
            traverseTopRow(matrix, result, topRow, leftCol, rightCol);
            topRow++;

            traverseRightColumn(matrix, result, rightCol, topRow, bottomRow);
            rightCol--;

            if (topRow <= bottomRow) {
                traverseBottomRow(matrix, result, bottomRow, leftCol, rightCol);
                bottomRow--;
            }

            if (leftCol <= rightCol) {
                traverseLeftColumn(matrix, result, leftCol, topRow, bottomRow);
                leftCol++;
            }
        }
    ...
    private void validateInput(int[][] matrix, int rows, int cols) {
        ...
        for (int i = 0; i < rows; i++) {
            if (matrix[i] == null || matrix[i].length < cols) {
                throw new IllegalArgumentException("Matrix row " + i + " has fewer columns than specified.");
            }
        }
    }
