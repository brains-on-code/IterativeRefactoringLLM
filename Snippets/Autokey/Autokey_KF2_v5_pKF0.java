...
public class Autokey {
    ...
    private static final String NON_ALPHABET_REGEX = "[^A-Z]";

    public String encrypt(String plaintext, String keyword) {
        String normalizedPlaintext = normalizeText(plaintext);
        String normalizedKeyword = normalizeText(keyword);

        StringBuilder extendedKey = buildExtendedKeyForEncryption(
            normalizedPlaintext,
            normalizedKeyword
        );
        ...
        for (int i = 0; i < normalizedPlaintext.length(); i++) {
            ...
            ciphertext.append(shiftChar(plainChar, keyChar));
        }
    ...
    private StringBuilder buildExtendedKeyForEncryption(
        String normalizedPlaintext,
        String normalizedKeyword
    ) {
        return new StringBuilder(normalizedKeyword).append(normalizedPlaintext);
    }
    ...
    private String normalizeText(String text) {
        ...
        return text.toUpperCase().replaceAll(NON_ALPHABET_REGEX, "");
    }

    private char shiftChar(char plainChar, char keyChar) {
        int shiftedValue = (plainChar - BASE_CHAR + keyChar - BASE_CHAR) % ALPHABET_SIZE;
        return (char) (shiftedValue + BASE_CHAR);
    }

    private char unshiftChar(char cipherChar, char keyChar) {
        int shiftedValue =
            (cipherChar - BASE_CHAR - (keyChar - BASE_CHAR) + ALPHABET_SIZE) % ALPHABET_SIZE;
        return (char) (shiftedValue + BASE_CHAR);
    }
}