...
public class Class1 {
    ...
    int knapsackRecursive(int capacity, int[] weights, int[] values, int itemIndex, int[][] memo) {
        ...
        int lastItemIndex = itemIndex - 1;
        int lastItemWeight = weights[lastItemIndex];
        int lastItemValue = values[lastItemIndex];
        ...
        int includeLastItem =
            lastItemValue
                + knapsackRecursive(
                    capacity - lastItemWeight, weights, values, itemIndex - 1, memo);

        int excludeLastItem =
            knapsackRecursive(capacity, weights, values, itemIndex - 1, memo);

        memo[itemIndex][capacity] = Math.max(includeLastItem, excludeLastItem);
        return memo[itemIndex][capacity];
    }
}