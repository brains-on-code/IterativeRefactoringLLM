...
final class Sudoku {
     ...
     * is valid according to Sudoku rules for the given board.
     *
    ...
    public static boolean isSafe(int[][] board, int row, int col, int num) {
        ...
        return !isInRow(board, row, num, size)
            ...
            && !isInSubgrid(board, row, col, num, size);
    }
    ...
    private static boolean isInSubgrid(int[][] board, int row, int col, int num, int size) {
        int subgridSize = (int) Math.sqrt(size);
        int boxRowStart = row - row % subgridSize;
        int boxColStart = col - col % subgridSize;

        for (int r = boxRowStart; r < boxRowStart + subgridSize; r++) {
            for (int c = boxColStart; c < boxColStart + subgridSize; c++) {
                if (board[r][c] == num) {
                    return true;
                }
            }
    ...
    /**
     * Returns the next empty cell (with value 0) in the board.
     *
     * @param board the Sudoku board
     * @param n     the size of the board (n x n)
