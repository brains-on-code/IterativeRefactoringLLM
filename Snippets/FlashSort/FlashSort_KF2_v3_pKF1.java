...
public class FlashSort implements SortAlgorithm {
    ...
    private <T extends Comparable<? super T>> void flashSort(T[] array) {
        ...
        countElementsPerBucket(array, bucketCounts, bucketScale, minValue);
        accumulateBucketCounts(bucketCounts);
        permuteIntoBuckets(array, bucketCounts, bucketScale, minValue, array.length, bucketCount);
        insertionSort(array);
    }
    ...
    private <T extends Comparable<? super T>> void permuteIntoBuckets(
        ...
        while (movedElementCount < arrayLength - 1) {
            ...
            while (currentIndex != bucketCounts[currentBucketIndex]) {
                currentBucketIndex = (int) (bucketScale * currentValue.compareTo(minValue));
