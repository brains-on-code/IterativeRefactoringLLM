...
/**
 * Utility class for computing the Levenshtein (edit) distance between two strings.
 *
...
public final class LevenshteinDistance {
     ...
     * where m = {@code first.length()}, n = {@code second.length()}.
     *
    ...
    public static int computeIterative(String first, String second) {
        ...
        // Base case: distance from prefix of first to empty second (all deletions).
        for (int i = 0; i <= firstLength; i++) {
            dp[i][0] = i;
        }

        // Base case: distance from empty first to prefix of second (all insertions).
    ...
    /**
     * Recursive helper for computing the Levenshtein distance with memoization.
     *
     * <p>{@code memo[i][j]} stores the distance between
     * {@code first[0..i-1]} and {@code second[0..j-1]}.
     *
     * @param first  the first string
     * @param second the second string
    ...
    public static int computeRecursive(String first, String second, int[][] memo) {
        ...
        // If one string is empty, distance is the length of the other (all insertions/deletions).
        if (firstLength == 0) {
        ...
        if (first.charAt(0) == second.charAt(0)) {
            // First characters match: move to the next characters in both strings.
