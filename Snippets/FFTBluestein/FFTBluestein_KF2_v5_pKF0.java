...
public final class FFTBluestein {
    ...
    public static void fftBluestein(List<FFT.Complex> input, boolean inverse) {
        ...
        final int convolutionSize = 2 * n - 1;
        final int direction = inverse ? -1 : 1;
        final double baseAngle = Math.PI * direction / n;

        List<FFT.Complex> modulatedInput = new ArrayList<>(n);
        List<FFT.Complex> chirpSequence = createZeroInitializedList(convolutionSize);
    ...
    private static List<FFT.Complex> createZeroInitializedList(int size) {
    ...
    private static void buildModulatedInput(
        ...
        for (int i = 0; i < n; i++) {
            ...
            FFT.Complex chirp = complexFromAngle(angle);
            FFT.Complex modulatedValue = input.get(i).multiply(chirp);
            modulatedInput.add(modulatedValue);
        }
    }
    ...
    private static void applyPostProcessing(
        ...
        for (int i = 0; i < n; i++) {
            double angle = -i * i * baseAngle;
            FFT.Complex chirp = complexFromAngle(angle);
            FFT.Complex value = chirp.multiply(convolution.get(i + n - 1));
            output.set(i, value);
        }
    }
    ...
    private static FFT.Complex complexFromAngle(double angle) {
        ...
        return new FFT.Complex(real, imaginary);
    }
}