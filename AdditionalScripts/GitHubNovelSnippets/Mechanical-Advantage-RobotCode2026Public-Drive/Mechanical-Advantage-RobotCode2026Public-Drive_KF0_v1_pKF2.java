...
public class Drive extends FullSubsystem {
  /** Lock to prevent odometry updates while reading sensor data. */
  ...
  public void periodic() {
    // Read sensors and module inputs under lock to keep odometry consistent
    odometryLock.lock();
    try {
    ...
    // When disabled, log empty setpoints so old values don't persist
    if (DriverStation.isDisabled()) {
      Logger.recordOutput("SwerveStates/Setpoints", new SwerveModuleState[] {});
      Logger.recordOutput("SwerveStates/SetpointsOptimized", new SwerveModuleState[] {});
    }

    // Update odometry and robot velocity
    ...
    // Hardware alert for gyro connection status
    gyroDisconnectedAlert.set(Robot.showHardwareAlerts() && !gyroInputs.connected);

    // Update brake / coast mode based on motion and enable state
  ...
  public void runVelocity(ChassisSpeeds speeds) {
    // Discretize for the current loop period and compute module states
    ...
    // Log unoptimized setpoints and chassis speeds
    Logger.recordOutput("SwerveStates/Setpoints", setpointStates);
    Logger.recordOutput("SwerveChassisSpeeds/Setpoints", discreteSpeeds);

    // Apply setpoints to modules (runSetpoint may mutate the states)
    for (int i = 0; i < modules.length; i++) {
      modules[i].runSetpoint(setpointStates[i]);
    }

    // Log optimized setpoints after module optimization
  ...
  /**
   * Stop the drive and point the modules in an "X" to passively resist motion. Modules will return
   * to normal orientation on the next nonzero velocity command.
   */
  ...
  /** @return current module states (turn angles and drive velocities) for all modules. */
  ...
  /** @return current module positions (turn angles and drive positions) for all modules. */
  ...
  /** @return measured chassis speeds of the robot. */
  @AutoLogOutput(key = "SwerveChassisSpeeds/Measured")
  private ChassisSpeeds getChassisSpeeds() {
    return kinematics.toChassisSpeeds(getModuleStates());
  }

  /** @return wheel positions used for wheel radius characterization, in radians. */
