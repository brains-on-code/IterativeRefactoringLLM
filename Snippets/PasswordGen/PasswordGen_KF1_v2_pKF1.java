...
final class RandomStringGenerator {
    ...
    public static String generateRandomString(int minLength, int maxLength) {
        ...
        StringBuilder randomStringBuilder = new StringBuilder(targetLength);
        int characterSetLength = ALL_CHARACTERS.length();

        for (int i = 0; i < targetLength; i++) {
            int randomIndex = random.nextInt(characterSetLength);
            randomStringBuilder.append(ALL_CHARACTERS.charAt(randomIndex));
        }
