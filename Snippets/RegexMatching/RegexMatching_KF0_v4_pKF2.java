...
public final class RegexMatching {
    ...
    public static boolean regexRecursion(String src, String pat) {
        ...
        if (src.length() == 0) {
            // Source is empty: pattern must be composed only of '*'
        ...
        if (patChar == '*') {
            // '*' can match:
            // - empty sequence: advance pattern only
            // - one or more characters: advance source only
    ...
    static boolean regexRecursion(String src, String pat, int srcIdx, int patIdx) {
        ...
        if (srcIdx == src.length()) {
            // Source exhausted: remaining pattern must be composed only of '*'
