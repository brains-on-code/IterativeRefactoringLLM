...
public final class NumberToWords {
    ...
    public static String convert(BigDecimal number) {
        ...
        String fractionDigits = extractFractionDigits(integerAndFraction[1]);
    ...
    /**
     * Extracts the fractional digits (after the decimal point) as a string.
     *
     * @param fractionPart the fractional part of the number
     * @return the fractional digits as a string, or an empty string if none
     */
    private static String extractFractionDigits(BigDecimal fractionPart) {
        if (fractionPart.compareTo(BigDecimal.ZERO) <= 0) {
            return "";
        }
        return fractionPart.toPlainString().substring(2);
    }
