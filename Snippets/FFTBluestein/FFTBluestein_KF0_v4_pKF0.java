...
public final class FFTBluestein {
    ...
    private static void initializeBnSequence(
        List<FFT.Complex> bn,
        int n,
        int bnSize,
        int direction
    ) {
        // Pre-fill list with placeholder complex numbers
    ...
    private static void initializeAnSequence(
        List<FFT.Complex> an,
        List<FFT.Complex> x,
        int n,
        int direction
    ...
    private static void applyFinalMultiplication(
        List<FFT.Complex> x,
        List<FFT.Complex> convolution,
        int n,
        int direction
    ) {
        for (int i = 0; i < n; i++) {
    ...
    private static double computeAngle(double factor, int n, int direction) {
        return factor * Math.PI * direction / n;
    }
