...
public final class MidpointEllipse {
    ...
    private static void computeEllipsePoints(
        ...
        while (deltaX < deltaY) {
            ...
            if (region1Decision < 0) {
                currentX++;
                deltaX += 2 * radiusYSquared;
                region1Decision += deltaX + radiusYSquared;
            } else {
                currentX++;
                currentY--;
                deltaX += 2 * radiusYSquared;
                deltaY -= 2 * radiusXSquared;
                region1Decision += deltaX - deltaY + radiusYSquared;
            }
        }

        // Region 2: Initial decision parameter for the second region
        double region2Decision =
        ...
        while (currentY >= 0) {
            ...
            if (region2Decision > 0) {
                currentY--;
                deltaY -= 2 * radiusXSquared;
                region2Decision += radiusXSquared - deltaY;
            } else {
                currentY--;
                currentX++;
                deltaX += 2 * radiusYSquared;
                deltaY -= 2 * radiusXSquared;
                region2Decision += deltaX - deltaY + radiusXSquared;
            }
        }
    }
