package com.thealgorithms.dynamicprogramming;

/**
 * Dynamic programming solution for the Egg Dropping Puzzle.
 */
public final class EggDropping {

    private EggDropping() {
        // Utility class
    }

    /**
     * Computes the minimum number of trials needed in the worst case to determine
     * the highest floor from which an egg can be dropped without breaking.
     *
     * @param eggCount   number of eggs
     * @param floorCount number of floors
     * @return minimum number of trials in the worst case
     */
    public static int minTrials(int eggCount, int floorCount) {
        int[][] minTrialsTable = new int[eggCount + 1][floorCount + 1];

        // Base cases:
        // 0 floors -> 0 trials, 1 floor -> 1 trial
        for (int eggs = 1; eggs <= eggCount; eggs++) {
            minTrialsTable[eggs][0] = 0;
            minTrialsTable[eggs][1] = 1;
        }

        // With 1 egg, trials equal number of floors
        for (int floors = 1; floors <= floorCount; floors++) {
            minTrialsTable[1][floors] = floors;
        }

        // Bottom-up DP computation
        for (int eggs = 2; eggs <= eggCount; eggs++) {
            for (int floors = 2; floors <= floorCount; floors++) {
                minTrialsTable[eggs][floors] = Integer.MAX_VALUE;

                for (int currentFloor = 1; currentFloor <= floors; currentFloor++) {
                    int trialsIfEggBreaks = minTrialsTable[eggs - 1][currentFloor - 1];
                    int trialsIfEggSurvives = minTrialsTable[eggs][floors - currentFloor];
                    int worstCaseTrials = 1 + Math.max(trialsIfEggBreaks, trialsIfEggSurvives);

                    if (worstCaseTrials < minTrialsTable[eggs][floors]) {
                        minTrialsTable[eggs][floors] = worstCaseTrials;
                    }
                }
            }
        }

        return minTrialsTable[eggCount][floorCount];
    }

    public static void main(String[] args) {
        int eggCount = 2;
        int floorCount = 4;
        int minimumTrials = minTrials(eggCount, floorCount);
        System.out.println(minimumTrials);
    }
}