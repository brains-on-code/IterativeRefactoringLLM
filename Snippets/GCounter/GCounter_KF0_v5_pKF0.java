...
class GCounter {
    ...
    GCounter(int nodeId, int nodeCount) {
        ...
        this.counters = new HashMap<>(nodeCount);
        initializeCounters();
    }
    ...
    public boolean compare(GCounter other) {
        for (int nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++) {
            if (getValueForNode(nodeIndex) > other.getValueForNode(nodeIndex)) {
                return false;
            }
        }
        return true;
    ...
    public void merge(GCounter other) {
        for (int nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++) {
            int mergedValue = Math.max(
                getValueForNode(nodeIndex),
                other.getValueForNode(nodeIndex)
            );
            counters.put(nodeIndex, mergedValue);
        }
    }

    private int getValueForNode(int nodeIndex) {
        return counters.getOrDefault(nodeIndex, 0);
    }
}