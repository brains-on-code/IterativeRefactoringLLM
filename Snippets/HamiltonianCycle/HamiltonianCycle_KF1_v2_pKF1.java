...
public class HamiltonianCycleFinder {
    ...
    public boolean searchHamiltonianCycle(int currentVertex) {
        boolean isCycleComplete =
                this.graph[currentVertex][0] == 1 && this.currentPathLength == this.vertexCount;
        if (isCycleComplete) {
            return true;
        }

        if (this.currentPathLength == this.vertexCount) {
            return false;
        }
    ...
    public boolean isVertexInCurrentPath(int vertex) {
        for (int index = 0; index < currentPathLength - 1; index++) {
            if (hamiltonianPath[index] == vertex) {
                return true;
            }
        }
        return false;
