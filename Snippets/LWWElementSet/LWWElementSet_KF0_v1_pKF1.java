...
class LwwElement {
    ...
    /**
     * Constructs a new LwwElement with the specified elementId, timestamp and bias.
     *
     ...
     */
    LwwElement(String elementId, int timestamp, LwwBias bias) {
...
class LWWElementSet {
    ...
    public void add(LwwElement element) {
        addSet.put(element.elementId, element);
    }
    ...
    public void remove(LwwElement element) {
        if (lookup(element)) {
            removeSet.put(element.elementId, element);
        }
    }
    ...
    public boolean lookup(LwwElement element) {
        LwwElement addedElement = addSet.get(element.elementId);
        LwwElement removedElement = removeSet.get(element.elementId);
    ...
    public void merge(LWWElementSet otherSet) {
        for (LwwElement otherAddedElement : otherSet.addSet.values()) {
            LwwElement currentAddedElement = addSet.get(otherAddedElement.elementId);
            if (currentAddedElement == null || isNewerOrBiased(otherAddedElement, currentAddedElement)) {
                addSet.put(otherAddedElement.elementId, otherAddedElement);
            }
        }

        for (LwwElement otherRemovedElement : otherSet.removeSet.values()) {
            LwwElement currentRemovedElement = removeSet.get(otherRemovedElement.elementId);
            if (currentRemovedElement == null || isNewerOrBiased(otherRemovedElement, currentRemovedElement)) {
                removeSet.put(otherRemovedElement.elementId, otherRemovedElement);
            }
        }
    }
