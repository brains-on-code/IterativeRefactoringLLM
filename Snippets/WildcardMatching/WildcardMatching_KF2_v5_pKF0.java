...
public final class WildcardMatching {
    ...
    private static void initializeFirstRow(String pattern, boolean[][] dp) {
        ...
        for (int patternIndex = 1; patternIndex <= pattern.length(); patternIndex++) {
            boolean isStar = pattern.charAt(patternIndex - 1) == '*';
            dp[0][patternIndex] = isStar && dp[0][patternIndex - 1];
        }
    }
        ...
        for (int textIndex = 1; textIndex <= textLength; textIndex++) {
            ...
            for (int patternIndex = 1; patternIndex <= patternLength; patternIndex++) {
                ...
                if (isDirectMatch(textChar, patternChar)) {
                    ...
                    boolean matchWithStarAsMultiple = dp[textIndex - 1][patternIndex];
                    boolean matchWithStarAsEmpty = dp[textIndex][patternIndex - 1];
                    dp[textIndex][patternIndex] = matchWithStarAsMultiple || matchWithStarAsEmpty;
                } else {
