    ...
    int computeMaxValue(
        ...
        if (currentItemWeight > remainingCapacity) {
            memoTable[currentItemIndex][remainingCapacity] =
                computeMaxValue(remainingCapacity, weights, values, currentItemIndex - 1, memoTable);
        } else {
            int valueIfIncluded =
                currentItemValue
                    + computeMaxValue(
            ...
            memoTable[currentItemIndex][remainingCapacity] = Math.max(valueIfIncluded, valueIfExcluded);
        }
