    ...
    private static class BoruvkaState {
        ...
        void merge(final Edge[] cheapest) {
            for (int vertex = 0; vertex < graph.vertexCount; vertex++) {
                final Edge edge = cheapest[vertex];
                if (edge == null) {
                    continue;
                }

                final int componentOfSrc = find(components, edge.src);
                final int componentOfDest = find(components, edge.dest);

                if (componentOfSrc == componentOfDest) {
                    continue;
                }

                result.add(edge);
                union(components, componentOfSrc, componentOfDest);
            }
        }
        ...
        Edge[] computeCheapestEdges() {
            ...
            for (final Edge edge : graph.edges) {
                final int setOfSrc = find(components, edge.src);
                final int setOfDest = find(components, edge.dest);

                if (setOfSrc == setOfDest) {
                    continue;
                }

                updateCheapestEdge(cheapest, setOfSrc, edge);
                updateCheapestEdge(cheapest, setOfDest, edge);
            }

            return cheapest;
        }

        private static void updateCheapestEdge(final Edge[] cheapest, final int setIndex, final Edge candidateEdge) {
            final Edge currentCheapest = cheapest[setIndex];
            if (currentCheapest == null || candidateEdge.weight < currentCheapest.weight) {
                cheapest[setIndex] = candidateEdge;
            }
        }
