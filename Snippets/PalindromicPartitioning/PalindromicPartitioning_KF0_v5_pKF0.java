...
public final class PalindromicPartitioning {
    ...
    public static int minimalPartitions(String word) {
        ...
        final boolean[][] palindromeTable = buildPalindromeTable(word);
        final int[] minCuts = buildMinCutsTable(palindromeTable);
    ...
    private static boolean[][] buildPalindromeTable(String word) {
        final int length = word.length();
        final boolean[][] palindromeTable = new boolean[length][length];
        ...
        for (int subLen = 2; subLen <= length; subLen++) {
            for (int start = 0; start <= length - subLen; start++) {
                final int end = start + subLen - 1;
                final char startChar = word.charAt(start);
                final char endChar = word.charAt(end);
    ...
    private static int[] buildMinCutsTable(boolean[][] palindromeTable) {
        final int length = palindromeTable.length;
