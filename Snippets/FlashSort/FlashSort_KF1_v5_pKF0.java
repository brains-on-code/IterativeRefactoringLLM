...
public class ProxmapSort implements SortAlgorithm {
    ...
    private <T extends Comparable<? super T>> void rearrangeByBuckets(
        ...
        while (placedCount < length - 1) {
            currentIndex = advanceToValidIndex(array, bucketCounts, mappingFactor, min, currentIndex);
            ...
            while (currentIndex != bucketCounts[currentBucket]) {
                ...
                valueToPlace = swap(array, targetIndex, valueToPlace);
    ...
    private <T extends Comparable<? super T>> int advanceToValidIndex(
            final T[] array,
            final int[] bucketCounts,
            final double mappingFactor,
            final T min,
        ...
        int currentBucket = bucketIndexFor(array[currentIndex], min, mappingFactor);
    ...
    private <T> T swap(T[] array, int targetIndex, T newValue) {
        T oldValue = array[targetIndex];
