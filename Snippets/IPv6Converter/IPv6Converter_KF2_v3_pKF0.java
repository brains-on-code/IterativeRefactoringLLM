...
public final class IPv6Converter {
    ...
    private static final int IPV6_PREFIX_ZERO_BYTES = 10;
    ...
    private static byte[] createIpv6MappedFromIpv4(byte[] ipv4Bytes) {
        ...
        // First 10 bytes are zero by default in a new byte array
        ipv6Bytes[IPV6_PREFIX_ZERO_BYTES] = IPV6_MAPPED_PREFIX_BYTE;
        ipv6Bytes[IPV6_PREFIX_ZERO_BYTES + 1] = IPV6_MAPPED_PREFIX_BYTE;
    ...
    private static boolean isValidIpv6MappedIpv4(byte[] ipv6Bytes) {
        ...
        for (int i = 0; i < IPV6_PREFIX_ZERO_BYTES; i++) {
            if (ipv6Bytes[i] != 0) {
                return false;
            }
        }

        return ipv6Bytes[IPV6_PREFIX_ZERO_BYTES] == IPV6_MAPPED_PREFIX_BYTE
            && ipv6Bytes[IPV6_PREFIX_ZERO_BYTES + 1] == IPV6_MAPPED_PREFIX_BYTE;
    }
}