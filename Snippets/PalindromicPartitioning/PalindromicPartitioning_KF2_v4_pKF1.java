    ...
    public static int minimalPartitions(String input) {
        ...
        for (int endIndex = 0; endIndex < n; endIndex++) {
            if (palindromeTable[0][endIndex]) {
                ...
                int minCutsForCurrentEnd = Integer.MAX_VALUE;
                for (int partitionIndex = 0; partitionIndex < endIndex; partitionIndex++) {
                    if (palindromeTable[partitionIndex + 1][endIndex]
                        && 1 + minCutsUpToIndex[partitionIndex] < minCutsForCurrentEnd) {
                        minCutsForCurrentEnd = 1 + minCutsUpToIndex[partitionIndex];
                    }
                }
                minCutsUpToIndex[endIndex] = minCutsForCurrentEnd;
            }
        }
