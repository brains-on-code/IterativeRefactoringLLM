...
public final class BankersAlgorithm {
    ...
    static boolean isSystemInSafeState(
        ...
        while (completedProcesses < processCount) {
            ...
            for (int process = 0; process < processCount; process++) {
                ...
                if (canProcessBeSatisfied(need[process], work)) {
                    releaseAllocatedResources(work, allocation[process]);
                    safeSequence[completedProcesses++] = processIds[process];
    ...
    private static boolean canProcessBeSatisfied(int[] needRow, int[] work) {
    ...
    private static void releaseAllocatedResources(int[] work, int[] allocationRow) {
        for (int resource = 0; resource < work.length; resource++) {
    ...
    private static int[][] readMatrix(
        ...
        boolean isMaximumMatrix = "maximum".equals(matrixName);

        for (int process = 0; process < processCount; process++) {
            System.out.println("For process " + process + ": ");
            for (int resource = 0; resource < resourceCount; resource++) {
                String prompt = isMaximumMatrix
