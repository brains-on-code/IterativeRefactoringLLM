...
public class MLFQScheduler {
    ...
    private final List<Queue<Process>> readyQueues;
    ...
    public MLFQScheduler(int levels, int[] timeQuantums) {
        this.readyQueues = new ArrayList<>(levels);
    ...
    public void run() {
        while (!allQueuesEmpty()) {
            for (int level = 0; level < readyQueues.size(); level++) {
                Queue<Process> queue = readyQueues.get(level);
                if (queue.isEmpty()) {
                    continue;
                }
                ...
                if (process.isFinished()) {
                    ...
                    boolean hasLowerLevel = level < readyQueues.size() - 1;
                    if (hasLowerLevel) {
    ...
    private boolean allQueuesEmpty() {
        for (Queue<Process> queue : readyQueues) {
            if (!queue.isEmpty()) {
                return false;
            }
        }
...
class Process {
    ...
    /** Increments the priority level (numerically higher means lower priority). */
