...
public final class MatrixInverter {
    ...
    public static double[][] invert(double[][] matrix) {
        ...
        applyForwardSubstitution(matrix, identity, pivotIndices);
        applyBackwardSubstitution(matrix, identity, pivotIndices, inverse);
    ...
    private static void applyForwardSubstitution(double[][] luMatrix, double[][] rhs, int[] pivotIndices) {
    ...
    private static void luDecompose(double[][] matrix, int[] pivotIndices) {
        ...
        performLuDecomposition(matrix, pivotIndices, scalingFactors);
    }
    ...
    private static void computeScalingFactors(double[][] matrix, double[] scalingFactors) {
        ...
        for (int row = 0; row < size; row++) {
            ...
            scalingFactors[row] = maxAbs;
        }
    }

    private static void performLuDecomposition(
        double[][] matrix,
        int[] pivotIndices,
    ...
    private static void swapPivotIndices(int[] pivotIndices, int firstIndex, int secondIndex) {
        int temp = pivotIndices[firstIndex];
        pivotIndices[firstIndex] = pivotIndices[secondIndex];
        pivotIndices[secondIndex] = temp;
    }
