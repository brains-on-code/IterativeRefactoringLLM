...
class PNCounter {
    ...
    PNCounter(int nodeId, int nodeCount) {
        validateNodeConfiguration(nodeId, nodeCount);
    ...
    private static void validateNodeConfiguration(int nodeId, int nodeCount) {
        if (nodeCount <= 0) {
    ...
    public void increment() {
        increments.computeIfPresent(nodeId, (id, value) -> value + 1);
    }

    /** Decrements the counter for this node. */
    public void decrement() {
        decrements.computeIfPresent(nodeId, (id, value) -> value + 1);
    }

    /** Returns the current value of the PN-Counter. */
    public int getValue() {
        return sumValues(increments) - sumValues(decrements);
    }
    ...
    public boolean isLessThanOrEqual(PNCounter other) {
        ...
        for (int i = 0; i < nodeCount; i++) {
            int thisIncrements = increments.get(i);
            int otherIncrements = other.increments.get(i);
            int thisDecrements = decrements.get(i);
            int otherDecrements = other.decrements.get(i);

            if (thisIncrements > otherIncrements && thisDecrements > otherDecrements) {
                return false;
            }
        }
        return true;
    ...
    public void merge(PNCounter other) {
        ...
        for (int i = 0; i < nodeCount; i++) {
            increments.computeIfPresent(i, (id, value) -> Math.max(value, other.increments.get(id)));
            decrements.computeIfPresent(i, (id, value) -> Math.max(value, other.decrements.get(id)));
        }
    }
