...
/**
 * Utility class for running Dijkstra's algorithm on a weighted graph
...
public class DijkstraAdjacencyMatrix {
     ...
     */
    public int[] computeShortestPaths(int[][] adjacencyMatrix, int source) {
    ...
    private void validateAdjacencyMatrix(int[][] adjacencyMatrix) {
        if (adjacencyMatrix == null || adjacencyMatrix.length != vertexCount) {
            throw new IllegalArgumentException(
                "Adjacency matrix must be non-null and of size " + vertexCount
            );
        }
        for (int[] row : adjacencyMatrix) {
            if (row == null || row.length != vertexCount) {
                throw new IllegalArgumentException(
                    "Adjacency matrix must be square with size " + vertexCount
                );
            }
        }
    }
    ...
    private void relaxNeighbors(
        ...
        for (int neighbor = 0; neighbor < vertexCount; neighbor++) {
            ...
            if (newDistance < distances[neighbor]) {
                Pair<Integer, Integer> oldEntry = Pair.of(distances[neighbor], neighbor);
