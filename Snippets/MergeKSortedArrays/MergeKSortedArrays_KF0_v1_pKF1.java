...
public final class MergeKSortedArrays {
    ...
    public static int[] mergeKArrays(int[][] arrays) {
        PriorityQueue<int[]> minHeap =
            new PriorityQueue<>(Comparator.comparingInt(elementInfo -> elementInfo[0]));

        int totalElementCount = 0;
        for (int arrayIndex = 0; arrayIndex < arrays.length; arrayIndex++) {
            if (arrays[arrayIndex].length > 0) {
                int firstValue = arrays[arrayIndex][0];
                int elementIndex = 0;
                minHeap.offer(new int[] {firstValue, arrayIndex, elementIndex});
                totalElementCount += arrays[arrayIndex].length;
            }
        }
        ...
        while (!minHeap.isEmpty()) {
            int[] elementInfo = minHeap.poll();
            int value = elementInfo[0];
            int arrayIndex = elementInfo[1];
            int elementIndex = elementInfo[2];
