...
public final class KnightsTour {
    ...
    static boolean solve(int row, int column, int moveNumber) {
        ...
        if (neighbors.isEmpty() && moveNumber != total) {
            return false;
        }
        ...
        for (int[] move : neighbors) {
            ...
            if (!orphanDetected(moveNumber, nextRow, nextCol) && solve(nextRow, nextCol, moveNumber + 1)) {
                return true;
            }
    ...
    static boolean orphanDetected(int moveNumber, int row, int column) {
        if (moveNumber >= total - 1) {
            return false;
        }

        List<int[]> neighbors = getNeighbors(row, column);
        for (int[] move : neighbors) {
            if (countNeighbors(move[0], move[1]) == 0) {
                return true;
            }
        }
