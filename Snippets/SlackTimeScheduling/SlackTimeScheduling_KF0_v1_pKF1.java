...
public class SlackTimeScheduling {
    ...
    public void addTask(String taskName, int executionTime, int deadline) {
        scheduledTasks.add(new Task(taskName, executionTime, deadline));
    }
    ...
    public List<String> scheduleTasks() {
        scheduledTasks.sort(Comparator.comparingInt(Task::calculateSlackTime));
        List<String> executionOrder = new ArrayList<>();
        for (Task task : scheduledTasks) {
            executionOrder.add(task.getTaskName());
        }
