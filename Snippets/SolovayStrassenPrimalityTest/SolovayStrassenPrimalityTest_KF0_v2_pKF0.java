...
final class SolovayStrassenPrimalityTest {
    ...
    private static long calculateModularExponentiation(long base, long exponent, long mod) {
        ...
        while (exponent > 0) {
            boolean isExponentOdd = (exponent & 1L) == 1L;
            if (isExponentOdd) {
    ...
    public int calculateJacobi(long a, long n) {
        ...
        while (a != 0) {
            while ((a & 1L) == 0L) {
                a >>= 1;
                long nMod8 = n & 7L;
                boolean flipSign = (nMod8 == 3L || nMod8 == 5L);
                if (flipSign) {
                    jacobi = -jacobi;
                }
            }
            ...
            boolean bothCongruentTo3Mod4 = ((a & 3L) == 3L) && ((n & 3L) == 3L);
            if (bothCongruentTo3Mod4) {
                jacobi = -jacobi;
            }
    ...
    public boolean solovayStrassen(long num, int iterations) {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            boolean isMismatch = (modExpResult != jacobi);

            if (isJacobiZero || isMismatch) {
                return false;
            }
        }
