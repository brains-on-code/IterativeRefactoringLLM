...
final class SolovayStrassenPrimalityTest {
    ...
    public int calculateJacobi(long a, long n) {
        ...
        while (currentA != 0) {
            jacobi = extractFactorsOfTwo(currentN, jacobi);
            currentA = removeFactorsOfTwo(currentA, currentN, jacobi);

            long temp = currentA;
            currentA = currentN;
            currentN = temp;

            if (bothCongruentToThreeModuloFour(currentA, currentN)) {
                jacobi = -jacobi;
            }
    ...
    private int extractFactorsOfTwo(long n, int jacobi) {
        long nMod8 = n & 7;
        if (nMod8 == 3 || nMod8 == 5) {
            return -jacobi;
        }
    ...
    private long removeFactorsOfTwo(long a, long n, int jacobi) {
        long currentA = a;
        while ((currentA & 1) == 0) {
            currentA >>= 1;
            jacobi = extractFactorsOfTwo(n, jacobi);
        }
        return currentA;
    }
