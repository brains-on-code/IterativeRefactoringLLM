...
public class CircularLookScheduling {
    ...
    public List<Integer> execute(List<Integer> requests) {
        List<Integer> validRequests = getValidRequests(requests);

        List<Integer> upRequests = new ArrayList<>();
        List<Integer> downRequests = new ArrayList<>();
        partitionRequests(validRequests, upRequests, downRequests);

        Collections.sort(upRequests);
        Collections.sort(downRequests);

        List<Integer> scheduledOrder = createSchedule(upRequests, downRequests);
        updateCurrentPosition(scheduledOrder);

        return scheduledOrder;
    }

    private List<Integer> getValidRequests(List<Integer> requests) {
    ...
    private List<Integer> createSchedule(List<Integer> upRequests, List<Integer> downRequests) {
