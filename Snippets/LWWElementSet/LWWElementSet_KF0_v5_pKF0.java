...
class LWWElementSet {
    ...
    public boolean isSubsetOf(LWWElementSet other) {
        return isMapKeySubset(addSet, other.addSet) && isMapKeySubset(removeSet, other.removeSet);
    }

    private boolean isMapKeySubset(Map<String, Element> subset, Map<String, Element> superset) {
        return superset.keySet().containsAll(subset.keySet());
    }
    ...
    public void merge(LWWElementSet other) {
        other.addSet.values().forEach(element -> mergeElement(addSet, element));
        other.removeSet.values().forEach(element -> mergeElement(removeSet, element));
    }
