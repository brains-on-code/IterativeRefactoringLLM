...
public class MLFQScheduler {
    ...
    private final int[] queueTimeQuanta; // Time quantum for each queue level
    ...
    public void run() {
        while (!areAllQueuesEmpty()) {
            for (int queueIndex = 0; queueIndex < feedbackQueues.size(); queueIndex++) {
                Queue<ScheduledProcess> currentQueue = feedbackQueues.get(queueIndex);
                if (!currentQueue.isEmpty()) {
                    ScheduledProcess currentProcess = currentQueue.poll();
                    int quantumForQueue = queueTimeQuanta[queueIndex];

                    // Execute the process for the minimum of the time quantum or the remaining time
                    int timeSlice = Math.min(quantumForQueue, currentProcess.remainingTime);
                    ...
                    if (currentProcess.isFinished()) {
                        ...
                        if (queueIndex < feedbackQueues.size() - 1) {
                            currentProcess.priority++; // Demote the process to the next lower priority queue
                            feedbackQueues.get(queueIndex + 1).add(currentProcess); // Add to the next queue level
                        } else {
    ...
    private boolean areAllQueuesEmpty() {
        for (Queue<ScheduledProcess> queue : feedbackQueues) {
            if (!queue.isEmpty()) {
                return false;
            }
        }
