...
public class RobotState {
  ...
  private RobotState() {
    for (int stateIndex = 0; stateIndex < 3; ++stateIndex) {
      ...
      processNoiseVariances.set(stateIndex, 0, stdDev * stdDev);
    }
  ...
  public void addOdometryObservation(OdometryObservation odometryObservation) {
    ...
    poseHistoryBuffer.addSample(odometryObservation.timestamp(), odometryPose);
  ...
  public void addVisionObservation(VisionObservation visionObservation) {
    double visionTimestamp = visionObservation.timestamp();

    // If measurement is old enough to be outside the pose buffer's timespan, skip.
    try {
      double newestBufferedTimestamp = poseHistoryBuffer.getInternalBuffer().lastKey();
      if (newestBufferedTimestamp - POSE_BUFFER_DURATION_SECONDS > visionTimestamp) {
        return;
      }
    ...
    var odometrySampleAtObservationTime = poseHistoryBuffer.getSample(visionTimestamp);
    ...
    Transform2d odometrySampleToCurrentTransform =
    ...
    Matrix<N3, N1> visionStdDevs = visionObservation.standardDeviations();
    for (int stateIndex = 0; stateIndex < 3; ++stateIndex) {
      double stdDev = visionStdDevs.get(stateIndex, 0);
      visionMeasurementVariances[stateIndex] = stdDev * stdDev;
    }
    ...
    for (int stateIndex = 0; stateIndex < 3; ++stateIndex) {
      double processVariance = processNoiseVariances.get(stateIndex, 0);
      if (processVariance == 0.0) {
        kalmanGain.set(stateIndex, stateIndex, 0.0);
      } else {
        double measurementVariance = visionMeasurementVariances[stateIndex];
        kalmanGain.set(
            stateIndex,
            stateIndex,
            processVariance
    ...
    Transform2d estimateToVisionTransform =
        new Transform2d(
            estimatedPoseAtObservationTime, visionObservation.visionPose().toPose2d());
    ...
    estimatedPose =
        estimatedPoseAtObservationTime
            ...
            .plus(odometrySampleToCurrentTransform);
  }
