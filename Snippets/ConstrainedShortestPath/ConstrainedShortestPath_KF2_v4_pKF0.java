...
public class ConstrainedShortestPath {
    ...
    public int solve(int start, int target) {
        ...
        int[][] minCostForResource = initializeCostTable(numNodes);

        minCostForResource[0][start] = 0;
        relaxAllEdges(minCostForResource);

        int minCost = findMinCostToTarget(target, minCostForResource);
        return minCost == Integer.MAX_VALUE ? -1 : minCost;
    }

    private int[][] initializeCostTable(int numNodes) {
    ...
    private void relaxAllEdges(int[][] costTable) {
        int numNodes = graph.getNumNodes();
    ...
    private int findMinCostToTarget(int target, int[][] costTable) {
