...
public class ConstrainedShortestPath {
    ...
    public int solve(int startNode, int targetNode) {
        ...
        int[][] minCostForResourceAndNode = new int[maxResource + 1][nodeCount];
        ...
        for (int resourceUsed = 0; resourceUsed <= maxResource; resourceUsed++) {
            for (int currentNode = 0; currentNode < nodeCount; currentNode++) {
                if (minCostForResourceAndNode[resourceUsed][currentNode] == Integer.MAX_VALUE) {
                    continue;
                }
                for (Graph.Edge edge : graph.getEdges(currentNode)) {
                    ...
                    int newResourceUsage = resourceUsed + edgeResource;
                    if (newResourceUsage <= maxResource) {
                        int newCost = minCostForResourceAndNode[resourceUsed][currentNode] + edgeCost;
                        minCostForResourceAndNode[newResourceUsage][nextNode] =
                            Math.min(minCostForResourceAndNode[newResourceUsage][nextNode], newCost);
                    }
                }
            }
        }
