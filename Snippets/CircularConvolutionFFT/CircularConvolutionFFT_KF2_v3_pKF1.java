...
public final class CircularConvolutionFFT {
    ...
    public static ArrayList<FFT.Complex> computeCircularConvolution(
        ...
        int convolutionLength = Math.max(firstSequence.size(), secondSequence.size());

        padWithZeros(firstSequence, convolutionLength);
        padWithZeros(secondSequence, convolutionLength);
        ...
        for (int i = 0; i < convolutionLength; i++) {
            FFT.Complex product =
                    firstSequence.get(i).multiply(secondSequence.get(i));
