...
class GraphVertex {
    ...
    Set<Integer> neighbors = new HashSet<>(); // Adjacent vertex indices
}
        ...
        for (int startVertexIndex = 1; startVertexIndex <= vertexCount; startVertexIndex++) {
            ...
            while (!bfsQueue.isEmpty()) {
                ...
                for (int neighborIndex : currentVertex.neighbors) {
                    GraphVertex neighborVertex = vertices.get(neighborIndex);
                    ...
                    maxColorUsed = Math.max(maxColorUsed, Math.max(currentVertex.color, neighborVertex.color));
