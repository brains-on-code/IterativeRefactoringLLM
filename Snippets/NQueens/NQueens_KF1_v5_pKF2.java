...
public final class NQueensSolver {
     ...
     * @param n the size of the board and the number of queens
     */
    public static void printSolutions(final int n) {
        List<List<String>> solutions = new ArrayList<>();
        backtrack(n, solutions, new int[n], 0);

        if (solutions.isEmpty()) {
            System.out.println("There is no way to place " + n + " queens on a board of size " + n + "x" + n);
        } else {
            System.out.println("Arrangements for placing " + n + " queens:");
        }
    ...
    /**
     * Recursively generates all valid N-Queens configurations.
     *
    ...
    private static List<String> buildBoard(int[] positions, int n) {
        ...
        for (int row = 0; row < n; row++) {
            ...
            for (int col = 0; col < n; col++) {
                rowBuilder.append(col == positions[row] ? 'Q' : '.');
            }
    ...
    private static boolean isSafe(int[] positions, int col, int row) {
        for (int r = 0; r < row; r++) {
            ...
            boolean sameColumn = colDiff == 0;
            boolean sameDiagonal = row - r == colDiff;
