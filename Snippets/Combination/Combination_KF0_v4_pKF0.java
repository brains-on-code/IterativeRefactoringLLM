...
public final class Combination {
    ...
    public static <T> List<TreeSet<T>> combination(T[] arr, int n) {
        ...
        generateCombinations(sortedArray, n, 0, new TreeSet<>(), combinations);
        return combinations;
    }
    ...
    private static <T> void generateCombinations(
        ...
        if (current.size() == targetSize - 1) {
            addLastElements(arr, startIndex, current, result);
            return;
        }
        ...
        for (int i = startIndex; i < arr.length; i++) {
            T element = arr[i];
            current.add(element);
            generateCombinations(arr, targetSize, i + 1, current, result);
