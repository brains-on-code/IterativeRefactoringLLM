...
public class SimpleSubCipher {
    ...
    public String encode(String message, String cipherKey) {
        ...
        for (char ch : message.toCharArray()) {
            if (Character.isAlphabetic(ch)) {
                encoded.append(encodeMap.getOrDefault(ch, ch));
            } else {
                encoded.append(ch);
            }
        }
    ...
    public String decode(String encryptedMessage, String cipherKey) {
        ...
        for (char ch : encryptedMessage.toCharArray()) {
            if (Character.isAlphabetic(ch)) {
                decoded.append(decodeMap.getOrDefault(ch, ch));
            } else {
                decoded.append(ch);
            }
        }
     ...
     *
     * @param cipherKey 26-character substitution for 'a'–'z'
     * @return mapping from plain characters to cipher characters
     */
     ...
     *
     * @param cipherKey 26-character substitution for 'a'–'z'
     * @return mapping from cipher characters to plain characters
     */
    ...
    /**
     * Validates that the cipher key is non-null and has the expected length.
     *
