...
public class SkylineProblem {
    ...
    public List<Skyline> findSkyline(int start, int end) {
        if (start == end) {
            ...
            Building b = buildings[start];
            skyline.add(new Skyline(b.left, b.height));
            skyline.add(new Skyline(b.right, 0));
            return skyline;
        }
    ...
    public List<Skyline> mergeSkyline(List<Skyline> leftSkyline, List<Skyline> rightSkyline) {
        ...
        while (leftIndex < leftSkyline.size() && rightIndex < rightSkyline.size()) {
            ...
            int x;
            if (leftPoint.coordinates < rightPoint.coordinates) {
                x = leftPoint.coordinates;
                leftHeight = leftPoint.height;
                leftIndex++;
            } else {
                x = rightPoint.coordinates;
            ...
            if (maxHeightAtX != currentMaxHeight) {
                merged.add(new Skyline(x, maxHeightAtX));
