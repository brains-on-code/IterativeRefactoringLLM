...
public class QuadraticEquationSolver {
    ...
    public ComplexNumber[] solveEquation(double coefficientA, double coefficientB, double coefficientC) {
        ...
        double twoA = 2 * coefficientA;

        if (discriminant > 0) {
            double sqrtDiscriminant = Math.sqrt(discriminant);
            double firstRoot = (-coefficientB + sqrtDiscriminant) / twoA;
            double secondRoot = (-coefficientB - sqrtDiscriminant) / twoA;
        ...
        if (discriminant == 0) {
            double repeatedRoot = -coefficientB / twoA;
            return new ComplexNumber[] {
                new ComplexNumber(repeatedRoot)
            };
        }

        double realPart = -coefficientB / twoA;
