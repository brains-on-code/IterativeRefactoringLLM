...
public final class RgbHsvConversion {
    ...
    public static int[] hsvToRgb(double hueDegrees, double saturation, double value) {
        ...
        double hueSectorIndex = hueDegrees / 60;
        double secondaryComponent = chroma * (1 - Math.abs(hueSectorIndex % 2 - 1));
        double matchComponent = value - chroma;

        return convertHueSectorToRgb(hueSectorIndex, chroma, matchComponent, secondaryComponent);
    }
    ...
    private static boolean approximatelyEqualHsv(double[] hsvColor1, double[] hsvColor2) {
        boolean hueClose = Math.abs(hsvColor1[0] - hsvColor2[0]) < 0.2;
        boolean saturationClose = Math.abs(hsvColor1[1] - hsvColor2[1]) < 0.002;
        boolean valueClose = Math.abs(hsvColor1[2] - hsvColor2[2]) < 0.002;
