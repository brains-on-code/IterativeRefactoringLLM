...
public class DriveCommands {
  ...
  public static Translation2d getLinearVelocityFromJoysticks(double xInput, double yInput) {
    ...
    Rotation2d joystickDirection = new Rotation2d(xInput, yInput);

    double scaledMagnitude = joystickMagnitude * joystickMagnitude;

    return new Pose2d(Translation2d.kZero, joystickDirection)
        .transformBy(new Transform2d(scaledMagnitude, 0.0, Rotation2d.kZero))
        .getTranslation();
  }
  ...
  public static Command joystickDriveWhileLaunching(
    ...
    return Commands.run(
        () -> {
          int controllerInstanceId = headingController.hashCode();

          if (launchDriveKp.hasChanged(controllerInstanceId)
              || launchDriveKd.hasChanged(controllerInstanceId)
              || launchDriveMaxVelocity.hasChanged(controllerInstanceId)
              || launchDriveMaxAcceleration.hasChanged(controllerInstanceId)) {
