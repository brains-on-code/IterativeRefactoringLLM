...
public final class Ipv4Ipv6Mapper {
    ...
    /** Indices for the 0xffff IPv6-mapped prefix bytes. */
    private static final int IPV6_MAPPED_PREFIX_HIGH_INDEX = 10;
    private static final int IPV6_MAPPED_PREFIX_LOW_INDEX = 11;
    ...
    private static boolean isIpv6MappedIpv4(byte[] addressBytes) {
        ...
        if (!hasZeroPrefix(addressBytes)) {
            return false;
        }

        return hasMappedPrefix(addressBytes);
    }

    private static boolean hasZeroPrefix(byte[] addressBytes) {
        for (int i = 0; i < IPV6_MAPPED_PREFIX_HIGH_INDEX; i++) {
            if (addressBytes[i] != 0) {
                return false;
            }
        }
    ...
    private static boolean hasMappedPrefix(byte[] addressBytes) {
        return addressBytes[IPV6_MAPPED_PREFIX_HIGH_INDEX] == IPV6_MAPPED_PREFIX_BYTE
            && addressBytes[IPV6_MAPPED_PREFIX_LOW_INDEX] == IPV6_MAPPED_PREFIX_BYTE;
    }
    ...
    private static byte[] createIpv6MappedBytes(byte[] ipv4Bytes) {
        ...
        ipv6Bytes[IPV6_MAPPED_PREFIX_HIGH_INDEX] = IPV6_MAPPED_PREFIX_BYTE;
        ipv6Bytes[IPV6_MAPPED_PREFIX_LOW_INDEX] = IPV6_MAPPED_PREFIX_BYTE;
