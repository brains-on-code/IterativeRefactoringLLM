...
final class Sudoku {
    ...
    public static boolean isSafe(int[][] board, int row, int col, int value) {
        ...
        for (int rowIndex = subgridRowStart; rowIndex < subgridRowStart + subgridSize; rowIndex++) {
            for (int columnIndex = subgridColStart; columnIndex < subgridColStart + subgridSize; columnIndex++) {
    ...
    public static boolean solveSudoku(int[][] board, int boardSize) {
        ...
        for (int candidateValue = 1; candidateValue <= boardSize; candidateValue++) {
            if (isSafe(board, emptyRowIndex, emptyColIndex, candidateValue)) {
                board[emptyRowIndex][emptyColIndex] = candidateValue;
                if (solveSudoku(board, boardSize)) {
                    return true;
                } else {
                    // backtrack
                    board[emptyRowIndex][emptyColIndex] = 0;
                }
            }
        }
        return false;
