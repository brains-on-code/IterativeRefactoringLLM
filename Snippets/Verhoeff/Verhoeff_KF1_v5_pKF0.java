...
public final class Verhoeff {
    ...
    public static boolean isValid(String value) {
        ...
        return computeChecksum(digits) == 0;
    }
    ...
    public static String addCheckDigit(String value) {
        ...
        int[] digitsWithPlaceholder = toIntArray(value + "0");
        int checksum = computeChecksum(digitsWithPlaceholder);
    ...
    private static void printValidationResult(String value) {
        ...
        System.out.printf("Input '%s' is %s%n", value, result);
    }

    private static void printCheckDigitGeneration(String value) {
        String withCheckDigit = addCheckDigit(value);
        System.out.printf(
            "Generate and add checksum to initial value '%s'. Result: '%s'%n",
            value,
            withCheckDigit
        );
    }
    ...
    private static void validateNumeric(String value) {
        ...
        if (!value.matches("\\d+")) {
            throw new IllegalArgumentException(
                String.format("Input '%s' contains non-digit characters", value)
            );
        }
    }
    ...
    private static int computeChecksum(int[] digits) {
        ...
        for (int position = 0; position < length; position++) {
            int reversedIndex = length - 1 - position;
            int digit = digits[reversedIndex];
            int permuted = P_TABLE[position % P_TABLE.length][digit];
