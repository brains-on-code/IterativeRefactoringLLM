...
public final class ComplexCollectionUtils {
    ...
    public static ArrayList<FFT.Complex> convolve(
        ...
        ArrayList<FFT.Complex> result = multiplyElementWise(firstSequence, secondSequence);
    ...
    /**
     * Multiplies two complex sequences element-wise.
     *
     * @param firstSequence  the first complex sequence (already FFT-transformed)
     * @param secondSequence the second complex sequence (already FFT-transformed)
     * @return a new list containing the element-wise products
     */
    private static ArrayList<FFT.Complex> multiplyElementWise(
            ArrayList<FFT.Complex> firstSequence,
            ArrayList<FFT.Complex> secondSequence
    ) {
        int size = Math.min(firstSequence.size(), secondSequence.size());
        ArrayList<FFT.Complex> result = new ArrayList<>(size);

        for (int i = 0; i < size; i++) {
            FFT.Complex first = firstSequence.get(i);
            FFT.Complex second = secondSequence.get(i);
