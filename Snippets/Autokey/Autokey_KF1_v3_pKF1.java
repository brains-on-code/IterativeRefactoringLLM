...
public class AutokeyCipher {
    ...
    public String encrypt(String plaintext, String key) {
        ...
        String sanitizedKey = key.toUpperCase();

        StringBuilder autokey = new StringBuilder(sanitizedKey).append(sanitizedPlaintext);
        StringBuilder ciphertextBuilder = new StringBuilder();

        for (int index = 0; index < sanitizedPlaintext.length(); index++) {
            char plaintextCharacter = sanitizedPlaintext.charAt(index);
            char keyCharacter = autokey.charAt(index);
    ...
    public String decrypt(String ciphertext, String key) {
        ...
        for (int index = 0; index < sanitizedCiphertext.length(); index++) {
            ...
            evolvingKey.append(decryptedCharacter);
        }
