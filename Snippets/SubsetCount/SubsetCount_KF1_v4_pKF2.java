...
public final class Class1 {
     ...
     * Uses a 2D dynamic programming table where:
     *
    ...
    public static int method1(int[] nums, int target) {
        ...
        // There is exactly one subset (the empty subset) that makes sum 0
        for (int i = 0; i < n; i++) {
            dp[i][0] = 1;
        }

        // Initialize subsets using only nums[0]
        if (nums[0] <= target) {
            dp[0][nums[0]] = 1;
        }

        // Build the DP table for remaining elements
        for (int i = 1; i < n; i++) {
    ...
    /**
     * Counts the number of subsets of the given array whose elements sum to the given target.
     * Uses a space-optimized 1D dynamic programming approach where:
     *
    ...
    public static int method2(int[] nums, int target) {
        ...
        // One way to make sum 0: the empty subset
        prev[0] = 1;
