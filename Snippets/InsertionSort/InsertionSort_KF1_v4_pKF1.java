...
class InsertionSort implements SortAlgorithm {
    ...
    public <T extends Comparable<T>> T[] sort(T[] array, final int fromIndex, final int toIndex) {
        ...
        for (int currentIndex = fromIndex + 1; currentIndex < toIndex; currentIndex++) {
            final T currentElement = array[currentIndex];
            int sortedIndex = currentIndex - 1;

            while (sortedIndex >= fromIndex && SortUtils.less(currentElement, array[sortedIndex])) {
    ...
    public <T extends Comparable<T>> T[] sortWithSentinel(T[] array) {
        ...
        for (int currentIndex = 2; currentIndex < array.length; currentIndex++) {
            ...
            while (sortedIndex > 0 && SortUtils.less(currentElement, array[sortedIndex - 1])) {
                array[sortedIndex] = array[sortedIndex - 1];
                sortedIndex--;
            }
            array[sortedIndex] = currentElement;
        }
