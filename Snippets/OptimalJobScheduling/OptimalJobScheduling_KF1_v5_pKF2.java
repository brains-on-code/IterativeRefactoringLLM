...
/**
 * Computes a minimal-cost dynamic programming (DP) table based on:
 * <ul>
...
public class Class1 {
    ...
    /** Base cost matrix (dimensions: {@code rows x cols}). */
    private final int[][] baseCost;

    /** Transition cost matrix (dimensions: {@code cols x cols}). */
    private final int[][] transitionCost;

    /** DP table storing computed minimal costs (dimensions: {@code rows x cols}). */
     ...
     * <p>Recurrence:
     * <ul>
     *     <li>First row: {@code dp[0][c] = baseCost[0][c]}</li>
     *     <li>Row {@code r > 0}:
    ...
    private int computeCell(int row, int col) {
        ...
        for (int prevCol = 0; prevCol < cols; prevCol++) {
            int candidate = dp[row - 1][prevCol] + transitionCost[prevCol][col] + base;
