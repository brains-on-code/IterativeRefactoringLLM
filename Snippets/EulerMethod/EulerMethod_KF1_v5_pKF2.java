...
/**
 * Utility class for numerically solving first-order ODEs using Euler's method.
 *
 * <p>The {@link #main(String[])} method runs three sample problems:
 * <ul>
 *     <li>y' = x,            y(0) = 0</li>
 *     <li>y' = y,            y(0) = 1</li>
 *     <li>y' = x + y + x*y,  y(0) = 1</li>
 * </ul>
 */
public final class EulerSolver {

    private EulerSolver() {}

    /**
     * Runs three example ODEs using Euler's method and prints the results.
     *
    ...
    public static void main(String[] args) {
        ...
        List<double[]> result1 = eulerMethod(0, 4, 0.1, 0, derivative1);
        assert result1.get(result1.size() - 1)[1] == 7.800000000000003;
        printResult(result1);

        System.out.println("\n\nexample 2: y' = y, y(0) = 1");
        BiFunction<Double, Double, Double> derivative2 = (x, y) -> y;
        List<double[]> result2 = eulerMethod(0, 4, 0.1, 1, derivative2);
        ...
        List<double[]> result3 = eulerMethod(0, 0.1, 0.025, 1, derivative3);
        assert result3.get(result3.size() - 1)[1] == 1.1116729841674804;
        printResult(result3);
    }

    private static void printResult(List<double[]> result) {
        result.forEach(point -> System.out.printf("x: %1$f; y: %2$f%n", point[0], point[1]));
    }

    /**
     * Computes a single Euler step.
     *
     * <p>Given the current point (x, y), step size h, and derivative f(x, y),
     * this returns:
     *
     * <pre>
     ...
     */
    public static List<double[]> eulerMethod(
        double xStart,
        double xEnd,
        double stepSize,
        double yStart,
