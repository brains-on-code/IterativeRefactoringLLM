...
public final class LetterCombinationsOfPhoneNumber {
    ...
    private static final char EMPTY_MAPPING_CHAR = '\0';

    private static final String[] DIGIT_TO_LETTERS = {
        " ",
        String.valueOf(EMPTY_MAPPING_CHAR),
        "abc",
        "def",
        "ghi",
        "jkl",
    ...
    private static List<String> buildCombinations(int[] digits, int digitIndex, StringBuilder partialCombination) {
        ...
        String lettersForDigit = DIGIT_TO_LETTERS[digit];

        for (char letter : lettersForDigit.toCharArray()) {
            boolean hasNoMapping = (letter == EMPTY_MAPPING_CHAR);
