...
public class CRCAlgorithm {
    ...
    private boolean isMessageModified;
     ...
     * the generator polynomial, so the remainderBits list is created. If verify == true,
     ...
     * of the variables correctMessageCount, undetectedErroneousMessageCount, changes. If verify ==
    ...
    public void divideMessageWithGeneratorPolynomial(boolean verify) {
        ...
        if (!verify) {
            ...
            if (remainderHasOne && isMessageModified) {
                detectedErroneousMessageCount++;
            } else if (!remainderHasOne && isMessageModified) {
     ...
     * changes, the boolean variable isMessageModified gets the value: true, or
     * false.
     */
