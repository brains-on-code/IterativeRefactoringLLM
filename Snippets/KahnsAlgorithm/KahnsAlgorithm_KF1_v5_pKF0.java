...
class TopologicalSorter<E extends Comparable<E>> {
    ...
    ArrayList<E> topologicalSort() {
        computeInDegrees();
        Deque<E> zeroInDegreeQueue = initializeZeroInDegreeQueue();

        ArrayList<E> sortedOrder = new ArrayList<>();
        int visitedCount = 0;

        while (!zeroInDegreeQueue.isEmpty()) {
            E vertex = zeroInDegreeQueue.removeFirst();
            ...
            for (E neighbor : graph.getNeighbors(vertex)) {
                ...
                if (updatedInDegree == 0) {
                    zeroInDegreeQueue.addLast(neighbor);
                }
            }
        }
    ...
    private Deque<E> initializeZeroInDegreeQueue() {
        Deque<E> zeroInDegreeQueue = new ArrayDeque<>();
        for (Map.Entry<E, Integer> entry : inDegree.entrySet()) {
            if (entry.getValue() == 0) {
                zeroInDegreeQueue.addLast(entry.getKey());
            }
        }
