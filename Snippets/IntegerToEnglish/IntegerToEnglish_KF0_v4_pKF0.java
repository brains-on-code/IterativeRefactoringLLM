...
public final class IntegerToEnglish {
    ...
    private static final Map<Integer, String> BASE_NUMBERS_MAP =
        Map.ofEntries(
            Map.entry(0, ""),
            Map.entry(1, "One"),
            Map.entry(2, "Two"),
    ...
    private static final Map<Integer, String> THOUSAND_POWER_MAP =
        Map.ofEntries(
    ...
    private static String convertToWords(int number) {
        ...
        appendHundreds(result, hundredsDigit);
        appendRemainder(result, remainder);

        return result.toString();
    }

    private static void appendHundreds(StringBuilder result, int hundredsDigit) {
        if (hundredsDigit <= 0) {
            return;
        }

        result.append(BASE_NUMBERS_MAP.get(hundredsDigit))
              .append(SPACE)
              .append(BASE_NUMBERS_MAP.get(100));
    }

    private static void appendRemainder(StringBuilder result, int remainder) {
        if (remainder <= 0) {
            return;
        }
    ...
    private static void appendTensAndOnes(StringBuilder result, int numberBelowHundred) {
        if (numberBelowHundred <= 20 || BASE_NUMBERS_MAP.containsKey(numberBelowHundred)) {
            result.append(BASE_NUMBERS_MAP.get(numberBelowHundred));
            return;
        }

        int tensDigit = numberBelowHundred / 10;
        int onesDigit = numberBelowHundred % 10;
        ...
        result.append(tens);
        if (!ones.isEmpty()) {
            result.append(SPACE).append(ones);
        }
    }
    ...
    public static String integerToEnglishWords(int number) {
        ...
        while (number > 0) {
            ...
            if (groupValue > 0) {
                prependGroupWords(result, groupValue, thousandGroupIndex);
            }
    ...
    private static void prependGroupWords(StringBuilder result, int groupValue, int thousandGroupIndex) {
        StringBuilder groupBuilder = new StringBuilder(convertToWords(groupValue));

        if (thousandGroupIndex > 0) {
            groupBuilder.append(SPACE).append(THOUSAND_POWER_MAP.get(thousandGroupIndex));
        }

        if (result.length() > 0) {
            result.insert(0, SPACE);
        }
        result.insert(0, groupBuilder);
    }
}