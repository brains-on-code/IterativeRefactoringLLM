...
public final class BinarySearch2dArray {
    ...
    static int[] binarySearch(int[][] matrix, int target) {
        ...
        boolean inFirstQuadrant = target <= matrix[startRow][midCol - 1];
        if (inFirstQuadrant) {
            return binarySearchInRow(matrix, target, startRow, 0, midCol - 1);
        }

        boolean inSecondQuadrant =
                target >= matrix[startRow][midCol + 1] && target <= matrix[startRow][colCount - 1];
        if (inSecondQuadrant) {
            return binarySearchInRow(matrix, target, startRow, midCol + 1, colCount - 1);
        }

        boolean inThirdQuadrant = target <= matrix[endRow][midCol - 1];
        if (inThirdQuadrant) {
