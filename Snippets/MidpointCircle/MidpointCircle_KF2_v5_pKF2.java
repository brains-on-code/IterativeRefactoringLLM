...
/**
 * Utility class for generating integer points on the circumference of a circle
...
public final class MidpointCircle {
    ...
    public static List<int[]> generateCirclePoints(int centerX, int centerY, int radius) {
        ...
        while (x > y) {
            ...
            if (decisionParam <= 0) {
                // Midpoint is inside or on the perimeter; move vertically.
                decisionParam += 2 * y + 1;
            } else {
                // Midpoint is outside; move diagonally.
                x--;
    ...
    private static void addSymmetricPoints(
        ...
        // Four points in the primary axes-aligned directions.
        points.add(new int[] {centerX + x, centerY + y});
        points.add(new int[] {centerX - x, centerY + y});
        points.add(new int[] {centerX + x, centerY - y});
        points.add(new int[] {centerX - x, centerY - y});

        // Four points obtained by swapping x and y (diagonal symmetry).
