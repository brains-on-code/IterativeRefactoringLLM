    ...
    public int solve(int start, int target) {
        ...
        for (int resourceUsed = 0; resourceUsed <= maxResource; resourceUsed++) {
            for (int currentNode = 0; currentNode < numNodes; currentNode++) {
                if (dp[resourceUsed][currentNode] == Integer.MAX_VALUE) {
                    continue;
                }
                for (Graph.Edge edge : graph.getEdges(currentNode)) {
                    ...
                    int nextResource = resourceUsed + edgeResource;

                    if (nextResource <= maxResource) {
                        dp[nextResource][nextNode] =
                            Math.min(dp[nextResource][nextNode], dp[resourceUsed][currentNode] + edgeCost);
                    }
                }
            }
        }
