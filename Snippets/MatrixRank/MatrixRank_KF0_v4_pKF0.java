...
public final class MatrixRank {
    ...
    public static int computeRank(double[][] matrix) {
        ...
        int rank = 0;
        boolean[] isPivotRow = new boolean[rowCount];
        double[][] workingMatrix = copyMatrix(matrix);

        for (int column = 0; column < columnCount; column++) {
            int pivotRow = findPivotRow(workingMatrix, isPivotRow, column);
     ...
     * @param isPivotRow flags indicating which rows are already used as pivot rows
     * @param column     the column index
     * @return the pivot row index, or the number of rows if no suitable pivot row was found
     */
    private static int findPivotRow(double[][] matrix, boolean[] isPivotRow, int column) {
        int rowCount = matrix.length;

        for (int row = 0; row < rowCount; row++) {
            if (!isPivotRow[row] && !isZero(matrix[row][column])) {
                return row;
            }
        }
    ...
    private static void eliminateColumnFromOtherRows(double[][] matrix, int pivotRow, int column) {
        ...
        for (int row = 0; row < rowCount; row++) {
            if (row == pivotRow || isZero(matrix[row][column])) {
                continue;
            }
