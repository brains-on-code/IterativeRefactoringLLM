...
public final class FastInverseSqrt {
    ...
    private static final int DOUBLE_NEWTON_ITERATIONS = 4;
    ...
    public static float fastInverseSqrt(float number) {
        final float halfNumber = 0.5f * number;
        ...
        approximation = newtonRaphsonStep(approximation, halfNumber);

        return approximation;
    }

    private static float newtonRaphsonStep(float approximation, float halfNumber) {
    ...
    public static boolean verifyInverseSqrt(float number) {
        final float approx = fastInverseSqrt(number);
        final float exact = 1.0f / (float) Math.sqrt(number);
        return approx == exact;
    }
    ...
    public static double fastInverseSqrt(double number) {
        final double halfNumber = 0.5d * number;
        ...
        for (int i = 0; i < DOUBLE_NEWTON_ITERATIONS; i++) {
            approximation = newtonRaphsonStep(approximation, halfNumber);
        }
    ...
    private static double newtonRaphsonStep(double approximation, double halfNumber) {
        return approximation * (DOUBLE_THREE_HALVES - halfNumber * approximation * approximation);
    }
    ...
    public static boolean verifyInverseSqrt(double number) {
        final double approx = fastInverseSqrt(number);
        final double exact = 1.0d / Math.sqrt(number);
        return approx == exact;
    }
}