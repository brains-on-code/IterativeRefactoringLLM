...
public class PoseEstimator {
  ...
  private PoseEstimator() {
    for (int row = 0; row < 3; ++row) {
      visionProcessVariance.set(
          row, 0, Math.pow(VISION_MEASUREMENT_STD_DEVIATIONS.get(row, 0), 2));
    }
  ...
  public void addVisionMeasurement(VisionMeasurement visionMeasurement) {
    try {
      if (poseHistoryBuffer.getInternalBuffer().lastKey() - POSE_BUFFER_DURATION_SECONDS
    ...
    var odometrySampleAtVisionTime = poseHistoryBuffer.getSample(visionMeasurement.timestamp());
