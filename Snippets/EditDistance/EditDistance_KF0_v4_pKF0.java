        ...
        for (int sourceIndex = 0; sourceIndex < sourceLength; sourceIndex++) {
            ...
            for (int targetIndex = 0; targetIndex < targetLength; targetIndex++) {
                ...
                if (sourceChar == targetChar) {
                    dp[sourceIndex + 1][targetIndex + 1] = dp[sourceIndex][targetIndex];
                } else {
                    int replaceCost = dp[sourceIndex][targetIndex];
                    int insertCost = dp[sourceIndex][targetIndex + 1];
                    int deleteCost = dp[sourceIndex + 1][targetIndex];

                    dp[sourceIndex + 1][targetIndex + 1] =
                        1 + min(replaceCost, insertCost, deleteCost);
                }
            }
        }
