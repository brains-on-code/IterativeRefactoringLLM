...
public final class EggDropping {
    ...
    public static int minAttempts(int eggs, int floors) {
        ...
        computeMinAttempts(dp, eggs, floors);
    ...
    private static void computeMinAttempts(int[][] dp, int eggs, int floors) {
        for (int egg = 2; egg <= eggs; egg++) {
            for (int floor = 2; floor <= floors; floor++) {
                dp[egg][floor] = computeStateMinAttempts(dp, egg, floor);
            }
        }
    }

    private static int computeStateMinAttempts(int[][] dp, int eggs, int floors) {
