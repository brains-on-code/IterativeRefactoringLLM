...
public class ScanDiskScheduler {
    ...
    public List<Integer> scheduleRequests(List<Integer> requests) {
        ...
        if (movingTowardsHigherCylinders) {
            processMovingTowardsHigherCylinders(scheduledRequests, lowerCylinderRequests, higherCylinderRequests);
        } else {
            processMovingTowardsLowerCylinders(scheduledRequests, lowerCylinderRequests, higherCylinderRequests);
        }
    ...
    private void processMovingTowardsHigherCylinders(
        List<Integer> scheduledRequests,
    ...
    private void processMovingTowardsLowerCylinders(
        List<Integer> scheduledRequests,
        List<Integer> lowerCylinderRequests,
        List<Integer> higherCylinderRequests
    ) {
        ...
        if (!higherCylinderRequests.isEmpty()) {
            lastServicedRequest =
                Math.max(lastServicedRequest, higherCylinderRequests.get(higherCylinderRequests.size() - 1));
        }
    }
