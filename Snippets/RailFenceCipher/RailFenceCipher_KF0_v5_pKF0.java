...
public class RailFenceCipher {
    ...
    private void fillRailsForEncryption(String text, char[][] railMatrix) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            movingDown = getNextDirection(row, totalRows, movingDown);
            row += movingDown ? 1 : -1;
        }
    }
    ...
    private void markRailPattern(char[][] railMatrix) {
        ...
        while (col < cols) {
            ...
            movingDown = getNextDirection(row, rows, movingDown);
            row += movingDown ? 1 : -1;
        }
    }
    ...
    private boolean getNextDirection(int currentRow, int totalRows, boolean currentDirection) {
        if (currentRow == 0) {
            return true;
        }
