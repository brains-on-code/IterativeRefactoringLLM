...
class CycleFinder {
    ...
    CycleFinder(int nodeCount, int edgeCount, int[][] edges) {
        ...
        buildAdjacencyMatrix(edgeCount, edges);
    }

    private void buildAdjacencyMatrix(int edgeCount, int[][] edges) {
    ...
    public void findCycles() {
        for (int node = 0; node < nodeCount; node++) {
            ...
            dfs(node, node, currentPath);
            removeAllIncidentEdges(node);
        }
    }

    private void dfs(int startNode, int currentNode, List<Integer> currentPath) {
        ...
        for (int neighbor = 0; neighbor < nodeCount; neighbor++) {
            ...
            if (neighbor == startNode) {
                ...
                dfs(startNode, neighbor, currentPath);
            }
        }
    ...
    private void removeAllIncidentEdges(int node) {
