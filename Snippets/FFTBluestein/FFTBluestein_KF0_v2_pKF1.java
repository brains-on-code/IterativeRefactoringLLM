...
public final class FFTBluestein {
    ...
    public static void fftBluestein(List<FFT.Complex> signal, boolean inverse) {
        ...
        for (int index = 0; index < signalLength; index++) {
            double angle = -index * index * Math.PI / signalLength * transformSign;
            FFT.Complex modulationFactor = new FFT.Complex(Math.cos(angle), Math.sin(angle));
            modulatedSignal.add(signal.get(index).multiply(modulationFactor));
        }
        ...
        if (inverse) {
            for (int index = 0; index < signalLength; index++) {
                FFT.Complex currentValue = signal.get(index);
                signal.set(index, currentValue.divide(signalLength));
            }
        }
    }
}