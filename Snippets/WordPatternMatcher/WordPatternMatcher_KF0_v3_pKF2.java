...
public final class WordPatternMatcher {
    ...
    private static boolean backtrack(
        ...
        // Base case: both pattern and string are fully consumed
        if (patternIndex == pattern.length() && stringIndex == inputString.length()) {
            return true;
        }

        // Mismatch in lengths: one is exhausted before the other
        if (patternIndex == pattern.length() || stringIndex == inputString.length()) {
            return false;
        }
        ...
        if (patternToString.containsKey(currentPatternChar)) {
            ...
            // The next part of the input must match the mapped substring
        ...
        // Case 2: no mapping yet; try all possible non-empty substrings
        for (int end = stringIndex + 1; end <= inputString.length(); end++) {
            String candidateSubstring = inputString.substring(stringIndex, end);

            // Enforce bijection: candidate must not already be mapped to another pattern character
