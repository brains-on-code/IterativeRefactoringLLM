...
public final class BinaryToHexConverter {
    ...
    private static int extractNextHexDigitValue(int binaryNumber) {
        int decimalValue = 0;
        int remainingBinary = binaryNumber;
    ...
    private static int removeProcessedBits(int binaryNumber) {
        int processedBits = 0;
        int remainingBinary = binaryNumber;

        while (processedBits < BITS_PER_HEX_DIGIT && remainingBinary != 0) {
            remainingBinary /= DECIMAL_BASE;
            processedBits++;
        }
