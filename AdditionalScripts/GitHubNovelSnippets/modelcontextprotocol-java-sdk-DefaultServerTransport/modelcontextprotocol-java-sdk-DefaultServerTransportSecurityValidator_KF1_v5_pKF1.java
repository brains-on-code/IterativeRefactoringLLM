...
public class OriginHeaderSecurityValidator implements ServerTransportSecurityValidator {
	...
	protected void validateOrigin(String origin) throws ServerTransportSecurityException {
		...
		for (String allowedOriginPattern : allowedOriginPatterns) {
			if (isOriginAllowed(origin, allowedOriginPattern)) {
				return;
			}
		}
	...
	private boolean isOriginAllowed(String origin, String allowedOriginPattern) {
		...
		if (allowedOriginPattern.endsWith(":*")) {
			...
			return origin.equals(allowedOriginPrefix) || origin.startsWith(allowedOriginPrefix + ":");
		}
