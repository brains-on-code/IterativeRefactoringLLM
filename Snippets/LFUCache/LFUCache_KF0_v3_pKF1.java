...
public class LFUCache<K, V> {
     ...
     */
    public V get(K key) {
        CacheEntry cacheEntry = keyToEntryMap.get(key);
        if (cacheEntry == null) {
            return null;
        }
     ...
     *
     * @param cacheEntry The entry to be inserted into the list.
     */
    private void insertEntryByFrequency(CacheEntry cacheEntry) {
        if (leastFrequentEntry != null && mostFrequentEntry != null) {
            CacheEntry currentEntry = leastFrequentEntry;
            while (currentEntry != null) {
                if (currentEntry.frequency > cacheEntry.frequency) {
                    if (currentEntry == leastFrequentEntry) {
                        cacheEntry.nextEntry = currentEntry;
                        currentEntry.previousEntry = cacheEntry;
                        leastFrequentEntry = cacheEntry;
                        break;
                    } else {
                        cacheEntry.nextEntry = currentEntry;
                        cacheEntry.previousEntry = currentEntry.previousEntry;
                        currentEntry.previousEntry.nextEntry = cacheEntry;
                        currentEntry.previousEntry = cacheEntry;
                        break;
                    }
                } else {
                    ...
                    if (currentEntry == null) {
                        mostFrequentEntry.nextEntry = cacheEntry;
                        cacheEntry.previousEntry = mostFrequentEntry;
                        cacheEntry.nextEntry = null;
                        mostFrequentEntry = cacheEntry;
                        break;
                    }
                }
            }
     ...
     *
     * @param cacheEntry The entry to be removed from the list.
     */
    private void removeEntry(CacheEntry cacheEntry) {
        if (cacheEntry.previousEntry != null) {
            cacheEntry.previousEntry.nextEntry = cacheEntry.nextEntry;
        } else {
            leastFrequentEntry = cacheEntry.nextEntry;
        }

        if (cacheEntry.nextEntry != null) {
            cacheEntry.nextEntry.previousEntry = cacheEntry.previousEntry;
        } else {
            mostFrequentEntry = cacheEntry.previousEntry;
        }
    }
}