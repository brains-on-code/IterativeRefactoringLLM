...
public final class Class1 {
    ...
    public static int method1(final int rows, final int cols) {
        ...
        // Use the smaller dimension for the DP array to reduce space.
        if (rows > cols) {
    ...
    public static int method2(final int rows, final int cols) {
        ...
        // Base case: first column (only moves down).
        for (int r = 0; r < rows; r++) {
            dp[r][0] = 1;
        }

        // Base case: first row (only moves right).
        for (int c = 0; c < cols; c++) {
            dp[0][c] = 1;
        }

        // Transition: dp[r][c] = dp[r - 1][c] (top) + dp[r][c - 1] (left).
        for (int r = 1; r < rows; r++) {
            for (int c = 1; c < cols; c++) {
