...
public final class MatrixInversion {
    ...
    public static double[][] invert(double[][] matrix) {
        ...
        luDecomposition(matrix, pivotOrder);
            ...
            for (int row = dimension - 2; row >= 0; --row) {
                ...
                for (int k = row + 1; k < dimension; ++k) {
                    inverseMatrix[row][column] -=
                        matrix[pivotOrder[row]][k] * inverseMatrix[k][column];
                }
     ...
     */
    private static void luDecomposition(double[][] matrix, int[] pivotOrder) {
        ...
        for (int row = 0; row < dimension; ++row) {
            ...
            for (int column = 0; column < dimension; ++column) {
                double currentAbsoluteValue = Math.abs(matrix[row][column]);
                if (currentAbsoluteValue > maxAbsoluteValueInRow) {
                    maxAbsoluteValueInRow = currentAbsoluteValue;
                }
            }
        ...
        for (int column = 0; column < dimension - 1; ++column) {
            ...
            for (int row = column + 1; row < dimension; ++row) {
                ...
                for (int k = column + 1; k < dimension; ++k) {
                    matrix[pivotOrder[row]][k] -=
                        eliminationFactor * matrix[pivotOrder[column]][k];
                }
            }
        }
    }
