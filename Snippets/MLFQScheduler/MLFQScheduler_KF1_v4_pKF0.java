...
public class MultilevelFeedbackQueueScheduler {
    ...
    public void run() {
        while (!areAllQueuesEmpty()) {
            for (int level = 0; level < priorityQueues.size(); level++) {
                processQueueAtLevel(level);
            }
        }
    }

    private void processQueueAtLevel(int level) {
    ...
    private void executeProcessAtLevel(ScheduledProcess process, int level, int quantum) {
        ...
        requeueProcess(process, level);
    }

    private void requeueProcess(ScheduledProcess process, int currentLevel) {
        boolean hasLowerPriorityLevel = currentLevel < priorityQueues.size() - 1;

        if (hasLowerPriorityLevel) {
            process.incrementQueueLevel();
            priorityQueues.get(currentLevel + 1).offer(process);
        } else {
            priorityQueues.get(currentLevel).offer(process);
        }
    }
