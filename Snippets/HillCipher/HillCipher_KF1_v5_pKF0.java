...
public class HillCipher {
    ...
    private String transform(String text, int[][] matrix) {
        ...
        for (int index = 0; index < sanitizedText.length(); index += matrixSize) {
    ...
    private String sanitizeInput(String input) {
        ...
        return input.toUpperCase().replaceAll(NON_ALPHABET_REGEX, "");
    }

    private void validateKeyMatrix(int[][] matrix, int size) {
        int determinantMod = Math.floorMod(determinant(matrix, size), ALPHABET_SIZE);
        if (determinantMod == 0) {
    ...
    private void fillBlockFromText(String text, int[] block, int blockSize, int startIndex) {
        for (int i = 0; i < blockSize; i++) {
            ...
            char currentChar = textIndex < text.length()
    ...
    private void multiplyMatrixVector(int[][] matrix, int[] vector, int[] result, int size) {
        for (int row = 0; row < size; row++) {
            ...
            result[row] = Math.floorMod(value, ALPHABET_SIZE);
        }
    }
