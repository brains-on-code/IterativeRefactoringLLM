...
public final class MajorityElementFinder {
    ...
    public static Optional<Integer> findMajorityElement(int[] numbers) {
        ...
        int occurrences = countOccurrences(candidate, numbers);
    ...
    /**
     * Selects a candidate for the majority element using the
     ...
     */
    private static int countOccurrences(int candidate, int[] numbers) {
        int count = 0;
        for (int value : numbers) {
            if (value == candidate) {
                count++;
    ...
    /**
     * Determines whether the candidate's count is greater than half
     * of the array size.
     *
     * @param count     the number of occurrences of the candidate
     * @param arraySize the size of the array
     * @return {@code true} if the candidate is a majority element,
