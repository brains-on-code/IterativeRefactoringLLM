...
public final class WordPatternMatcher {
    ...
    private static boolean isPatternMatch(
        ...
        for (int nextInputIndex = inputIndex + 1; nextInputIndex <= input.length(); nextInputIndex++) {
            String candidateWord = input.substring(inputIndex, nextInputIndex);
            if (wordToPattern.containsKey(candidateWord)) {
                continue;
            }

            patternToWord.put(patternChar, candidateWord);
            wordToPattern.put(candidateWord, patternChar);
            ...
            wordToPattern.remove(candidateWord);
        }
