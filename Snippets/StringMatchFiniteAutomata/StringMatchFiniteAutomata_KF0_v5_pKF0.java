...
public final class StringMatchFiniteAutomata {
    ...
    private static int[][] buildTransitionTable(final String pattern) {
        ...
        for (int state = 0; state <= patternLength; state++) {
            for (int ch = 0; ch < ALPHABET_SIZE; ch++) {
                transitionTable[state][ch] = computeNextState(pattern, state, ch);
            }
        }
    ...
    private static int computeNextState(
        ...
        if (isDirectMatch(pattern, currentState, inputChar)) {
            return currentState + 1;
        }

        for (int candidateState = currentState; candidateState > 0; candidateState--) {
            if (!isPotentialPrefix(pattern, candidateState, inputChar)) {
                continue;
            }

            if (isPrefixSuffix(pattern, currentState, candidateState)) {
    ...
    private static boolean isDirectMatch(
        ...
        return currentState < pattern.length() && inputChar == pattern.charAt(currentState);
    }
    ...
    private static boolean isPotentialPrefix(
        ...
        return pattern.charAt(candidateState - 1) == inputChar;
    }
    ...
    private static boolean isPrefixSuffix(
        ...
        final int offset = currentState - candidateState + 1;
