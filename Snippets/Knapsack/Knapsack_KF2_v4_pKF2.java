...
public final class Knapsack {
    ...
    public static int knapSack(final int weightCapacity, final int[] weights, final int[] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            // Iterate capacities in reverse so each item is considered at most once
            for (int currentCapacity = weightCapacity; currentCapacity >= itemWeight; currentCapacity--) {
                int bestWithoutItem = maxValueForCapacity[currentCapacity];
                int bestWithItem = maxValueForCapacity[currentCapacity - itemWeight] + itemValue;

                maxValueForCapacity[currentCapacity] = Math.max(bestWithoutItem, bestWithItem);
            }
        }
