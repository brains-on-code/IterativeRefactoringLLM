...
public final class TurkishToLatinConverter {
    ...
    public static String convertTurkishToLatin(String turkishText) {
        if (turkishText == null) {
            return null;
        }

        String convertedText = turkishText;
        for (int index = 0; index < TURKISH_CHARACTERS.length; index++) {
            convertedText =
                convertedText.replace(TURKISH_CHARACTERS[index], LATIN_CHARACTERS[index]);
        }
