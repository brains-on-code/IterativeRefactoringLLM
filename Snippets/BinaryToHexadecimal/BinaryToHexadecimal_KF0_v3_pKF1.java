...
public final class BinaryToHexadecimal {
    ...
    public static String binToHex(int binaryInput) {
        ...
        while (remainingBinaryDigits != 0) {
            int decimalValueOfCurrentNibble = 0;
            for (int bitPosition = 0; bitPosition < BITS_PER_HEX_DIGIT; bitPosition++) {
                int currentBinaryDigit = remainingBinaryDigits % DECIMAL_BASE;
                ...
                decimalValueOfCurrentNibble += (int) (currentBinaryDigit * Math.pow(BINARY_BASE, bitPosition));
            }
            hexResult.insert(0, decimalToHexDigitMap.get(decimalValueOfCurrentNibble));
        }
