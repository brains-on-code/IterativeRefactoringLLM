...
public final class Combination {
    ...
    /**
     * Recursively builds all combinations of length {@code n} from {@code arr}.
     *
    ...
    private static <T> void backtracking(
        ...
        // Prune: not enough remaining elements to complete a combination
        if (index + n - current.size() > arr.length) {
            return;
        }
        ...
        for (int i = index; i < arr.length; i++) {
            current.add(arr[i]);
            backtracking(arr, n, i + 1, current, result);
            current.remove(arr[i]);
        }
    }
}