...
public final class Class1 {
    ...
    public static int method1(int[] nums, int target) {
        ...
        // Base case: there is exactly one subset (the empty subset) that makes sum 0
        for (int i = 0; i < n; i++) {
            dp[i][0] = 1;
        }
        ...
        // Fill the DP table for remaining elements
        for (int i = 1; i < n; i++) {
    ...
    public static int method2(int[] nums, int target) {
        ...
        // Base case: one way to make sum 0 (the empty subset)
        prev[0] = 1;
