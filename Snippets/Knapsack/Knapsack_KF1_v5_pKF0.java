    ...
    public static int solve(final int capacity, final int[] weights, final int[] values) {
        ...
        for (int itemIndex = 0; itemIndex < itemCount; itemIndex++) {
            ...
            for (int currentCapacity = capacity; currentCapacity >= itemWeight; currentCapacity--) {
                final int bestWithoutItem = maxValueForWeight[currentCapacity];
                final int bestWithItem = maxValueForWeight[currentCapacity - itemWeight] + itemValue;
                maxValueForWeight[currentCapacity] = Math.max(bestWithoutItem, bestWithItem);
            }
        }
