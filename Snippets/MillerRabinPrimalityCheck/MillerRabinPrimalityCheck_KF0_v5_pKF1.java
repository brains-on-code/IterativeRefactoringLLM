...
public final class MillerRabinPrimalityCheck {
     ...
     * @param rounds number of test rounds
     * @return true if the number is probably prime, false if it is composite
     */
    public static boolean millerRabin(long number, int rounds) {
        if (number < 4) {
    ...
    public static boolean deterministicMillerRabin(long number) {
        ...
        int[] deterministicBasesFor64Bit =
    ...
    private static boolean isComposite(
            long number, long base, long oddComponent, int exponentOfTwo) {
        long witness = modularExponentiation(base, oddComponent, number);
        if (witness == 1 || witness == number - 1) {
            return false;
        }
        for (int i = 1; i < exponentOfTwo; i++) {
            witness = modularExponentiation(witness, 2, number);
            if (witness == number - 1) {
                return false;
            }
        }
