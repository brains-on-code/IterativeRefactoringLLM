...
public final class MergeSortNoExtraSpace {
    ...
    private static void merge(int[] array, int start, int middle, int end, int maxElement) {
        ...
        while (leftIndex <= middle && rightIndex <= end) {
            int leftValue = getOriginalValue(array[leftIndex], maxElement);
            int rightValue = getOriginalValue(array[rightIndex], maxElement);

            if (leftValue <= rightValue) {
                encode(array, mergeIndex++, leftValue, maxElement);
                leftIndex++;
            } else {
                encode(array, mergeIndex++, rightValue, maxElement);
                rightIndex++;
            }
        }
        ...
        decodeRange(array, start, end, maxElement);
    }

    private static int getOriginalValue(int encodedValue, int maxElement) {
        return encodedValue % maxElement;
    }

    private static void encode(int[] array, int index, int value, int maxElement) {
