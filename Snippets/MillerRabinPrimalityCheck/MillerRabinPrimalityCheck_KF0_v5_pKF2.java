...
public final class MillerRabinPrimalityCheck {
    ...
    public static boolean millerRabin(long n, int k) {
        ...
        for (int i = 0; i < k; i++) {
            long a = randomBase(n);
            if (isComposite(n, a, decomposition.d, decomposition.s)) {
                return false;
            }
        }
        return true;
    ...
    /**
     * Checks if {@code n} is composite for the given base {@code a},
     * using the decomposition {@code n - 1 = 2^s * d}.
     *
    ...
    /**
     * Computes {@code (base^exponent) mod modulus} using binary exponentiation.
     */
    ...
    /**
     * Computes {@code (a * b) mod p} avoiding overflow by splitting operands.
     */
    ...
    /**
     * Decomposes {@code value} as {@code value = 2^s * d} with {@code d} odd.
     */
