...
public final class Class1 {
    ...
    public static ArrayList<Double> method1(int size, List<Double> coefficients) {
        ...
        double[][] eliminatedMatrix = method2(size, augmentedMatrix);
        return method3(size, workingMatrix, eliminatedMatrix);
    }
     ...
     */
    public static ArrayList<Double> method3(int size, double[][] workingMatrix, double[][] matrix) {
        ArrayList<Double> solution = new ArrayList<>();

        // Copy matrix into workingMatrix
        for (int row = 0; row < size; row++) {
            for (int col = 0; col <= size; col++) {
                workingMatrix[row][col] = matrix[row][col];
            }
        }
