...
public class IterativeTernarySearch implements SearchAlgorithm {
    ...
    public <T extends Comparable<T>> int find(T[] array, T target) {
        ...
        while (endIndex > startIndex) {
            int startComparison = array[startIndex].compareTo(target);
            int endComparison = array[endIndex].compareTo(target);
            ...
            int firstThirdIndex = startIndex + (endIndex - startIndex) / 3 + 1;
            int secondThirdIndex = endIndex - (endIndex - startIndex) / 3 - 1;

            if (array[firstThirdIndex].compareTo(target) <= 0) {
                startIndex = firstThirdIndex;
            } else {
                endIndex = secondThirdIndex;
            }
        }
