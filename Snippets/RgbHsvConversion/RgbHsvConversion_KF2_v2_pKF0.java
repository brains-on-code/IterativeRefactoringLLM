...
public final class RgbHsvConversion {
    ...
    public static double[] rgbToHsv(int red, int green, int blue) {
        ...
        double dRed = normalizeRgb(red);
        double dGreen = normalizeRgb(green);
        double dBlue = normalizeRgb(blue);
    ...
    private static double calculateHue(
            double dRed, double dGreen, double dBlue, double value, double chroma) {
        ...
        double hue;
        if (value == dRed) {
            hue = HUE_SECTION_SIZE * ((dGreen - dBlue) / chroma);
        } else if (value == dGreen) {
            hue = HUE_SECTION_SIZE * (2 + (dBlue - dRed) / chroma);
        } else {
            hue = HUE_SECTION_SIZE * (4 + (dRed - dGreen) / chroma);
        }
    ...
    private static boolean approximatelyEqualHsv(double[] hsv1, double[] hsv2) {
        double hueTolerance = 0.2;
        double svTolerance = 0.002;

        boolean hueClose = Math.abs(hsv1[0] - hsv2[0]) < hueTolerance;
        boolean saturationClose = Math.abs(hsv1[1] - hsv2[1]) < svTolerance;
        boolean valueClose = Math.abs(hsv1[2] - hsv2[2]) < svTolerance;
