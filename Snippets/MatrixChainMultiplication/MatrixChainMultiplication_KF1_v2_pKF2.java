...
public final class MatrixChainMultiplication {
    ...
    /** DP table: minimal multiplication cost for subchains. */
    private static int[][] costTable;

    /** DP table: optimal split index for subchains. */
    private static int[][] splitTable;

    /** Dimensions array: for matrices A1..An, stores [p0, p1, ..., pn]. */
    ...
    private static void fillDpTables(int n) {
        // Base case: cost of a single matrix is zero.
        for (int i = 1; i <= n; i++) {
            costTable[i][i] = 0;
        }

        // chainLength is the length of the subchain being considered.
