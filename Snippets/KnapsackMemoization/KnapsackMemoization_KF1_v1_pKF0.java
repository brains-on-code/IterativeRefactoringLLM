...
public class Class1 {
    ...
    int knapsackRecursive(int capacity, int[] weights, int[] values, int itemIndex, int[][] memo) {
        ...
        if (weights[itemIndex - 1] > capacity) {
            memo[itemIndex][capacity] = knapsackRecursive(capacity, weights, values, itemIndex - 1, memo);
        } else {
            int includeItem =
                values[itemIndex - 1]
                    + knapsackRecursive(capacity - weights[itemIndex - 1], weights, values, itemIndex - 1, memo);

            int excludeItem = knapsackRecursive(capacity, weights, values, itemIndex - 1, memo);
