...
public final class RgbHsvConversion {
    ...
    private static final int HUE_SECTION_SIZE = 60;
    ...
    public static double[] rgbToHsv(int red, int green, int blue) {
        ...
        double max = Math.max(Math.max(normalizedRed, normalizedGreen), normalizedBlue);
        double min = Math.min(Math.min(normalizedRed, normalizedGreen), normalizedBlue);
        double chroma = max - min;

        double value = max;
        double saturation = (value == 0) ? 0 : chroma / value;
        double hue = calculateHue(normalizedRed, normalizedGreen, normalizedBlue, max, chroma);
    ...
    private static double calculateHue(
            double red, double green, double blue, double max, double chroma) {
        if (chroma == 0) {
            return 0;
        }

        if (max == red) {
            return normalizeHue(HUE_SECTION_SIZE * ((green - blue) / chroma));
        } else if (max == green) {
            return normalizeHue(HUE_SECTION_SIZE * (2 + (blue - red) / chroma));
        } else {
            return normalizeHue(HUE_SECTION_SIZE * (4 + (red - green) / chroma));
        }
    }
