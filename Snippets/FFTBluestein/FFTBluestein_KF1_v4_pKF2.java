...
public final class Class1 {
    ...
    public static void method1(List<FFT.Complex> input, boolean invert) {
        ...
        initializeChirpWithZeros(chirp, convolutionSize);
        buildSymmetricChirpSequence(chirp, n, convolutionSize, sign);
        ...
        demodulateAndStoreResult(input, convolved, n, sign);

        if (invert) {
            normalizeResult(input, n);
        }
    }

    private static void initializeChirpWithZeros(List<FFT.Complex> chirp, int size) {
        for (int i = 0; i < size; i++) {
            chirp.add(new FFT.Complex());
        }
    }

    private static void buildSymmetricChirpSequence(
    ...
    private static void demodulateAndStoreResult(
    ...
    private static void normalizeResult(List<FFT.Complex> input, int n) {
        for (int i = 0; i < n; i++) {
