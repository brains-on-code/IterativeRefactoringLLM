...
public final class WineProblem {
    ...
    public static int maxProfitRecursive(int[] prices, int left, int right) {
        ...
        int profitIfSellLeft =
                maxProfitRecursive(prices, left + 1, right)
                        + prices[left] * year;
        int profitIfSellRight =
                maxProfitRecursive(prices, left, right - 1)
                        + prices[right] * year;
    ...
    public static int maxProfitTopDown(int[] prices, int left, int right, int[][] memo) {
        ...
        int profitIfSellLeft =
                maxProfitTopDown(prices, left + 1, right, memo)
                        + prices[left] * year;
        int profitIfSellRight =
                maxProfitTopDown(prices, left, right - 1, memo)
                        + prices[right] * year;
