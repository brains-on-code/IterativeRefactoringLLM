...
public final class Class1 {
    ...
    public static double[][] invert(double[][] matrix) {
        ...
        double[][] transformedIdentity = createIdentityMatrix(size);
        int[] pivotIndices = new int[size];

        luDecompose(matrix, pivotIndices);
        forwardSubstitution(matrix, transformedIdentity, pivotIndices);
        backwardSubstitution(matrix, transformedIdentity, pivotIndices, inverse);
    ...
    private static void forwardSubstitution(
        double[][] luMatrix,
        double[][] transformedIdentity,
        int[] pivotIndices
    ) {
        ...
        for (int col = 0; col < size - 1; ++col) {
            ...
            for (int row = col + 1; row < size; ++row) {
                ...
                for (int k = 0; k < size; ++k) {
                    transformedIdentity[pivotRowIndex][k] -=
                        multiplier * transformedIdentity[pivotColIndex][k];
                }
            }
        }
    }
    ...
    private static void backwardSubstitution(
        ...
        for (int col = 0; col < size; ++col) {
            ...
            inverse[size - 1][col] =
                transformedIdentity[lastPivotIndex][col] /
                luMatrix[lastPivotIndex][size - 1];

            for (int row = size - 2; row >= 0; --row) {
                int pivotRowIndex = pivotIndices[row];
                double sum = transformedIdentity[pivotRowIndex][col];
