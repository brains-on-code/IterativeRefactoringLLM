...
public class GepettoApplication {
	...
	public CommandLineRunner commandLineRunner(CommandLine.IFactory commandLineFactory, GepettoCommand gepettoCommand) {
		return rawArgs -> {
			try {
				CommandLine cli = new CommandLine(gepettoCommand, commandLineFactory);

				cli.setExecutionExceptionHandler((exception, cmd, parseResult) -> {
					System.err.println(exception.getMessage());
					return cmd.getCommandSpec().exitCodeOnExecutionException();
				});

				cli.setParameterExceptionHandler((parameterException, args) -> {
					...
					if (!CommandLine.UnmatchedArgumentException.printSuggestions(parameterException, System.err)) {
						CommandLine parameterCommandLine = parameterException.getCommandLine();
						parameterCommandLine.usage(System.err, parameterCommandLine.getColorScheme());
					}
					return parameterException.getCommandLine().getCommandSpec().exitCodeOnInvalidInput();
				});

				int commandExitCode = cli.execute(rawArgs);
