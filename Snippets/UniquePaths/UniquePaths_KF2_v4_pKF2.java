...
public final class UniquePaths {
     ...
     *
     * The 1D array {@code dp} stores, for the current row, the number of ways
     * to reach each column.
     *
    ...
    public static int uniquePaths(final int m, final int n) {
        // Ensure the DP array is as small as possible by keeping m <= n.
        if (m > n) {
            return uniquePaths(n, m);
        }

        int[] dp = new int[n];
        // First row: only one way to reach each cell (move right).
        Arrays.fill(dp, 1);
    ...
    public static int uniquePaths2(final int m, final int n) {
        ...
        // First column: only one way to reach each cell (move down).
