...
public final class Class1 {
    ...
    public static int method1(final int rows, final int cols) {
        ...
        // Keep the DP array as small as possible by ensuring cols >= rows.
        if (rows > cols) {
            return method1(cols, rows);
        }

        int[] dp = new int[cols];
        // Base case: first row has exactly one path to each cell (only moves to the right).
        Arrays.fill(dp, 1);

        for (int r = 1; r < rows; r++) {
            for (int c = 1; c < cols; c++) {
                // dp[c] (current cell) = paths from top (dp[c]) + paths from left (dp[c - 1])
    ...
    public static int method2(final int rows, final int cols) {
        ...
        // Base case: first column has exactly one path to each cell (only moves down).
        for (int r = 0; r < rows; r++) {
            dp[r][0] = 1;
        }

        // Base case: first row has exactly one path to each cell (only moves right).
