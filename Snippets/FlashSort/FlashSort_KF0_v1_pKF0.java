...
public class FlashSort implements SortAlgorithm {
    ...
    private <T extends Comparable<? super T>> void flashSort(T[] arr) {
        ...
        permute(arr, classificationArray, c1, min, n, m);
        insertionSort(arr);
    }
    ...
    private <T extends Comparable<? super T>> void permute(
        ...
        while (movedElements < n - 1) {
            while (currentIndex > classificationArray[currentClass] - 1) {
                currentIndex++;
            ...
            while (currentIndex != classificationArray[currentClass]) {
                currentClass = (int) (c1 * flash.compareTo(min));
                int targetIndex = classificationArray[currentClass] - 1;

                T temp = arr[targetIndex];
                arr[targetIndex] = flash;
                flash = temp;

                classificationArray[currentClass]--;
                movedElements++;
            }
        }
    }
