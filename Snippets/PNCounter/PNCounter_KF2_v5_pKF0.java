...
class PNCounter {
    ...
    public void merge(PNCounter other) {
        ...
        for (int i = 0; i < nodeCount; i++) {
            positiveCounter.put(i, maxCounterValue(positiveCounter, other.positiveCounter, i));
            negativeCounter.put(i, maxCounterValue(negativeCounter, other.negativeCounter, i));
        }
    }

    private int maxCounterValue(Map<Integer, Integer> first, Map<Integer, Integer> second, int index) {
        return Math.max(first.get(index), second.get(index));
    }
