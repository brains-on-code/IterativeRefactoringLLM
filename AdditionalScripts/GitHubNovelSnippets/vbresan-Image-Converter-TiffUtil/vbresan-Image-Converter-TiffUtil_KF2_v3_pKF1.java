...
public class TiffUtil {
    ...
    private static void convertSingleTiffPage(
        ...
        int directoryIndex = options.inDirectoryNumber;
    ...
    public static List<FilenameUriTuple> convertFromTIF(
        ...
        for (int directoryIndex = 0; directoryIndex < directoryCount; directoryIndex++) {
            ...
            try {
                convertSingleTiffPage(inputFile, outputFormat, options, outputFile);
            } catch (EncodeException e) {
                outputFile.delete();
                failedDirectoryIndices.add(directoryIndex);
            }

            convertedFileTuples.add(new FilenameUriTuple(outputFilename, Uri.fromFile(outputFile)));
        }
