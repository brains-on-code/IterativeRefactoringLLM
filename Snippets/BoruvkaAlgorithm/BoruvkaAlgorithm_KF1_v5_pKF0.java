...
final class Class1 {
    ...
    private static class BoruvkaMST {
        ...
        void addCheapestEdges(final Edge[] cheapestEdges) {
            for (int vertex = 0; vertex < graph.vertexCount; vertex++) {
                ...
                final int sourceSet = findSet(disjointSet, edge.source);
                final int destinationSet = findSet(disjointSet, edge.destination);

                if (sourceSet != destinationSet) {
                    mstEdges.add(edge);
                    union(disjointSet, sourceSet, destinationSet);
                }
            }
        }
        ...
        Edge[] findCheapestEdgesForComponents() {
            ...
            for (final Edge edge : graph.edges) {
                ...
                if (isCheaper(edge, cheapestEdges[sourceSet])) {
                    cheapestEdges[sourceSet] = edge;
                }
                if (isCheaper(edge, cheapestEdges[destinationSet])) {
                    cheapestEdges[destinationSet] = edge;
                }
            }
            return cheapestEdges;
        }

        private static boolean isCheaper(final Edge candidate, final Edge current) {
    ...
    static int findSet(final DisjointSetNode[] disjointSet, final int vertex) {
        final DisjointSetNode node = disjointSet[vertex];
        if (node.parent != vertex) {
            node.parent = findSet(disjointSet, node.parent);
        }
        return node.parent;
    }
