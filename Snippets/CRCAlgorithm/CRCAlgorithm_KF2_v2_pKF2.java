...
public class CRCAlgorithm {
    ...
    public void divideMessageWithPolynomial(boolean isCheckPhase) {
    ...
    private void xorWithGenerator(List<Integer> remainder) {
        for (int i = 0; i < generatorPolynomial.size(); i++) {
            int bit = remainder.get(i) == generatorPolynomial.get(i) ? 0 : 1;
            remainder.set(i, bit);
        }
    }
    ...
    private void updateStatistics(List<Integer> remainder) {
    ...
    public void introduceErrors() {
        for (int i = 0; i < message.size(); i++) {
            ...
            if (randomValue < bitErrorRate) {
                messageChanged = true;
                int flippedBit = message.get(i) == 1 ? 0 : 1;
                message.set(i, flippedBit);
            }
        }
