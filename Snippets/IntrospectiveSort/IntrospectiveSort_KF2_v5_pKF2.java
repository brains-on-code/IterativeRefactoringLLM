...
public class IntrospectiveSort implements SortAlgorithm {
    ...
    private static <T extends Comparable<T>> void heapify(
        ...
        int leftChild = offset + leftChildIndex;
        int rightChild = offset + rightChildIndex;
        int current = offset + largestIndex;

        if (leftChildIndex < heapSize
            && array[leftChild].compareTo(array[current]) > 0) {
            largestIndex = leftChildIndex;
            current = offset + largestIndex;
        }

        if (rightChildIndex < heapSize
            && array[rightChild].compareTo(array[current]) > 0) {
            largestIndex = rightChildIndex;
        }

        if (largestIndex != index) {
            SortUtils.swap(array, offset + index, offset + largestIndex);
