...
public class FloydWarshall {
        ...
        for (int intermediate = 1; intermediate <= vertexCount; intermediate++) {
            for (int source = 1; source <= vertexCount; source++) {
                for (int destination = 1; destination <= vertexCount; destination++) {
                    relaxEdge(source, destination, intermediate);
                }
            }
        }
    }
    ...
    /**
     * Relaxes the edge between source and destination using the given
     * intermediate vertex, updating the distance if a shorter path is found.
     */
    private void relaxEdge(int source, int destination, int intermediate) {
        int currentDistance = distanceMatrix[source][destination];
        int distanceViaIntermediate =
            distanceMatrix[source][intermediate] + distanceMatrix[intermediate][destination];

        if (distanceViaIntermediate < currentDistance) {
            distanceMatrix[source][destination] = distanceViaIntermediate;
        }
    }
