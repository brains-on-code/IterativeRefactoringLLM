...
/**
 * Knight's Tour solver using backtracking and Warnsdorff's heuristic.
 *
 * Problem:
...
public final class KnightsTour {
    ...
    /** Full board dimension, including a 2-cell buffer around the 8×8 area. */
    ...
    static boolean solve(int row, int column, int count) {
        ...
        for (int[] neighbor : neighbors) {
            ...
            boolean noOrphanCreated = !orphanDetected(count, nextRow, nextCol);
            if (noOrphanCreated && solve(nextRow, nextCol, count + 1)) {
                return true;
            }
