...
public class GepettoApplication {
	...
	@Bean
	@Profile("!test")
	public CommandLineRunner gepettoCommandLineRunner(
			CommandLine.IFactory commandLineFactory,
			GepettoCommand gepettoCommand
	) {
		return commandLineArguments -> {
			try {
				CommandLine gepettoCommandLine = new CommandLine(gepettoCommand, commandLineFactory);

				gepettoCommandLine.setExecutionExceptionHandler((executionException, commandLine, parseResult) -> {
					System.err.println(executionException.getMessage());
					return commandLine.getCommandSpec().exitCodeOnExecutionException();
				});

				gepettoCommandLine.setParameterExceptionHandler((parameterException, originalArguments) -> {
				...
				int commandExitCode = gepettoCommandLine.execute(commandLineArguments);
