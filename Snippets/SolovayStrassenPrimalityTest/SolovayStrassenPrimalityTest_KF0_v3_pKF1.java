...
final class SolovayStrassenPrimalityTest {
    ...
    public boolean isProbablePrime(long candidate, int iterations) {
        ...
        for (int iteration = 0; iteration < iterations; iteration++) {
            ...
            long adjustedJacobi = (candidate + jacobiSymbol(base, candidate)) % candidate;
            long exponent = (candidate - 1) / 2;
            long powerResult = modularExponentiation(base, exponent, candidate);

            if (adjustedJacobi == 0 || powerResult != adjustedJacobi) {
                return false;
            }
        }
