...
public final class FastInverseSqrt {
    ...
    private static float fastInverseSqrt(float number) {
        ...
        return newtonRaphsonRefinement(approximation, halfNumber);
    }
    ...
    private static double fastInverseSqrt(double number) {
        ...
        for (int i = 0; i < DOUBLE_REFINEMENT_ITERATIONS; i++) {
            approximation = newtonRaphsonRefinement(approximation, halfNumber);
        }

        return approximation;
    }

    private static float newtonRaphsonRefinement(float approximation, float halfNumber) {
        return approximation * (FLOAT_THREE_HALVES - halfNumber * approximation * approximation);
    }

    private static double newtonRaphsonRefinement(double approximation, double halfNumber) {
