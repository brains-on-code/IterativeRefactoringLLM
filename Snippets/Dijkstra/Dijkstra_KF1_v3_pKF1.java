...
class DijkstraGraph {
    ...
    public static class Edge {
        ...
        Edge(String sourceName, String destinationName, int weight) {
    ...
    DijkstraGraph(Edge[] edges) {
        ...
        for (Edge edge : edges) {
            ...
            if (!vertexByName.containsKey(edge.destinationName)) {
                vertexByName.put(edge.destinationName, new Vertex(edge.destinationName));
            }
        }

        for (Edge edge : edges) {
            Vertex sourceVertex = vertexByName.get(edge.sourceName);
            Vertex destinationVertex = vertexByName.get(edge.destinationName);
            sourceVertex.neighborToWeight.put(destinationVertex, edge.weight);
        }
    }
    ...
    private void computeShortestPaths(final NavigableSet<Vertex> unsettledVertices) {
        while (!unsettledVertices.isEmpty()) {
            ...
            for (Map.Entry<Vertex, Integer> neighborEntry :
                    currentVertex.neighborToWeight.entrySet()) {
