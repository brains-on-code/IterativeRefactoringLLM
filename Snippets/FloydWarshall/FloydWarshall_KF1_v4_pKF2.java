...
/**
 * Floyd–Warshall algorithm for all-pairs shortest paths on a weighted graph
        ...
        for (int intermediate = 1; intermediate <= vertexCount; intermediate++) {
            for (int source = 1; source <= vertexCount; source++) {
                for (int target = 1; target <= vertexCount; target++) {
                    int pathThroughIntermediate =
                        distance[source][intermediate] + distance[intermediate][target];
