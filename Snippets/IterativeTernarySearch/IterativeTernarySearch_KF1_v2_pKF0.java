...
public class Class1 implements SearchAlgorithm {
    ...
    public <T extends Comparable<T>> int method1(T[] array, T target) {
        ...
        while (left <= right) {
            ...
            if (right - left < 3) {
                // Interval too small for further ternary partitioning; break to avoid infinite loop
                break;
            }
            ...
            int firstThirdComparison = array[firstThird].compareTo(target);
            int secondThirdComparison = array[secondThird].compareTo(target);
            ...
            if (target.compareTo(array[firstThird]) < 0) {
                right = firstThird - 1;
            } else if (target.compareTo(array[secondThird]) > 0) {
                left = secondThird + 1;
            } else {
                left = firstThird + 1;
                right = secondThird - 1;
