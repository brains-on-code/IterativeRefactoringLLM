...
public final class MandelbrotGenerator {
        ...
        for (int pixelX = 0; pixelX < imageWidth; pixelX++) {
            for (int pixelY = 0; pixelY < imageHeight; pixelY++) {
                ...
                double normalizedEscapeTime =
                        computeNormalizedEscapeTime(complexReal, complexImaginary, maxIterations);
    ...
    private static Color getColorForNormalizedEscapeTime(double normalizedEscapeTime) {
        ...
        double maxColorComponent = 255.0;

        int hueSectorIndex = (int) Math.floor(hueDegrees / 60) % 6;
        double sectorPosition = hueDegrees / 60 - Math.floor(hueDegrees / 60);

        int maxComponent = (int) maxColorComponent;
        int minComponent = 0;
        int descendingComponent =
                (int) (maxColorComponent * (1 - sectorPosition * saturation));
        int ascendingComponent =
                (int) (maxColorComponent * (1 - (1 - sectorPosition) * saturation));

        switch (hueSectorIndex) {
            case 0:
                return new Color(maxComponent, ascendingComponent, minComponent);
            case 1:
                return new Color(descendingComponent, maxComponent, minComponent);
            case 2:
                return new Color(minComponent, maxComponent, ascendingComponent);
            case 3:
                return new Color(minComponent, descendingComponent, maxComponent);
            case 4:
                return new Color(ascendingComponent, minComponent, maxComponent);
            default:
                return new Color(maxComponent, minComponent, descendingComponent);
        }
    }
