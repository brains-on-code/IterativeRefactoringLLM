...
public class SimpleSubCipher {
    ...
    private static final int MAP_CAPACITY = 52;
    ...
    private Map<Character, Character> buildEncodingMap(String cipherSmall) {
        validateCipherAlphabet(cipherSmall);

        Map<Character, Character> encodingMap = new HashMap<>(MAP_CAPACITY);
    ...
    private Map<Character, Character> buildDecodingMap(String cipherSmall) {
        ...
        Map<Character, Character> decodingMap = new HashMap<>(MAP_CAPACITY);
    ...
    /**
     * Validates that the cipher alphabet is non-null and has the correct length.
     *
     * @param cipherSmall the 26-letter lowercase substitution alphabet
     */
    private void validateCipherAlphabet(String cipherSmall) {
        if (cipherSmall == null || cipherSmall.length() != ALPHABET_SIZE) {
            throw new IllegalArgumentException(
                "Cipher alphabet must be a non-null string of length " + ALPHABET_SIZE
            );
        }
    }
}