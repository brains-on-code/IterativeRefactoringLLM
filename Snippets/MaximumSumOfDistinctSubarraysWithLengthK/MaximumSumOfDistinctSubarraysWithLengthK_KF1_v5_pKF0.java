...
public final class Class1 {
    ...
    public static long method1(int windowSize, int... values) {
        ...
        initializeFirstWindow(values, windowSize, frequencyByValue);

        if (hasAllDistinctElements(frequencyByValue, windowSize)) {
            maxSum = currentSum = sumWindow(values, 0, windowSize);
        } else {
            currentSum = sumWindow(values, 0, windowSize);
        }
    ...
    private static void initializeFirstWindow(int[] values, int windowSize, Map<Integer, Integer> frequencyByValue) {
        for (int i = 0; i < windowSize; i++) {
            incrementFrequency(frequencyByValue, values[i]);
        }
    }

    private static long sumWindow(int[] values, int start, int windowSize) {
        long sum = 0;
