...
public final class AnagramUtils {
     ...
     * @return true if the strings are anagrams, false otherwise
     */
    public static boolean areAnagramsUsingAsciiArray(String first, String second) {
        if (!areNonNullAndSameLength(first, second)) {
            return false;
        }
     ...
     */
    public static boolean areAnagramsUsingLowercaseArray(String first, String second) {
        if (!areNonNullAndSameLength(first, second)) {
            return false;
        }
    ...
    private static void validateAsciiCharacter(char c) {
        if (c >= ASCII_SIZE) {
            throw new IllegalArgumentException(
                "Strings must contain only ASCII characters (0–127)."
            );
        }
    }
