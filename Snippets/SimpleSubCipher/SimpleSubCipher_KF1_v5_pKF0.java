...
public class SubstitutionCipher {
    ...
    private Map<Character, Character> buildSubstitutionMap(String key, boolean encode) {
        ...
        for (int i = 0; i < limit; i++) {
            char lowerAlphabetChar = LOWER_ALPHABET.charAt(i);
            char upperAlphabetChar = UPPER_ALPHABET.charAt(i);
            char lowerKeyChar = lowerKey.charAt(i);
            char upperKeyChar = upperKey.charAt(i);

            if (encode) {
                substitutionMap.put(lowerAlphabetChar, lowerKeyChar);
                substitutionMap.put(upperAlphabetChar, upperKeyChar);
            } else {
                substitutionMap.put(lowerKeyChar, lowerAlphabetChar);
                substitutionMap.put(upperKeyChar, upperAlphabetChar);
            }
        }
