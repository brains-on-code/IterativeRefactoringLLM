...
public final class NumberToWords {
    ...
    public static String convert(BigDecimal number) {
        ...
        if (isNegative) {
            resultWords.append(NEGATIVE_WORD_PREFIX);
        }
        resultWords.append(convertIntegerPartToWords(integerPart));

        if (!fractionalPartDigits.isEmpty()) {
            resultWords.append(POINT_WORD);
            for (char digitChar : fractionalPartDigits.toCharArray()) {
                int digit = Character.getNumericValue(digitChar);
                resultWords.append(" ").append(digit == 0 ? ZERO_WORD : UNIT_WORDS[digit]);
            }
        }
