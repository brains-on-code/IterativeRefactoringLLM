...
/**
 * Utility class for converting a decimal number to its string representation
...
public final class DecimalToAnyBase {
     ...
     * @param base    the base to convert to (must be between MIN_BASE and MAX_BASE)
    ...
    public static String convertToAnyBase(int decimal, int base) {
        ...
        List<Character> digits = extractDigits(decimal, base);
        return reverseDigitsToString(digits);
    }
    ...
    private static List<Character> extractDigits(int decimal, int base) {
        List<Character> digits = new ArrayList<>();
        int value = decimal;

        while (value > 0) {
            int remainder = value % base;
            digits.add(toBaseChar(remainder));
            value /= base;
        }
    ...
    private static String reverseDigitsToString(List<Character> digits) {
     ...
     */
    private static char toBaseChar(int value) {
        if (value >= 0 && value <= 9) {
