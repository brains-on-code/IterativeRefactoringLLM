...
public final class IPv4IPv6Mapper {
    ...
    private static final int IPV6_MAPPED_FF_FIRST_POSITION = 10;
    private static final int IPV6_MAPPED_FF_SECOND_POSITION = 11;
    ...
    public static String toIPv6MappedIPv4(String ipv4Address) throws UnknownHostException {
        ...
        ipv6AddressBytes[IPV6_MAPPED_FF_FIRST_POSITION] = (byte) 0xff;
        ipv6AddressBytes[IPV6_MAPPED_FF_SECOND_POSITION] = (byte) 0xff;
        System.arraycopy(
            ipv4AddressBytes,
            0,
            ipv6AddressBytes,
        ...
        for (int index = IPV6_MAPPED_PREFIX_LENGTH_BYTES; index < IPV6_ADDRESS_LENGTH_BYTES; index++) {
            ipv6MappedAddressBuilder.append(ipv6AddressBytes[index] & 0xFF);
            if (index < IPV6_ADDRESS_LENGTH_BYTES - 1) {
    ...
    public static String fromIPv6MappedIPv4(String ipv6MappedAddress) throws UnknownHostException {
        ...
        if (isIPv6MappedIPv4(addressBytes)) {
            byte[] ipv4AddressBytes =
                Arrays.copyOfRange(addressBytes, IPV6_MAPPED_PREFIX_LENGTH_BYTES, IPV6_ADDRESS_LENGTH_BYTES);
    ...
    private static boolean isIPv6MappedIPv4(byte[] addressBytes) {
        ...
        for (int index = 0; index < IPV6_MAPPED_PREFIX_LENGTH_BYTES - 2; index++) {
            if (addressBytes[index] != 0) {
                return false;
            }
        }

        return addressBytes[IPV6_MAPPED_FF_FIRST_POSITION] == (byte) 0xff
            && addressBytes[IPV6_MAPPED_FF_SECOND_POSITION] == (byte) 0xff;
    }
}