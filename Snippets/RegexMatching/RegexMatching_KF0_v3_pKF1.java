...
public final class RegexMatching {
    ...
    static boolean regexBU(String source, String pattern) {
        boolean[][] isMatchTable = new boolean[source.length() + 1][pattern.length() + 1];
        isMatchTable[source.length()][pattern.length()] = true;
        ...
        for (int sourceIndex = source.length(); sourceIndex >= 0; sourceIndex--) {
            for (int patternIndex = pattern.length() - 1; patternIndex >= 0; patternIndex--) {
                if (sourceIndex == source.length()) {
                    if (pattern.charAt(patternIndex) == '*') {
                        isMatchTable[sourceIndex][patternIndex] = isMatchTable[sourceIndex][patternIndex + 1];
                    } else {
                        isMatchTable[sourceIndex][patternIndex] = false;
                    }
                } else {
                    char sourceChar = source.charAt(sourceIndex);
                    char patternChar = pattern.charAt(patternIndex);
                    ...
                    boolean isMatch;
                    if (sourceChar == patternChar || patternChar == '?') {
                        isMatch = isMatchTable[sourceIndex + 1][patternIndex + 1];
                    } else if (patternChar == '*') {
                        boolean matchEmpty = isMatchTable[sourceIndex][patternIndex + 1];
                        boolean matchMultiple = isMatchTable[sourceIndex + 1][patternIndex];
                        isMatch = matchEmpty || matchMultiple;
                    } else {
                        isMatch = false;
                    }
                    isMatchTable[sourceIndex][patternIndex] = isMatch;
                }
            }
        }
        return isMatchTable[0][0];
    }
}