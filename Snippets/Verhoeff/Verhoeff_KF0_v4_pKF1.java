...
public final class Verhoeff {
     ...
     */
    private static final byte[] MULTIPLICATIVE_INVERSE_TABLE = {
        0,
        4,
        3,
        2,
    ...
    public static boolean verhoeffCheck(String digits) {
        ...
        int[] digitArray = toIntArray(digits);
        int checksum = calculateChecksum(digitArray);
        return checksum == 0;
    }
    ...
    public static String addVerhoeffChecksum(String initialDigits) {
        ...
        String digitsWithPlaceholderChecksum = initialDigits + "0";
        int[] digitArray = toIntArray(digitsWithPlaceholderChecksum);

        int checksum = calculateChecksum(digitArray);
        checksum = MULTIPLICATIVE_INVERSE_TABLE[checksum];
    ...
    private static void printGeneratedChecksum(String input) {
        String resultWithChecksum = addVerhoeffChecksum(input);
        System.out.println(
            "Generate and add checksum to initial value '" + input + "'. Result: '" + resultWithChecksum + "'"
        );
    }
    ...
    private static int[] toIntArray(String numericString) {
        return numericString.chars().map(ch -> Character.digit(ch, 10)).toArray();
    }
