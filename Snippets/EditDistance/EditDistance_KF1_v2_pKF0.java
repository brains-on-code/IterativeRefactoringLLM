...
public final class LevenshteinDistance {
    ...
    public static int computeRecursive(String source, String target) {
        ...
        return computeRecursive(source, target, memo);
    }
     ...
     * @return the edit distance between {@code source} and {@code target}
     */
    private static int computeRecursive(String source, String target, int[][] memo) {
        int sourceLength = source.length();
        int targetLength = target.length();
        ...
        if (source.charAt(0) == target.charAt(0)) {
            memo[sourceLength][targetLength] =
                computeRecursive(source.substring(1), target.substring(1), memo);
        } else {
            int insertCost = computeRecursive(source, target.substring(1), memo);
            int deleteCost = computeRecursive(source.substring(1), target, memo);
            int replaceCost = computeRecursive(source.substring(1), target.substring(1), memo);
