        ...
        for (int sourceIndex = source.length(); sourceIndex >= 0; sourceIndex--) {
            for (int patternIndex = pattern.length() - 1; patternIndex >= 0; patternIndex--) {
                if (sourceIndex == source.length()) {
                    if (pattern.charAt(patternIndex) == '*') {
                        dp[sourceIndex][patternIndex] = dp[sourceIndex][patternIndex + 1];
                    } else {
                        dp[sourceIndex][patternIndex] = false;
                    }
                } else {
                    char sourceChar = source.charAt(sourceIndex);
                    char patternChar = pattern.charAt(patternIndex);
                    ...
                    boolean isMatch;
                    if (sourceChar == patternChar || patternChar == '?') {
                        isMatch = dp[sourceIndex + 1][patternIndex + 1];
                    } else if (patternChar == '*') {
                        boolean matchEmpty = dp[sourceIndex][patternIndex + 1];
                        boolean matchMultiple = dp[sourceIndex + 1][patternIndex];
                        isMatch = matchEmpty || matchMultiple;
                    } else {
                        isMatch = false;
                    }
                    dp[sourceIndex][patternIndex] = isMatch;
                }
            }
        }
        return dp[0][0];
