...
public final class NumberToWords {
    ...
    private static String convertIntegerPartToWords(BigDecimal integerPart) {
        ...
        while (remainingValue.compareTo(BigDecimal.ZERO) > 0) {
            ...
            if (currentThreeDigitGroup > 0) {
                String groupWords = convertThreeDigitNumberToWords(currentThreeDigitGroup);
                if (scaleIndex > 0) {
    ...
    private static String convertThreeDigitNumberToWords(int threeDigitNumber) {
        ...
        if (threeDigitNumber < 100) {
            String tensName = TENS_NAMES[threeDigitNumber / 10];
            String unitName = threeDigitNumber % 10 > 0 ? " " + UNIT_NAMES[threeDigitNumber % 10] : "";
            return tensName + unitName;
        }

        String hundredsName = UNIT_NAMES[threeDigitNumber / 100] + " Hundred";
        String remainderWords =
            threeDigitNumber % 100 > 0 ? " " + convertThreeDigitNumberToWords(threeDigitNumber % 100) : "";
        return hundredsName + remainderWords;
    }
}