...
class PNCounter {
    ...
    public void increment() {
        updateCounter(positiveCounter, nodeId, 1);
    }
    ...
    public void decrement() {
        updateCounter(negativeCounter, nodeId, 1);
    }

    private void updateCounter(Map<Integer, Integer> counter, int nodeId, int delta) {
        counter.put(nodeId, counter.get(nodeId) + delta);
    }
    ...
    private boolean isStrictlyGreaterAtIndex(PNCounter other, int index) {
        boolean positiveGreater = this.positiveCounter.get(index) > other.positiveCounter.get(index);
        boolean negativeGreater = this.negativeCounter.get(index) > other.negativeCounter.get(index);
        return positiveGreater && negativeGreater;
    }
    ...
    private void mergeIndex(PNCounter other, int index) {
        positiveCounter.put(index, Math.max(this.positiveCounter.get(index), other.positiveCounter.get(index)));
        negativeCounter.put(index, Math.max(this.negativeCounter.get(index), other.negativeCounter.get(index)));
    }
