package com.thealgorithms.datastructures.graphs;

import java.util.Arrays;
import java.util.Scanner;

class BellmanFord {

    private final int vertexCount;
    private final int edgeCapacity;
    private final Edge[] edges;
    private int edgeIndex = 0;

    BellmanFord(int vertexCount, int edgeCapacity) {
        this.vertexCount = vertexCount;
        this.edgeCapacity = edgeCapacity;
        this.edges = new Edge[edgeCapacity];
    }

    static class Edge {
        final int source;
        final int destination;
        final int weight;

        Edge(int source, int destination, int weight) {
            this.source = source;
            this.destination = destination;
            this.weight = weight;
        }
    }

    public static void main(String[] args) {
        new BellmanFord(0, 0).runFromInput();
    }

    public void runFromInput() {
        try (Scanner scanner = new Scanner(System.in)) {
            int vertices = readVertexCount(scanner);
            int edgesCount = readEdgeCount(scanner);
            Edge[] inputEdges = readEdges(scanner, edgesCount);

            int[] distance = createInitializedArray(vertices, Integer.MAX_VALUE);
            int[] parent = createInitializedArray(vertices, -1);

            int sourceVertex = 0;
            distance[sourceVertex] = 0;

            relaxEdges(vertices, edgesCount, inputEdges, distance, parent);

            if (hasNegativeCycle(edgesCount, inputEdges, distance)) {
                System.out.println("Negative cycle detected");
                return;
            }

            printDistancesFromSource(vertices, distance, sourceVertex);
            printPathsFromSource(vertices, parent, sourceVertex);
        }
    }

    private int readVertexCount(Scanner scanner) {
        System.out.println("Enter number of vertices and edges:");
        return scanner.nextInt();
    }

    private int readEdgeCount(Scanner scanner) {
        return scanner.nextInt();
    }

    private Edge[] readEdges(Scanner scanner, int edgesCount) {
        System.out.println("Input edges (source destination weight):");
        Edge[] inputEdges = new Edge[edgesCount];
        for (int i = 0; i < edgesCount; i++) {
            int source = scanner.nextInt();
            int destination = scanner.nextInt();
            int weight = scanner.nextInt();
            inputEdges[i] = new Edge(source, destination, weight);
        }
        return inputEdges;
    }

    private int[] createInitializedArray(int size, int initialValue) {
        int[] array = new int[size];
        Arrays.fill(array, initialValue);
        return array;
    }

    private void printDistancesFromSource(int vertices, int[] distance, int sourceVertex) {
        System.out.println("Distances from source (" + sourceVertex + "):");
        for (int vertex = 0; vertex < vertices; vertex++) {
            System.out.println(vertex + " " + distance[vertex]);
        }
    }

    private void printPathsFromSource(int vertices, int[] parent, int sourceVertex) {
        System.out.println("Paths from source (" + sourceVertex + "):");
        for (int vertex = 0; vertex < vertices; vertex++) {
            System.out.print(sourceVertex + " ");
            printPath(parent, vertex);
            System.out.println();
        }
    }

    private void printPath(int[] parent, int vertex) {
        if (vertex == -1) {
            return;
        }
        printPath(parent, parent[vertex]);
        System.out.print(vertex + " ");
    }

    public void show(int source, int destination, Edge[] edges) {
        int[] distance = createInitializedArray(vertexCount, Integer.MAX_VALUE);
        int[] parent = createInitializedArray(vertexCount, -1);

        distance[source] = 0;

        relaxEdges(vertexCount, edges.length, edges, distance, parent);

        if (hasNegativeCycle(edges.length, edges, distance)) {
            System.out.println("Negative cycle detected");
            return;
        }

        System.out.println("Distance from " + source + " to " + destination + " is: " + distance[destination]);
        System.out.println("Path followed:");
        System.out.print(source + " ");
        printPath(parent, destination);
        System.out.println();
    }

    private void relaxEdges(int vertices, int edgesCount, Edge[] edges, int[] distance, int[] parent) {
        for (int iteration = 0; iteration < vertices - 1; iteration++) {
            for (int edgeIdx = 0; edgeIdx < edgesCount; edgeIdx++) {
                Edge edge = edges[edgeIdx];
                if (distance[edge.source] == Integer.MAX_VALUE) {
                    continue;
                }
                int newDistance = distance[edge.source] + edge.weight;
                if (distance[edge.destination] > newDistance) {
                    distance[edge.destination] = newDistance;
                    parent[edge.destination] = edge.source;
                }
            }
        }
    }

    private boolean hasNegativeCycle(int edgesCount, Edge[] edges, int[] distance) {
        for (int edgeIdx = 0; edgeIdx < edgesCount; edgeIdx++) {
            Edge edge = edges[edgeIdx];
            if (distance[edge.source] == Integer.MAX_VALUE) {
                continue;
            }
            if (distance[edge.destination] > distance[edge.source] + edge.weight) {
                return true;
            }
        }
        return false;
    }

    public void addEdge(int source, int destination, int weight) {
        if (edgeIndex >= edgeCapacity) {
            throw new IllegalStateException("Cannot add more edges than the initialized edgeCapacity");
        }
        edges[edgeIndex++] = new Edge(source, destination, weight);
    }

    public Edge[] getEdgeArray() {
        return Arrays.copyOf(edges, edgeIndex);
    }
}