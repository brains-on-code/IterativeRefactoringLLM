...
public final class Huffman {
    ...
    private static HuffmanNode buildHuffmanTree(char[] characters, int[] frequencies) {
        ...
        while (priorityQueue.size() > 1) {
            HuffmanNode firstMin = pollNode(priorityQueue, "first");
            HuffmanNode secondMin = pollNode(priorityQueue, "second");
    ...
    private static HuffmanNode pollNode(PriorityQueue<HuffmanNode> queue, String position) {
        return Objects.requireNonNull(
            queue.poll(),
            "Priority queue returned null while polling " + position + " minimum node."
        );
    }
