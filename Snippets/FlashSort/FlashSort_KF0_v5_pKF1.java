...
public class FlashSort implements SortAlgorithm {
    ...
    private <T extends Comparable<? super T>> void flashSort(T[] array) {
        ...
        final int numberOfClasses = (int) (classRatio * length);
        final int[] classCounts = new int[numberOfClasses];

        final double normalizationFactor =
            (double) (numberOfClasses - 1) / array[maxIndex].compareTo(minValue);

        classifyElements(array, classCounts, normalizationFactor, minValue);
        accumulateClassCounts(classCounts);
        permuteElements(array, classCounts, normalizationFactor, minValue, length, numberOfClasses);
        insertionSort(array);
    }
    ...
    private <T extends Comparable<? super T>> void permuteElements(
        ...
        while (movedElementCount < length - 1) {
            while (currentIndex > classCounts[currentClassIndex] - 1) {
                currentIndex++;
                currentClassIndex =
                    (int) (normalizationFactor * array[currentIndex].compareTo(minValue));
            }
            currentElement = array[currentIndex];
            while (currentIndex != classCounts[currentClassIndex]) {
                currentClassIndex =
                    (int) (normalizationFactor * currentElement.compareTo(minValue));
