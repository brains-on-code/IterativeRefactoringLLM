...
public final class KCenters {
    ...
    /**
     * Uses a greedy heuristic to choose k centers that minimize the maximum
     * distance from any node to its nearest center.
     *
    ...
    public static int findKCenters(int[][] distances, int k) {
        ...
        // Start with node 0 as the first center
        isCenter[0] = true;
        for (int i = 1; i < n; i++) {
        ...
        for (int centersChosen = 1; centersChosen < k; centersChosen++) {
            ...
            // Node with the largest distance to its nearest center
            for (int i = 0; i < n; i++) {
                if (!isCenter[i] && (farthestNode == -1 || nearestCenterDist[i] > nearestCenterDist[farthestNode])) {
                    farthestNode = i;
                }
            }
            ...
            for (int i = 0; i < n; i++) {
                nearestCenterDist[i] = Math.min(nearestCenterDist[i], distances[farthestNode][i]);
            }
        }
