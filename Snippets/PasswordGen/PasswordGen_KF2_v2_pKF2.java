...
final class PasswordGen {
     ...
     * inclusive.
     *
     * @param minLength the minimum length of the password (must be > 0 and <= maxLength)
     * @param maxLength the maximum length of the password (must be > 0 and >= minLength)
    ...
    private static void validateLengthRange(int minLength, int maxLength) {
        boolean nonPositive = minLength <= 0 || maxLength <= 0;
        boolean invertedRange = minLength > maxLength;

        if (nonPositive || invertedRange) {
