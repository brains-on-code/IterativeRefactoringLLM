...
public final class MaximumMinimumWindow {
    ...
    public static int[] calculateMaxOfMin(int[] arr, int n) {
        ...
        /*
         * result[len] = maximum of minimums for all windows of size len (1..n)
         * result[0] is unused.
         */
        int[] result = new int[n + 1];

        // For each arr[i], it is the minimum in all windows of length (right[i] - left[i] - 1)
        for (int i = 0; i < n; i++) {
            int len = right[i] - left[i] - 1;
            result[len] = Math.max(result[len], arr[i]);
        }

        // Propagate values: for smaller windows, the answer is at least that of larger windows
