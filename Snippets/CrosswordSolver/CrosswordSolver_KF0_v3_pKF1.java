...
public final class CrosswordSolver {
    ...
    public static void placeWord(
        ...
        for (int charIndex = 0; charIndex < wordLength; charIndex++) {
            ...
            if (isVertical) {
                grid[startRow + charIndex][startCol] = currentChar;
            } else {
                grid[startRow][startCol + charIndex] = currentChar;
            }
        }
    }
                ...
                if (grid[rowIndex][columnIndex] == ' ') {
                    for (String word : new ArrayList<>(remainingWords)) {
                        for (boolean isVertical : new boolean[] {true, false}) {
                            if (canPlaceWord(grid, word, rowIndex, columnIndex, isVertical)) {
                                placeWord(grid, word, rowIndex, columnIndex, isVertical);
                                ...
                                removeWord(grid, word, rowIndex, columnIndex, isVertical);
                            }
                        }
                    }
                    return false;
