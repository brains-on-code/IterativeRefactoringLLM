...
public class IntrospectiveSort implements SortAlgorithm {
    ...
    private static <T extends Comparable<T>> void heapify(
        ...
        int largestIndex = offset + largest;
        int leftIndex = offset + left;
        int rightIndex = offset + right;

        if (left < heapSize && array[leftIndex].compareTo(array[largestIndex]) > 0) {
            largest = left;
            largestIndex = leftIndex;
        }

        if (right < heapSize && array[rightIndex].compareTo(array[largestIndex]) > 0) {
            largest = right;
            largestIndex = rightIndex;
        }

        if (largest != index) {
            SortUtils.swap(array, offset + index, largestIndex);
