package com.thealgorithms.scheduling;

import java.util.Arrays;
import java.util.Comparator;

public final class JobSchedulingWithDeadline {

    private JobSchedulingWithDeadline() {
        // Utility class; prevent instantiation
    }

    static class Job {
        final int id;
        final int arrivalTime;
        final int deadline;
        final int profit;

        Job(int id, int arrivalTime, int deadline, int profit) {
            this.id = id;
            this.arrivalTime = arrivalTime;
            this.deadline = deadline;
            this.profit = profit;
        }
    }

    /**
     * Returns an array of two integers:
     * [0] = number of scheduled jobs
     * [1] = total profit
     */
    public static int[] jobSequencingWithDeadlines(Job[] jobs) {
        if (jobs == null || jobs.length == 0) {
            return new int[] {0, 0};
        }

        sortJobsByProfitDescending(jobs);

        int maxDeadline = findMaxDeadline(jobs);
        if (maxDeadline <= 0) {
            return new int[] {0, 0};
        }

        int[] timeSlots = createEmptyTimeSlots(maxDeadline);
        int scheduledJobsCount = 0;
        int totalProfit = 0;

        for (Job job : jobs) {
            if (!isFeasible(job)) {
                continue;
            }

            int latestSlot = latestPossibleSlot(job, maxDeadline);
            int earliestSlot = earliestPossibleSlot(job);

            int scheduledSlot = findAvailableSlot(timeSlots, earliestSlot, latestSlot);
            if (scheduledSlot != -1) {
                timeSlots[scheduledSlot] = job.id;
                scheduledJobsCount++;
                totalProfit += job.profit;
            }
        }

        return new int[] {scheduledJobsCount, totalProfit};
    }

    private static void sortJobsByProfitDescending(Job[] jobs) {
        Arrays.sort(jobs, Comparator.comparingInt(job -> job.profit).reversed());
    }

    private static int findMaxDeadline(Job[] jobs) {
        return Arrays.stream(jobs)
            .mapToInt(job -> job.deadline)
            .max()
            .orElse(0);
    }

    private static int[] createEmptyTimeSlots(int maxDeadline) {
        int[] timeSlots = new int[maxDeadline];
        Arrays.fill(timeSlots, -1);
        return timeSlots;
    }

    private static boolean isFeasible(Job job) {
        return job.arrivalTime <= job.deadline;
    }

    private static int latestPossibleSlot(Job job, int maxDeadline) {
        return Math.min(job.deadline, maxDeadline) - 1;
    }

    private static int earliestPossibleSlot(Job job) {
        return Math.max(job.arrivalTime, 1) - 1;
    }

    private static int findAvailableSlot(int[] timeSlots, int earliestSlot, int latestSlot) {
        for (int slot = latestSlot; slot >= earliestSlot; slot--) {
            if (timeSlots[slot] == -1) {
                return slot;
            }
        }
        return -1;
    }
}