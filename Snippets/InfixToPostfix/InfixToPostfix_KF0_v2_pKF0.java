...
public final class InfixToPostfix {
    ...
    private static final Pattern NON_BRACKETS_PATTERN = Pattern.compile("[^(){}\\[\\]<>]");
    ...
    public static String infix2PostFix(String infixExpression) throws Exception {
        ...
        for (char token : infixExpression.toCharArray()) {
            if (Character.isLetterOrDigit(token)) {
                ...
                popUntilMatchingOpeningParenthesis(postfix, operators, token);
    ...
    private static void popUntilMatchingOpeningParenthesis(
            StringBuilder output, Stack<Character> operators, char closingParenthesis) {

        char expectedOpening = '(';

        while (!operators.isEmpty() && operators.peek() != expectedOpening) {
            output.append(operators.pop());
        }

        if (!operators.isEmpty() && operators.peek() == expectedOpening) {
            operators.pop(); // Remove matching opening parenthesis
        }
    }
    ...
    private static String extractBrackets(String input) {
        Matcher matcher = NON_BRACKETS_PATTERN.matcher(input);
