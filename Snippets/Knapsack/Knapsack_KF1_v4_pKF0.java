...
public final class Knapsack01 {
    ...
    public static int solve(final int capacity, final int[] weights, final int[] values) {
        ...
        for (int i = 0; i < itemCount; i++) {
            final int itemWeight = weights[i];
            final int itemValue = values[i];

            for (int currentCapacity = capacity; currentCapacity >= itemWeight; currentCapacity--) {
                final int withoutItem = maxValueForWeight[currentCapacity];
                final int withItem = maxValueForWeight[currentCapacity - itemWeight] + itemValue;
