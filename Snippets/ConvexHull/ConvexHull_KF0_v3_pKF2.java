...
public final class ConvexHull {
    ...
    /**
     * Returns {@code true} if point {@code k} lies to the left of the directed
     * line from {@code i} to {@code j}, or lies on the segment [i, j].
     */
    private static boolean isLeftOrOnSegment(Point i, Point j, Point k) {
        int orientation = Point.orientation(i, j, k);

        if (orientation > 0) {
            // k is strictly to the left of segment ij
            return true;
        }
        if (orientation < 0) {
            // k is strictly to the right of segment ij
            return false;
        }
