    ...
    private static class BoruvkaState {
        ...
        void merge(final Edge[] cheapest) {
            for (int vertex = 0; vertex < graph.vertexCount; vertex++) {
                ...
                final int srcComponent = find(components, edge.src);
                final int destComponent = find(components, edge.dest);
                ...
                result.add(edge);
                union(components, srcComponent, destComponent);
            }
        }
        ...
        Edge[] computeCheapestEdges() {
            ...
            for (final Edge edge : graph.edges) {
                ...
                updateCheapestEdge(cheapest, srcComponent, edge);
                updateCheapestEdge(cheapest, destComponent, edge);
            }

            return cheapest;
        }

        private static void updateCheapestEdge(final Edge[] cheapest, final int componentIndex, final Edge candidateEdge) {
            final Edge currentCheapest = cheapest[componentIndex];
            if (currentCheapest == null || candidateEdge.weight < currentCheapest.weight) {
                cheapest[componentIndex] = candidateEdge;
            }
        }
