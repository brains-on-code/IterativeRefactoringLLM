...
public final class StringMatchFiniteAutomata {
    ...
    /** Number of possible char values (size of the input alphabet). */
    ...
    private static int computeNextState(
        ...
        // Case 1: we can extend the current match by one character.
        if (state < patternLength && ch == pattern.charAt(state)) {
            return state + 1;
        }

        /*
         * Case 2: we cannot extend directly. We must find the longest prefix of
         * the pattern that is also a suffix of (pattern[0..state-1] + ch).
         */
        ...
        // Case 3: no prefix-suffix match; go back to the initial state.
        return 0;
    }
