 ...
 * <ul>
 *   <li>a grow-only counter for increments ({@code positiveCounts})</li>
 *   <li>a grow-only counter for decrements ({@code negativeCounts})</li>
 * </ul>
 *
 * <p>The logical value is: {@code sum(positiveCounts) - sum(negativeCounts)}.
 */
class PNCounter {
     ...
     */
    public int value() {
        return sumCounts(positiveCounts) - sumCounts(negativeCounts);
    }
    ...
    public boolean compare(PNCounter other) {
        ...
        for (int i = 0; i < replicaCount; i++) {
            boolean hasGreaterPositive = positiveCounts.get(i) > other.positiveCounts.get(i);
            boolean hasGreaterNegative = negativeCounts.get(i) > other.negativeCounts.get(i);
