...
public class Class1 implements SortAlgorithm {
    ...
    private <T extends Comparable<T>> void bitonicSort(
        ...
        bitonicSort(array, start + mid, mid, Direction.DESCENDING);
    ...
    private <T extends Comparable<T>> void bitonicMerge(
        ...
        int end = start + mid;
        for (int leftIndex = start; leftIndex < end; leftIndex++) {
            int rightIndex = leftIndex + mid;
            if (!comparator.test(array[leftIndex], array[rightIndex])) {
