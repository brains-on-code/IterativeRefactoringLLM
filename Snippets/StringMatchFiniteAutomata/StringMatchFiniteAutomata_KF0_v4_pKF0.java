...
public final class StringMatchFiniteAutomata {
    ...
    public static Set<Integer> searchPattern(final String text, final String pattern) {
        ...
        final int[][] transitionTable = buildTransitionTable(pattern, patternLength);
     ...
     */
    private static int[][] buildTransitionTable(final String pattern, final int patternLength) {
    ...
    private static boolean isPrefixSuffix(
        ...
        for (int i = 0; i < candidateState - 1; i++) {
            if (pattern.charAt(i) != pattern.charAt(currentState - candidateState + i + 1)) {
                return false;
            }
        }
        return true;
