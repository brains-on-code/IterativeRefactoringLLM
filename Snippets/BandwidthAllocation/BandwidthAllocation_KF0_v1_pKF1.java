...
public final class BandwidthAllocation {
    ...
    public static int maxValue(int totalBandwidth, int[] userDemands, int[] userValues) {
        ...
        double[][] userValueRatios = new double[userCount][2]; // {userIndex, valuePerUnitBandwidth}

        for (int userIndex = 0; userIndex < userCount; userIndex++) {
            userValueRatios[userIndex][0] = userIndex;
            userValueRatios[userIndex][1] = (double) userValues[userIndex] / userDemands[userIndex];
        }

        Arrays.sort(userValueRatios, (firstUser, secondUser) -> Double.compare(secondUser[1], firstUser[1]));

        int totalValue = 0;
        for (int i = 0; i < userCount; i++) {
            int userIndex = (int) userValueRatios[i][0];
            int demand = userDemands[userIndex];
            int value = userValues[userIndex];
            double valuePerUnit = userValueRatios[i][1];

            if (totalBandwidth >= demand) {
                totalValue += value;
                totalBandwidth -= demand;
            } else {
                totalValue += (int) (valuePerUnit * totalBandwidth);
                break;
            }
        }
        return totalValue;
